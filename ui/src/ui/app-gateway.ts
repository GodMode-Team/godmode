...
export async function switchModelFromChat(host: GatewayHost, modelId: string) {
  ...
  try {
    // Pass sessionKey so the backend sets the session-level modelOverride
    // (session override has highest priority in OpenClaw's 3-tier model resolution)
    await host.client.request("godmode.config.model.set", {
      primary: modelId,
...
async function handleOpenTaskParam(host: GodModeApp) {
  ...
  try {
    ...
    if (result?.sessionKey) {
      ...
      // Ensure session appears as a tab (prevents ghost sessions from deep links)
      const lower = key.toLowerCase();
      const isMainAlias = lower === "main" || lower === "agent:main:main" || lower.endsWith(":main");
      if (!isMainAlias && !host.settings.openTabs.includes(key)) {
        applySettings(host as unknown as Parameters<typeof applySettings>[0], {
          ...host.settings,
          openTabs: [...host.settings.openTabs, key],
          sessionKey: key,
      ...
      await loadChatHistory(host, key);
    }
  } catch (err) {
