...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/godmode/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/godmode");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/godmode/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/godmode";
    const app = mountApp("/godmode/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/godmode");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/godmode/sessions");
  });
