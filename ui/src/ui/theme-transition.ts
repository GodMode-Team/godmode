...
export const startThemeTransition = ({
  ...
  if (canUseViewTransition) {
    ...
    // Failsafe timeout: ensure cleanup happens even if View Transition API misbehaves
    // This prevents the "black triangle" glitch where the transition class gets stuck
    const failsafeTimeout = setTimeout(() => {
      cleanupThemeTransition(root);
    }, 1000); // 1 second max - animation is 0.4s, so this is generous
