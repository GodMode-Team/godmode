import{d as k,b as a,A as r,o as x,g as C,r as u,t as E}from"./lit-core-CTInmNPB.js";import{a as m,o as _}from"./index-gdMsyFDQ.js";import{a as A}from"./views-settings-Bxvk8En_.js";import{f as T}from"./ctrl-settings-BplS35Q0.js";import"./markdown-i_gIkIP3.js";async function D(e){if(!e.client||!e.connected)return;const n=e.secondBrainSubtab??"identity";e.secondBrainLoading=!0,e.secondBrainError=null;try{if(n==="identity"){const s=await e.client.request("secondBrain.identity",{});e.secondBrainIdentity=s}else if(n==="knowledge"){const[s,i,d]=await Promise.all([e.client.request("secondBrain.memoryBank",{}),e.client.request("secondBrain.research",{}),e.client.request("secondBrain.fileTree",{depth:4})]);e.secondBrainMemoryBank=s,e.secondBrainResearchData=i,e.secondBrainFileTree=d.tree??[],e.secondBrainBrowsingFolder=null,e.secondBrainFolderEntries=null,e.secondBrainFolderName=null}else if(n==="context"){const[s,i,d]=await Promise.all([e.client.request("secondBrain.aiPacket",{}),e.client.request("secondBrain.sources",{}),e.client.request("secondBrain.vaultHealth",{})]);e.secondBrainAiPacket=s,e.secondBrainSourcesData=i,e.secondBrainVaultHealth=d}}catch(s){console.error("[SecondBrain] Failed to load:",s),e.secondBrainError=s instanceof Error?s.message:"Failed to load Second Brain data"}finally{e.secondBrainLoading=!1}}async function P(e,n){if(!(!e.client||!e.connected)){e.secondBrainLoading=!0;try{const s=await e.client.request("secondBrain.memoryBank",{folder:n});e.secondBrainBrowsingFolder=s.folder,e.secondBrainFolderName=s.folderName,e.secondBrainFolderEntries=s.entries}catch(s){console.error("[SecondBrain] Failed to browse folder:",s),e.secondBrainError=s instanceof Error?s.message:"Failed to browse folder"}finally{e.secondBrainLoading=!1}}}async function R(e){if(!(!e.client||!e.connected)){e.secondBrainSyncing=!0;try{const n=await e.client.request("secondBrain.sync",{});e.secondBrainAiPacket={snapshot:n.snapshot??null}}catch(n){console.error("[SecondBrain] Sync failed:",n),e.secondBrainError=n instanceof Error?n.message:"Sync failed"}finally{e.secondBrainSyncing=!1}}}var L=Object.defineProperty,N=Object.getOwnPropertyDescriptor,o=(e,n,s,i)=>{for(var d=i>1?void 0:i?N(n,s):n,h=e.length-1,l;h>=0;h--)(l=e[h])&&(d=(i?l(n,s,d):l(d))||d);return i&&d&&L(n,s,d),d};function v(e){if(!e)return"";try{return T(new Date(e).getTime())}catch{return""}}function $(e){return a`<div class="second-brain-md-body">${x(A(e))}</div>`}function H(e){const{identity:n}=e;return!n||n.files.length===0?a`
      ...
      <div class="second-brain-entry-list">
        ${n.files.map(s=>a`
          <div class="second-brain-entry" @click=${()=>e.onSelectEntry(s.key)}>
            ...
            ${s.updatedAt?a`<div class="second-brain-entry-meta">${v(s.updatedAt)}</div>`:r}
          </div>
        `)}
      </div>
    </div>
  `}function B(e,n){const s=e.isDirectory;return a`
    <div class="second-brain-entry" @click=${()=>{s?n.onBrowseFolder(e.path):n.onSelectEntry(e.path)}}>
      ...
      ${e.updatedAt?a`<div class="second-brain-entry-meta">${v(e.updatedAt)}</div>`:r}
    </div>
  `}function I(e,n){const s=e.isDirectory,i=s?"📁":"📑",d=()=>{s?n.onBrowseFolder(e.path):n.onSelectEntry(e.path)},h=e.frontmatter?.title||e.name;return a`
      ...
      <div class="second-brain-entry-body">
        ...
        ${e.frontmatter?.tags?.length?a`
          <div class="second-brain-research-tags">
            ${e.frontmatter.tags.map(l=>a`<span class="second-brain-research-tag">${l}</span>`)}
          </div>
        `:r}
      </div>
      ${e.updatedAt?a`<div class="second-brain-entry-meta">${v(e.updatedAt)}</div>`:r}
    </div>
  `}function q(e){return e<1024?`${e}B`:e<1024*1024?`${(e/1024).toFixed(1)}K`:`${(e/(1024*1024)).toFixed(1)}M`}function S(e,n,s=0){return a`
    <div class="sb-file-tree" style="padding-left: ${s*16}px">
      ${e.map(i=>i.type==="folder"?a`
          ...
          >
            ...
            ${i.size!=null?a`<span class="sb-tree-size">${q(i.size)}</span>`:r}
          </button>
        `)}
    </div>
  `}function Q(e){const{memoryBank:n,researchData:s,searchQuery:i,browsingFolder:d,folderEntries:h,folderName:l}=e;if(d&&h)return a`
      <div class="second-brain-panel">
        ...
        <div class="second-brain-section">
          <div class="second-brain-section-header">
            <span class="second-brain-section-title">${l??"Folder"}</span>
            <span class="second-brain-section-count">${h.length} items</span>
          </div>
          <div class="second-brain-entry-list">
            ${h.length>0?h.map(t=>B(t,e)):a`<div class="second-brain-empty-inline">Empty folder</div>`}
          </div>
        </div>
      </div>
    `;if(!(n&&n.totalEntries>0||s&&s.totalEntries>0||e.fileTree&&e.fileTree.length>0))return z("No knowledge found","Start building your second brain by telling your ally about the people, companies, and projects in your life.");const y=(i??"").toLowerCase().trim(),F=t=>y?t.filter(b=>b.name.toLowerCase().includes(y)||b.excerpt.toLowerCase().includes(y)):t,p=e.fileSearchResults;return a`
    <div class="second-brain-panel">
      ...
      ${!y&&n?a`
        ${n.sections.map(t=>{const b=F(t.entries);return t.entries.length===0?r:a`
            <details class="second-brain-section-details">
              ...
              <div class="second-brain-entry-list">
                ${b.length>0?b.map(w=>B(w,e)):y?a`<div class="second-brain-empty-inline">No matches</div>`:r}
              </div>
            </details>
          `})}
        ...
        ${n.curated?a`
          <details class="second-brain-section-details">
            <summary class="second-brain-section-header second-brain-section-header--toggle">
              ...
              <span class="second-brain-section-count">${v(n.curated.updatedAt)}</span>
            </summary>
            <div class="second-brain-card">
              <div class="second-brain-card-content">${$(n.curated.content)}</div>
            </div>
          </details>
        `:r}
      `:r}

      ${!y&&s&&s.totalEntries>0?a`
        ${s.categories.map(t=>t.entries.length===0?r:a`
            <details class="second-brain-section-details">
              ...
              <div class="second-brain-entry-list">
                ${t.entries.map(b=>I(b,e))}
              </div>
            </details>
          `)}
      `:r}

      ${!y&&e.fileTree&&e.fileTree.length>0?a`
        <details class="second-brain-section-details">
          <summary class="second-brain-section-header second-brain-section-header--toggle">
  ...
  `}const f={connected:{dot:"●",label:"Connected",cls:"second-brain-source--connected"},available:{dot:"○",label:"Available",cls:"second-brain-source--available"}};function g(e){const n=f[e.status]??f.available;return a`
    <div class="second-brain-source-card ${n.cls}">
      ...
      <div class="second-brain-source-status">
        ...
        ${e.status==="connected"&&e.lastSync?a`<span class="second-brain-source-sync">${v(e.lastSync)}</span>`:r}
      </div>
    </div>
  `}function V(e){const{aiPacket:n,sourcesData:s,vaultHealth:i,syncing:d}=e,h=n?.snapshot??null;return a`
    <div class="second-brain-panel">
      ...
      <div class="second-brain-section">
        <div class="second-brain-sync-bar">
          <div class="second-brain-sync-info">
            ...
            <span class="second-brain-sync-time">
              ${h?.updatedAt?`Last synced ${v(h.updatedAt)}`:"Not yet synced"}
              ${h?` • ${h.lineCount} lines`:""}
            </span>
          </div>
          <button
        ...
        ${h?a`
          <details class="second-brain-section-details">
            ...
            <div class="second-brain-card">
              <div class="second-brain-card-content">${$(h.content)}</div>
            </div>
          </details>
        `:a`
          <div class="second-brain-empty-block">
      ...
      ${s&&s.sources.length>0?a`
        <div class="second-brain-section">
          ...
          <div class="second-brain-sources-grid">
            ${s.sources.filter(l=>l.status==="connected").map(l=>g(l))}
          </div>
          ${s.sources.some(l=>l.status==="available")?a`
            <details style="margin-top: 8px;">
              <summary style="cursor: pointer; font-size: 12px; color: var(--muted);">
                ${s.sources.filter(l=>l.status==="available").length} available sources
              </summary>
              <div class="second-brain-sources-grid" style="margin-top: 8px;">
                ${s.sources.filter(l=>l.status==="available").map(l=>g(l))}
              </div>
            </details>
          `:r}
        </div>
        ...
        <div class="second-brain-section">
          ...
          ${i.available&&i.stats?a`
            <div class="context-health-row">
              ...
              <span>\u00B7</span>
              <span>Last: ${i.stats.lastActivity?v(i.stats.lastActivity):"never"}</span>
            </div>
          `:a`
