...
`,e=e.substring(m.length+1),c=!0),!c){let A=this.rules.other.nextBulletRegex(w),T=this.rules.other.hrRegex(w),C=this.rules.other.fencesBeginRegex(w),d=this.rules.other.headingBeginRegex(w),$=this.rules.other.htmlBeginRegex(w),S=this.rules.other.blockquoteBeginRegex(w);for(;e;){let _=e.split(`
`,1)[0],x;if(m=_,this.options.pedantic?(m=m.replace(this.rules.other.listReplaceNesting,"  "),x=m):x=m.replace(this.rules.other.tabCharGlobal,"    "),C.test(m)||d.test(m)||$.test(m)||S.test(m)||A.test(m)||T.test(m))break;if(x.search(this.rules.other.nonSpaceChar)>=w||!m.trim())p+=`
`+x.slice(w);else{if(g||f.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||C.test(f)||d.test(f)||T.test(f))break;p+=`
`+m}g=!m.trim(),u+=_+`
...
`)?"":`
`)+a.raw,o.text+=`
`+a.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=o.text):n.push(a);continue}if(t){let o="Infinite loop on byte: "+t.charCodeAt(0);if(this.options.silent){console.error(o);break}else throw new Error(o)}}return this.state.top=!0,n}inline(t,n=[]){return this.inlineQueue.push({src:t,tokens:n}),n}inlineTokens(t,n=[]){let s=t,a=null;if(this.tokens.links){let c=Object.keys(this.tokens.links);if(c.length>0)for(;(a=this.tokenizer.rules.inline.reflinkSearch.exec(s))!=null;)c.includes(a[0].slice(a[0].lastIndexOf("[")+1,-1))&&(s=s.slice(0,a.index)+"["+"a".repeat(a[0].length-2)+"]"+s.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(a=this.tokenizer.rules.inline.anyPunctuation.exec(s))!=null;)s=s.slice(0,a.index)+"++"+s.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let i;for(;(a=this.tokenizer.rules.inline.blockSkip.exec(s))!=null;)i=a[2]?a[2].length:0,s=s.slice(0,a.index+i)+"["+"a".repeat(a[0].length-i-2)+"]"+s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);s=this.options.hooks?.emStrongMask?.call({lexer:this},s)??s;let o=!1,l="";for(;t;){o||(l=""),o=!1;let c;if(this.options.extensions?.inline?.some(p=>(c=p.call({lexer:this},t,n))?(t=t.substring(c.raw.length),n.push(c),!0):!1))continue;if(c=this.tokenizer.escape(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.tag(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.link(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.reflink(t,this.tokens.links)){t=t.substring(c.raw.length);let p=n.at(-1);c.type==="text"&&p?.type==="text"?(p.raw+=c.raw,p.text+=c.text):n.push(c);continue}if(c=this.tokenizer.emStrong(t,s,l)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.codespan(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.br(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.del(t,s,l)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.autolink(t)){t=t.substring(c.raw.length),n.push(c);continue}if(!this.state.inLink&&(c=this.tokenizer.url(t))){t=t.substring(c.raw.length),n.push(c);continue}let u=t;if(this.options.extensions?.startInline){let p=1/0,f=t.slice(1),m;this.options.extensions.startInline.forEach(g=>{m=g.call({lexer:this},f),typeof m=="number"&&m>=0&&(p=Math.min(p,m))}),p<1/0&&p>=0&&(u=t.substring(0,p+1))}if(c=this.tokenizer.inlineText(u)){t=t.substring(c.raw.length),c.raw.slice(-1)!=="_"&&(l=c.raw.slice(-1)),o=!0;let p=n.at(-1);p?.type==="text"?(p.raw+=c.raw,p.text+=c.text):n.push(c);continue}if(t){let p="Infinite loop on byte: "+t.charCodeAt(0);if(this.options.silent){console.error(p);break}else throw new Error(p)}}return n}},Ws=class{options;parser;constructor(e){this.options=e||Bt}space(e){return""}code({text:e,lang:t,escaped:n}){let s=(t||"").match(me.notSpaceStart)?.[0],a=e.replace(me.endingNewline,"")+`
...
`),o=n==="assistant"?Ma(i):Es(i);return Ha(o)?null:o}}if(typeof t.text=="string"){const a=Wa(t.text);if(!a)return null;const i=n==="assistant"?Ma(a):Es(a);return Ha(i)?null:i}return null}function co(e){if(!e||typeof e!="object")return Lt(e);const t=e;if(qa.has(t))return qa.get(t)??null;const n=Lt(e);return qa.set(t,n),n}function Ai(e){const n=e.content,s=[];if(Array.isArray(n))for(const l of n){const c=l;if(c.type==="thinking"&&typeof c.thinking=="string"){const u=c.thinking.trim();u&&s.push(u)}}if(s.length>0)return s.join(`
...
`);return s.length>Zr?s.slice(0,Zr)+"…":n.length<t.length?s+"…":s}function im(e){const t=e,n=cm(t.content),s=[];for(const a of n){const i=(typeof a.type=="string"?a.type:"").toLowerCase();(["toolcall","tool_call","tooluse","tool_use"].includes(i)||typeof a.name=="string"&&a.arguments!=null)&&s.push({kind:"call",name:a.name??"tool",args:dm(a.arguments??a.args)})}for(const a of n){const i=(typeof a.type=="string"?a.type:"").toLowerCase();if(i!=="toolresult"&&i!=="tool_result")continue;const o=um(a);if(nl(o))continue;const l=typeof a.name=="string"?a.name:"tool";s.push({kind:"result",name:l,text:o})}if(md(e)&&!s.some(a=>a.kind==="result")){const a=typeof t.toolName=="string"&&t.toolName||typeof t.tool_name=="string"&&t.tool_name||"tool",i=co(e)??void 0;nl(i)||s.push({kind:"result",name:a,text:i})}return s}const om=new Set(["write","read","edit","attach"]);function rm(e){if(!e||typeof e!="object")return null;const t=e,n=t.path??t.file_path??t.filePath;return typeof n=="string"&&n.trim()?n.trim():null}function lm(e){if(!e)return null;const t=e.match(/(?:\/(?:Users|home|tmp|var)\/\S+|~\/\S+)/);return t?t[0].replace(/[.,;:!?)'"]+$/,""):null}function tl(e,t,n,s){const a=Xg({name:e.name,args:e.args}),i=Zg(a),o=!!e.text?.trim(),l=om.has(e.name.toLowerCase())?rm(e.args)??lm(e.text):null;if(l&&e.kind==="result"){const T=l.split("/").pop()||l,C=T.split(".").pop()?.toLowerCase()||"",d=vd(T),$=yd(C,T);return r`
    ...
    `}const c=!!t||!!(n&&l),u=c?T=>{if(T.stopPropagation(),n&&l){n(l);return}if(t&&o){t(el(e.text));return}if(t){const C=`## ${a.label}

${i?`**Command:** \`${i}\`

`:""}*No output — tool completed successfully.*`;t(C)}}:void 0,p=e.text?e.text.split(`
`).length:0,f=o&&(e.text?.length??0)<=tm,m=o&&!f&&p>sm,g=o&&!m,w=!o,A=e.kind==="call"?"chat-tool-card--call":"chat-tool-card--result";return r`
    <div
      class="chat-tool-card ${A} ${c?"chat-tool-card--clickable":""}"
      @click=${u}
      role=${c?"button":h}
      tabindex=${c?"0":h}
      @keydown=${c?T=>{T.key!=="Enter"&&T.key!==" "||(T.preventDefault(),T.stopPropagation(),u?.(T))}:h}
    >
...
`);)a=a.slice(1);return a.trim()}function mm(e){return typeof e!="number"||!Number.isFinite(e)||e<=0?null:e>=1024*1024?`${(e/(1024*1024)).toFixed(1)} MB`:e>=1024?`${Math.round(e/1024)} KB`:`${Math.round(e)} B`}function Ti(e){const t=e,n=t.content,s=[];if(Array.isArray(n))for(const i of n){if(typeof i!="object"||i===null)continue;const o=i;if(o.type==="image"){const l=o.source;if(l?.type==="base64"&&typeof l.data=="string"){const c=l.data,u=l.media_type||"image/png",p=c.startsWith("data:")?c:`data:${u};base64,${c}`;s.push({url:p})}else if(typeof o.data=="string"&&typeof o.mimeType=="string"){const c=o.data.startsWith("data:")?o.data:`data:${o.mimeType};base64,${o.data}`;s.push({url:c})}else typeof o.url=="string"?s.push({url:o.url}):o.omitted===!0&&s.push({omitted:!0,bytes:typeof o.bytes=="number"?o.bytes:void 0,mimeType:typeof o.mimeType=="string"?o.mimeType:void 0,alt:typeof o.fileName=="string"?o.fileName:void 0})}else if(o.type==="image_url"){const l=o.image_url;typeof l?.url=="string"&&s.push({url:l.url})}else o.type==="attachment"&&typeof o.content=="string"&&(o.mimeType||"").startsWith("image/")&&s.push({url:o.content,alt:o.fileName||void 0});if(o.type==="text"&&typeof o.text=="string"){const l=/data:image\/[^;]+;base64,[A-Za-z0-9+/=]+/g,c=o.text.match(l);if(c)for(const u of c)s.push({url:u})}if(Array.isArray(o.content))for(const l of o.content){if(typeof l!="object"||l===null)continue;const c=l;if(c.type==="image"){const u=c.source;if(u?.type==="base64"&&typeof u.data=="string"){const p=u.media_type||"image/png",f=u.data.startsWith("data:")?u.data:`data:${p};base64,${u.data}`;s.push({url:f})}else if(typeof c.data=="string"&&typeof c.mimeType=="string"){const p=c.data.startsWith("data:")?c.data:`data:${c.mimeType};base64,${c.data}`;s.push({url:p})}else c.omitted===!0&&s.push({omitted:!0,bytes:typeof c.bytes=="number"?c.bytes:void 0,mimeType:typeof c.mimeType=="string"?c.mimeType:void 0,alt:typeof c.fileName=="string"?c.fileName:void 0})}}}const a=t.attachments;if(Array.isArray(a))for(const i of a){if(typeof i!="object"||i===null)continue;const o=i;if(o.type==="image"&&typeof o.content=="string"){const l=o.mimeType||"image/png",c=o.content.startsWith("data:")?o.content:`data:${l};base64,${o.content}`;s.push({url:c,alt:o.fileName||void 0})}else o.type==="image"&&o.omitted===!0&&s.push({omitted:!0,bytes:typeof o.bytes=="number"?o.bytes:void 0,mimeType:typeof o.mimeType=="string"?o.mimeType:void 0,alt:typeof o.fileName=="string"?o.fileName:void 0})}return s}function vm(e){const t=e,n=t.content,s=[];if(Array.isArray(n))for(const i of n){if(typeof i!="object"||i===null)continue;const o=i;if(o.type==="attachment"&&typeof o.content=="string"){const l=o.mimeType||"application/octet-stream";l.startsWith("image/")||s.push({fileName:o.fileName||"file",mimeType:l,content:o.content})}}const a=t.attachments;if(Array.isArray(a))for(const i of a){if(typeof i!="object"||i===null)continue;const o=i;o.type==="file"&&typeof o.content=="string"&&s.push({fileName:o.fileName||"file",mimeType:o.mimeType||"application/octet-stream",content:o.content})}return s}function ym(e,t){return r`
  ...
  `}function wm(e,t){const n=uo(e.role),s=t.assistantName??"Assistant",a=t.userName??"You",i=n==="user"?a:n==="assistant"?s:n,o=n==="user"?"user":n==="assistant"?"assistant":"other",l=new Date(e.timestamp).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return r`
    <div class="chat-group ${o}">
      ...
      <div class="chat-group-messages">
        ${e.messages.map((c,u)=>kd(c.message,{isStreaming:e.isStreaming&&u===e.messages.length-1,showReasoning:t.showReasoning},t.onOpenSidebar,t.onOpenFile,t.onImageClick,t.resolveImageUrl,t.onPushToDrive))}
        <div class="chat-group-footer">
  ...
  `}function kd(e,t,n,s,a,i,o){try{return km(e,t,n,s,a,i,o)}catch(l){return console.error("[chat] message render error:",l),r`
    ...
    `}}function km(e,t,n,s,a,i,o){const l=e,c=typeof l.role=="string"?l.role:"unknown",u=md(e)||c.toLowerCase()==="toolresult"||c.toLowerCase()==="tool_result"||typeof l.toolCallId=="string"||typeof l.tool_call_id=="string",p=im(e),f=p.length>0,m=Ti(e),g=m.length>0,w=typeof l._chatIdx=="number"?l._chatIdx:-1,A=i&&w>=0?j=>i(w,j):void 0,T=vm(e),C=T.length>0,d=co(e),$=t.showReasoning&&c==="assistant"?pd(e):null,S=c==="user"&&d?pm(d):null,_=S&&S.length>0;let x=d;if(c==="user"&&x&&(x=gm(x)),x&&(x=x.replace(/<system-context\b[^>]*>[\s\S]*?<\/system-context>/gi,"").trim()||null),x){const j=ud(x);j&&(x=j)}_&&x&&(x=fm(x));const P=x?.trim()?x:null,K=$?fd($):null,q=P,X=c==="assistant"&&!!q?.trim(),H=["chat-bubble",X?"has-copy":"",t.isStreaming?"streaming":"","fade-in"].filter(Boolean).join(" ");if(f&&u)return r`
      ${g?il(m,a,A):h}
      ${p.map(j=>tl(j,n,s,o))}
...
No details available.`,title:t.task,mimeType:"text/markdown"}}const Yt=Object.freeze(Object.defineProperty({__proto__:null,AGENT_ROLE_NAMES:pu,approveCodingTask:my,approveQueueItem:gy,cancelCodingTask:fy,formatDuration:hu,loadAgentDetail:yy,loadMissionControl:fn,retryQueueItem:vy},Symbol.toStringTag,{value:"Module"}));function To(e){e.nodesPollInterval==null&&(e.nodesPollInterval=window.setInterval(()=>{ua(e,{quiet:!0})},5e3))}function _o(e){e.nodesPollInterval!=null&&(clearInterval(e.nodesPollInterval),e.nodesPollInterval=null)}function xo(e){e.logsPollInterval==null&&(e.logsPollInterval=window.setInterval(()=>{e.tab==="logs"&&Ao(e,{quiet:!0})},2e3))}function Co(e){e.logsPollInterval!=null&&(clearInterval(e.logsPollInterval),e.logsPollInterval=null)}function Eo(e){e.debugPollInterval==null&&(e.debugPollInterval=window.setInterval(()=>{e.tab==="debug"&&pa(e)},3e3))}function Lo(e){e.debugPollInterval!=null&&(clearInterval(e.debugPollInterval),e.debugPollInterval=null)}function fu(e){e.missionControlPollInterval==null&&(e.missionControlPollInterval=window.setInterval(()=>{e.tab==="mission-control"&&fn(e,{quiet:!0})},5e3))}function gu(e){e.missionControlPollInterval!=null&&(clearInterval(e.missionControlPollInterval),e.missionControlPollInterval=null)}async function as(e){if(!(!e.client||!e.connected))try{const t=await e.client.request("cron.status",{});e.cronStatus=t}catch(t){e.cronError=String(t)}}async function ha(e){if(!(!e.client||!e.connected)&&!e.cronLoading){e.cronLoading=!0,e.cronError=null;try{const t=await e.client.request("cron.list",{includeDisabled:!0});e.cronJobs=Array.isArray(t.jobs)?t.jobs:[]}catch(t){e.cronError=String(t)}finally{e.cronLoading=!1}}}function by(e){if(e.scheduleKind==="at"){const n=Date.parse(e.scheduleAt);if(!Number.isFinite(n))throw new Error("Invalid run time.");return{kind:"at",atMs:n}}if(e.scheduleKind==="every"){const n=Us(e.everyAmount,0);if(n<=0)throw new Error("Invalid interval amount.");const s=e.everyUnit;return{kind:"every",everyMs:n*(s==="minutes"?6e4:s==="hours"?36e5:864e5)}}const t=e.cronExpr.trim();if(!t)throw new Error("Cron expression required.");return{kind:"cron",expr:t,tz:e.cronTz.trim()||void 0}}function wy(e){if(e.payloadKind==="systemEvent"){const a=e.payloadText.trim();if(!a)throw new Error("System event text required.");return{kind:"systemEvent",text:a}}const t=e.payloadText.trim();if(!t)throw new Error("Agent message required.");const n={kind:"agentTurn",message:t};e.deliver&&(n.deliver=!0),e.channel&&(n.channel=e.channel),e.to.trim()&&(n.to=e.to.trim());const s=Us(e.timeoutSeconds,0);return s>0&&(n.timeoutSeconds=s),n}async function $y(e){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{const t=by(e.cronForm),n=wy(e.cronForm),s=e.cronForm.agentId.trim(),a={name:e.cronForm.name.trim(),description:e.cronForm.description.trim()||void 0,agentId:s||void 0,enabled:e.cronForm.enabled,schedule:t,sessionTarget:e.cronForm.sessionTarget,wakeMode:e.cronForm.wakeMode,payload:n,isolation:e.cronForm.postToMainPrefix.trim()&&e.cronForm.sessionTarget==="isolated"?{postToMainPrefix:e.cronForm.postToMainPrefix.trim()}:void 0};if(!a.name)throw new Error("Name required.");await e.client.request("cron.add",a),e.cronForm={...e.cronForm,name:"",description:"",payloadText:""},await ha(e),await as(e)}catch(t){e.cronError=String(t)}finally{e.cronBusy=!1}}}async function ky(e,t,n){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.update",{id:t.id,patch:{enabled:n}}),await ha(e),await as(e)}catch(s){e.cronError=String(s)}finally{e.cronBusy=!1}}}async function Sy(e,t){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.run",{id:t.id,mode:"force"}),await mu(e,t.id)}catch(n){e.cronError=String(n)}finally{e.cronBusy=!1}}}async function Ay(e,t){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.remove",{id:t.id}),e.cronRunsJobId===t.id&&(e.cronRunsJobId=null,e.cronRuns=[]),await ha(e),await as(e)}catch(n){e.cronError=String(n)}finally{e.cronBusy=!1}}}async function mu(e,t){if(!(!e.client||!e.connected))try{const n=await e.client.request("cron.runs",{id:t,limit:50});e.cronRunsJobId=t,e.cronRuns=Array.isArray(n.entries)?n.entries:[]}catch(n){e.cronError=String(n)}}async function Ut(e){if(!(!e.client||!e.connected)){e.guardrailsLoading=!0;try{const[t,n]=await Promise.all([e.client.request("guardrails.list",{}),e.client.request("guardrails.history",{limit:50})]);e.guardrailsData={gates:t.gates,activity:n.activity,custom:t.custom??[]}}catch(t){console.error("[Guardrails] load error:",t),e.guardrailsData=null}finally{e.guardrailsLoading=!1}}}async function Ty(e,t,n){if(!(!e.client||!e.connected))try{await e.client.request("guardrails.set",{gateId:t,enabled:n});const a=e.guardrailsData?.gates.find(i=>i.id===t)?.name??t;e.showToast(`${a} ${n?"enabled":"disabled"}`,"success",2e3),await Ut(e)}catch(s){e.showToast("Failed to update guardrail","error"),console.error("[Guardrails] toggle error:",s)}}async function _y(e,t,n,s){if(!(!e.client||!e.connected))try{await e.client.request("guardrails.set",{gateId:t,thresholds:{[n]:s}}),e.showToast("Threshold updated","success",2e3),await Ut(e)}catch(a){e.showToast("Failed to update threshold","error"),console.error("[Guardrails] threshold error:",a)}}async function xy(e,t,n){if(!(!e.client||!e.connected))try{await e.client.request("guardrails.setCustom",{id:t,enabled:n}),e.showToast(`Custom rule ${n?"enabled":"disabled"}`,"success",2e3),await Ut(e)}catch(s){e.showToast("Failed to update custom rule","error"),console.error("[Guardrails] toggleCustom error:",s)}}async function Cy(e,t){if(!(!e.client||!e.connected))try{await e.client.request("guardrails.removeCustom",{id:t}),e.showToast("Custom rule removed","success",2e3),await Ut(e)}catch(n){e.showToast("Failed to remove custom rule","error"),console.error("[Guardrails] deleteCustom error:",n)}}async function Ey(e,t){if(!(!e.client||!e.connected))try{await e.client.request("guardrails.addCustom",{guardrail:{name:t.name,description:"",enabled:!0,trigger:{tool:t.tool,patterns:t.patterns},action:t.action,message:t.message,...t.redirectTo?{redirectTo:t.redirectTo}:{}}}),e.showToast("Custom rule created","success",2e3),await Ut(e)}catch(n){e.showToast("Failed to create custom rule","error"),console.error("[Guardrails] addCustom error:",n)}}const Jt=Object.freeze(Object.defineProperty({__proto__:null,addCustomGuardrailFromUI:Ey,deleteCustomGuardrail:Cy,loadGuardrails:Ut,toggleCustomGuardrail:xy,toggleGuardrail:Ty,updateGuardrailThreshold:_y},Symbol.toStringTag,{value:"Module"}));function Ly(e){if(!e||e.kind==="gateway")return{method:"exec.approvals.get",params:{}};const t=e.nodeId.trim();return t?{method:"exec.approvals.node.get",params:{nodeId:t}}:null}function Ry(e,t){if(!e||e.kind==="gateway")return{method:"exec.approvals.set",params:t};const n=e.nodeId.trim();return n?{method:"exec.approvals.node.set",params:{...t,nodeId:n}}:null}async function Ro(e,t){if(!(!e.client||!e.connected)&&!e.execApprovalsLoading){e.execApprovalsLoading=!0,e.lastError=null;try{const n=Ly(t);if(!n){e.lastError="Select a node before loading exec approvals.";return}const s=await e.client.request(n.method,n.params);Py(e,s)}catch(n){e.lastError=String(n)}finally{e.execApprovalsLoading=!1}}}function Py(e,t){e.execApprovalsSnapshot=t,e.execApprovalsDirty||(e.execApprovalsForm=Pt(t.file??{}))}async function Iy(e,t){if(!(!e.client||!e.connected)){e.execApprovalsSaving=!0,e.lastError=null;try{const n=e.execApprovalsSnapshot?.hash;if(!n){e.lastError="Exec approvals hash missing; reload and retry.";return}const s=e.execApprovalsForm??e.execApprovalsSnapshot?.file??{},a=Ry(t,{file:s,baseHash:n});if(!a){e.lastError="Select a node before saving exec approvals.";return}await e.client.request(a.method,a.params),e.execApprovalsDirty=!1,await Ro(e,t)}catch(n){e.lastError=String(n)}finally{e.execApprovalsSaving=!1}}}function Dy(e,t,n){const s=Pt(e.execApprovalsForm??e.execApprovalsSnapshot?.file??{});Nc(s,t,n),e.execApprovalsForm=s,e.execApprovalsDirty=!0}function My(e,t){const n=Pt(e.execApprovalsForm??e.execApprovalsSnapshot?.file??{});Fc(n,t),e.execApprovalsForm=n,e.execApprovalsDirty=!0}async function vu(e){if(!(!e.client||!e.connected))try{const t=await e.client.request("lifetracks.config.get",{});t&&(e.lifetracksConfig=t)}catch(t){console.error("Failed to load lifetracks config:",t)}}async function Qs(e){if(!e.client||!e.connected){e.lifetracksError="Not connected to gateway";return}e.lifetracksLoading=!0,e.lifetracksError=null;try{await vu(e);const t=await e.client.request("lifetracks.list",{});if(t?.lifetracks)if(e.lifetracksData=t.lifetracks,t.lifetracks.length>0){const n=se(),s=t.lifetracks.find(a=>a.date===n);e.lifetracksCurrentTrack=s||t.lifetracks[0]}else e.lifetracksCurrentTrack=null;else e.lifetracksData=[],e.lifetracksCurrentTrack=null}catch(t){e.lifetracksError=t instanceof Error?t.message:"Failed to load lifetracks"}finally{e.lifetracksLoading=!1}}function Oy(e,t){e.lifetracksCurrentTrack=t}async function Ny(e){if(!e.client||!e.connected)return e.lifetracksError="Not connected to gateway",!1;try{const t=await e.client.request("lifetracks.config.update",{enabled:!0});return t?.updated?(e.lifetracksConfig=t.config,await Qs(e),!0):!1}catch(t){return e.lifetracksError=t instanceof Error?t.message:"Failed to enable lifetracks",!1}}async function Fy(e,t){if(!e.client||!e.connected)return e.lifetracksGenerationError="Not connected to gateway",!1;e.lifetracksGenerating=!0,e.lifetracksGenerationError=null;try{const n=await e.client.request("lifetracks.generate",t||{});return n?.generated&&n.track?(e.lifetracksData=[n.track,...e.lifetracksData||[]],e.lifetracksCurrentTrack=n.track,await vu(e),!0):n?.alreadyExists&&n.track?(e.lifetracksCurrentTrack=n.track,!0):(e.lifetracksGenerationError=n?.error||"Generation failed",!1)}catch(n){return e.lifetracksGenerationError=n instanceof Error?n.message:"Failed to generate lifetrack",!1}finally{e.lifetracksGenerating=!1}}const By=["~/godmode/memory/AGENT-DAY.md","~/godmode/AGENT-DAY.md"];function Uy(e){return[...[`~/godmode/memory/agent-day/${e}.md`,`~/godmode/memory/agent-log/${e}.md`,`~/godmode/memory/daily/${e}-agent-log.md`],...By]}function zy(e){return["Needs Review","Active Sub-Agents","Completed Today","Queue","Feedback Log","AGENT-DAY"].filter(s=>e.includes(s)).length>=2}async function yu(e,t){try{const n=t?{date:t}:{},s=await e.request("dailyBrief.get",n);return!s||!s.content||!s.date?null:s}catch(n){return console.error("[MyDay] Failed to load daily brief:",n),null}}async function bu(e,t,n){const s=t||se(),a="agentLog.get";try{const i=await e.request(a,{date:s});if(i?.content?.trim()&&i?.sourcePath)return{date:i.date||s,content:i.content,updatedAt:i.updatedAt||new Date().toISOString(),sourcePath:i.sourcePath}}catch(i){console.warn(`[MyDay] ${a} unavailable, falling back to files.read:`,i)}return Ky(e,s)}async function Ky(e,t){const n=Uy(t),s=a=>a.includes("AGENT-DAY.md");for(const a of n)try{const i=await e.request("files.read",{path:a,maxSize:1e6});if(!i?.content?.trim()||!zy(i.content)||s(a)&&typeof i.modifiedAt=="number"&&se(new Date(i.modifiedAt))!==t)continue;return{date:t,content:i.content,updatedAt:typeof i.modifiedAt=="number"?new Date(i.modifiedAt).toISOString():new Date().toISOString(),sourcePath:a}}catch{}return null}function As(e,t,n){return new Promise((s,a)=>{const i=setTimeout(()=>a(new Error(`${n} timed out after ${t}ms`)),t);e.then(o=>{clearTimeout(i),s(o)},o=>{clearTimeout(i),a(o)})})}const Wy={coding:"Builder",research:"Researcher",analysis:"Analyst",creative:"Creative",review:"Reviewer",ops:"Ops",task:"Agent",url:"Reader",idea:"Explorer"};async function wu(e){if(!e.client||!e.connected)return[];e.todayTasksLoading=!0;try{const t=e.todaySelectedDate??se(),[n,s]=await Promise.all([e.client.request("tasks.today",{date:t,includeCompleted:!0}),e.client.request("queue.list",{limit:100}).catch(()=>({items:[]}))]),a=new Map;for(const o of s.items)o.sourceTaskId&&(o.status==="processing"||o.status==="review"||o.status==="failed")&&a.set(o.sourceTaskId,{status:o.status,type:o.type,roleName:Wy[o.type]??o.type,queueItemId:o.id});const i=(n.tasks??[]).map(o=>({id:o.id,title:o.title,status:o.status,project:o.project,dueDate:o.dueDate,priority:o.priority,createdAt:o.createdAt,completedAt:o.completedAt,queueStatus:a.get(o.id)??null}));return e.todayTasks=i,i}catch(t){return console.error("[MyDay] Failed to load today tasks:",t),e.todayTasks??[]}finally{e.todayTasksLoading=!1}}async function qy(e){if(!e.client||!e.connected)return[];try{const n=(await e.client.request("queue.list",{limit:50}))?.items??[],s=Date.now()-1440*60*1e3;return n.filter(a=>!(a.status!=="review"&&a.status!=="done"||a.status==="done"&&(a.completedAt??0)<s)).map(a=>({id:a.id,title:a.title,summary:a.result?.summary??a.description??"",status:a.status,completedAt:a.completedAt,outputPath:a.result?.outputPath,prUrl:a.result?.prUrl,sourceTaskId:a.sourceTaskId}))}catch(t){return console.error("[MyDay] Failed to load queue results for decision cards:",t),[]}}function Vy(e,t){e.request("dailyBrief.syncTasks",t?{date:t}:{}).catch(n=>{console.warn("[MyDay] dailyBrief.syncTasks failed (non-blocking):",n)})}async function Is(e){if(!(!e.client||!e.connected)){e.dailyBriefLoading=!0,e.dailyBriefError=null;try{e.dailyBrief=await yu(e.client,e.todaySelectedDate),e.loadBriefNotes&&await e.loadBriefNotes()}catch(t){e.dailyBriefError=t instanceof Error?t.message:"Failed to load daily brief",console.error("[MyDay] Brief load error:",t)}finally{e.dailyBriefLoading=!1}}}async function Tt(e,t){if(!(!e.client||!e.connected)){e.agentLogLoading=!0,e.agentLogError=null;try{e.agentLog=await bu(e.client,e.todaySelectedDate,t)}catch(n){e.agentLogError=n instanceof Error?n.message:"Failed to load agent log",console.error("[MyDay] Agent log load error:",n)}finally{e.agentLogLoading=!1}}}function $u(e){const t=e||se(),n="VAULT",s=`01-Daily/${t}`,a=`obsidian://open?vault=${encodeURIComponent(n)}&file=${encodeURIComponent(s)}`;window.open(a,"_blank")}async function Ys(e){if(!e.client||!e.connected)return;e.myDayLoading=!0,e.myDayError=null,e.dailyBriefLoading=!0,e.agentLogLoading=!0,e.todayTasksLoading=!0;const t=e.todaySelectedDate,n=e.loadBriefNotes?As(e.loadBriefNotes(),5e3,"Brief Notes"):Promise.resolve(),s=await Promise.allSettled([As(yu(e.client,t),1e4,"Daily Brief"),n,As(bu(e.client,t),1e4,"Agent Log"),As(wu(e),8e3,"Today Tasks")]);e.dailyBrief=s[0].status==="fulfilled"?s[0].value:null,e.agentLog=s[2].status==="fulfilled"?s[2].value:null;const a=["Brief","Brief Notes","Agent Log","Today Tasks"],i=s.map((o,l)=>o.status==="rejected"?{section:a[l],reason:o.reason}:null).filter(Boolean);if(i.length>0){for(const o of i)console.warn(`[MyDay] ${o.section} failed:`,o.reason);i.length===s.length&&(e.myDayError="Failed to load daily brief")}e.myDayLoading=!1,e.dailyBriefLoading=!1,e.agentLogLoading=!1,e.todayTasksLoading=!1}function Hy(e,t){const n=s=>{t(s)};return e.on("daily-brief:update",n),()=>{e.off("daily-brief:update",n)}}function ku(e,t){const n=()=>{t()};return e.on("agent-log:update",n),()=>{e.off("agent-log:update",n)}}const On=Object.freeze(Object.defineProperty({__proto__:null,loadAgentLogOnly:Tt,loadBriefOnly:Is,loadMyDay:Ys,loadTodayQueueResults:qy,loadTodayTasksWithQueueStatus:wu,openBriefInObsidian:$u,subscribeToAgentLogUpdates:ku,subscribeToBriefUpdates:Hy,syncTodayTasks:Vy},Symbol.toStringTag,{value:"Module"}));async function Su(e){if(!(!e.client||!e.connected)){e.peopleLoading=!0,e.peopleError=null;try{const t=await e.client.request("people.list",{});e.peopleList=t.people??[]}catch(t){console.error("[People] Failed to load contacts:",t),e.peopleError=t instanceof Error?t.message:"Failed to load contacts"}finally{e.peopleLoading=!1}}}async function Po(e){if(!(!e.client||!e.connected)&&!e.presenceLoading){e.presenceLoading=!0,e.presenceError=null,e.presenceStatus=null;try{const t=await e.client.request("system-presence",{});Array.isArray(t)?(e.presenceEntries=t,e.presenceStatus=t.length===0?"No instances yet.":null):(e.presenceEntries=[],e.presenceStatus="No presence payload.")}catch(t){e.presenceError=String(t)}finally{e.presenceLoading=!1}}}function un(e,t,n){if(!t.trim())return;const s={...e.skillMessages};n?s[t]=n:delete s[t],e.skillMessages=s}function fa(e){return e instanceof Error?e.message:String(e)}async function is(e,t){if(t?.clearMessages&&Object.keys(e.skillMessages).length>0&&(e.skillMessages={}),!(!e.client||!e.connected)&&!e.skillsLoading){e.skillsLoading=!0,e.skillsError=null;try{const n=await e.client.request("skills.status",{});n&&(e.skillsReport=n)}catch(n){e.skillsError=fa(n)}finally{e.skillsLoading=!1}}}function jy(e,t,n){e.skillEdits={...e.skillEdits,[t]:n}}async function Gy(e,t,n){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{await e.client.request("skills.update",{skillKey:t,enabled:n}),await is(e),un(e,t,{kind:"success",message:n?"Skill enabled":"Skill disabled"})}catch(s){const a=fa(s);e.skillsError=a,un(e,t,{kind:"error",message:a})}finally{e.skillsBusyKey=null}}}async function Qy(e,t){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{const n=e.skillEdits[t]??"";await e.client.request("skills.update",{skillKey:t,apiKey:n}),await is(e),un(e,t,{kind:"success",message:"API key saved"})}catch(n){const s=fa(n);e.skillsError=s,un(e,t,{kind:"error",message:s})}finally{e.skillsBusyKey=null}}}async function Yy(e,t,n,s){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{const a=await e.client.request("skills.install",{name:n,installId:s,timeoutMs:12e4});await is(e),un(e,t,{kind:"success",message:a?.message??"Installed"})}catch(a){const i=fa(a);e.skillsError=i,un(e,t,{kind:"error",message:i})}finally{e.skillsBusyKey=null}}}async function Mi(e){if(!e.client||!e.connected){e.visionBoardError="Not connected to gateway";return}e.visionBoardLoading=!0,e.visionBoardError=null;try{const[t,n]=await Promise.all([e.client.request("visionBoard.get",{}),e.client.request("visionBoard.identityToday",{})]);t?e.visionBoardData=t:e.visionBoardError="No data returned",n&&(e.visionBoardIdentityToday=n.identity)}catch(t){e.visionBoardError=t instanceof Error?t.message:"Failed to load vision board"}finally{e.visionBoardLoading=!1}}async function Js(e){if(!e.client||!e.connected){e.wheelOfLifeError="Not connected to gateway";return}e.wheelOfLifeLoading=!0,e.wheelOfLifeError=null;try{const t=await e.client.request("wheelOfLife.get",{});t?e.wheelOfLifeData=t:e.wheelOfLifeError="No data returned"}catch(t){e.wheelOfLifeError=t instanceof Error?t.message:"Failed to load wheel data"}finally{e.wheelOfLifeLoading=!1}}async function Jy(e,t){if(!e.client||!e.connected)return e.wheelOfLifeError="Not connected to gateway",!1;try{return(await e.client.request("wheelOfLife.update",{updates:t}))?.updated?(await Js(e),!0):!1}catch(n){return e.wheelOfLifeError=n instanceof Error?n.message:"Failed to update wheel",!1}}function Xy(e){e.wheelOfLifeEditMode=!0}function $l(e){e.wheelOfLifeEditMode=!1}async function Au(e){if(!(!e.client||!e.connected)){e.workLoading=!0,e.workError=null;try{const t=await e.client.request("projects.list",{});e.workProjects=t.projects??[]}catch(t){console.error("[Work] Failed to load:",t),e.workError=t instanceof Error?t.message:"Failed to load"}finally{e.workLoading=!1}}}async function Zy(e,t){if(!e.client||!e.connected)return;const n=new Set(e.workDetailLoading??[]);n.add(t),e.workDetailLoading=n;try{const s=await e.client.request("projects.get",{id:t,includeFiles:!0,depth:2});if(s){const a=s.files??[];e.workProjectFiles={...e.workProjectFiles,[t]:a}}}catch(s){console.warn("[Work] Failed to load project details:",s)}finally{const s=new Set(e.workDetailLoading??[]);s.delete(t),e.workDetailLoading=s}}function ga(e,t=Date.now()){if(typeof e=="number"&&Number.isFinite(e))return new Date(e);if(typeof e=="string"){const n=Date.parse(e);if(Number.isFinite(n))return new Date(n)}return new Date(t)}function Io(e){return{id:e.id,name:e.name,emoji:e.emoji||"📁",type:e.type,path:e.path,artifactCount:e.artifactCount??0,sessionCount:e.sessionCount??0,lastUpdated:ga(e.lastUpdated,e.lastScanned)}}function ei(e){return{path:e.path,name:e.name,type:e.type,size:e.size,modified:ga(e.modified),isDirectory:e.isDirectory,searchText:e.searchText}}function kl(e){return{id:e.id,key:e.key,title:e.title,created:ga(e.created),status:e.status,workspaceSubfolder:e.workspaceSubfolder}}function zt(e){return{id:e.id,title:e.title,status:e.status,project:e.project,dueDate:e.dueDate,priority:e.priority,createdAt:e.createdAt,completedAt:e.completedAt}}function Tu(e){return e.map(t=>({name:t.name,path:t.path,type:t.type,fileType:t.fileType,size:t.size,modified:t.modified?ga(t.modified):void 0,children:t.children?Tu(t.children):void 0}))}function eb(e,t){return{...t,id:e.id,name:e.name,emoji:e.emoji,type:e.type,path:e.path,artifactCount:e.artifactCount,sessionCount:e.sessionCount,lastUpdated:e.lastUpdated}}async function ma(e){if(!e.client||!e.connected){e.workspaces=[],e.workspacesLoading=!1,e.workspacesError="Connect to gateway to see workspaces";return}e.workspacesLoading=!0,e.workspacesError=null;try{const t=await e.client.request("workspaces.list",{});if(e.workspaces=(t.workspaces??[]).map(Io),e.selectedWorkspace){const n=e.workspaces.find(s=>s.id===e.selectedWorkspace?.id);n&&(e.selectedWorkspace=eb(n,e.selectedWorkspace))}}catch(t){console.error("[Workspaces] load failed:",t),e.workspacesError=t instanceof Error?t.message:"Failed to load workspaces",e.workspaces=[]}finally{e.workspacesLoading=!1}}async function va(e,t){if(!e.client||!e.connected)return null;try{const n=await e.client.request("workspaces.get",{id:t});return n.workspace?{...Io(n.workspace),pinned:(n.pinned??[]).map(ei),pinnedSessions:(n.pinnedSessions??[]).map(kl),outputs:(n.outputs??[]).map(ei),folderTree:n.folderTree?Tu(n.folderTree):void 0,sessions:(n.sessions??[]).map(kl),tasks:(n.tasks??[]).map(zt),memory:(n.memory??[]).map(ei)}:null}catch(n){return console.error("[Workspaces] get failed:",n),null}}async function tb(e,t,n){if(!e.client||!e.connected)return null;try{const s=await e.client.request("workspaces.readFile",n?{workspaceId:n,filePath:t}:{path:t});return s.content?{content:s.content,mime:s.contentType??s.mime??"text/plain"}:(s.error&&console.warn("[Workspaces] readFile failed:",s.error),null)}catch(s){return console.error("[Workspaces] readFile error:",s),null}}async function nb(e){if(!(!e.client||!e.connected))try{e.workspacesLoading=!0,e.workspacesError=null,await e.client.request("workspaces.scan",{}),await ma(e)}catch(t){e.workspacesError=t instanceof Error?t.message:"Failed to scan workspaces"}finally{e.workspacesLoading=!1}}async function sb(e,t){if(!t){e.selectedWorkspace=null;return}const n=await va(e,t.id);e.selectedWorkspace=n||{...t,pinned:[],pinnedSessions:[],outputs:[],sessions:[],tasks:[]}}async function ab(e,t,n,s){if(!e.client||!e.connected)return!1;try{if(await e.client.request(s?"workspaces.unpin":"workspaces.pin",{workspaceId:t,filePath:n}),e.selectedWorkspace?.id===t){const a=await va(e,t);a&&(e.selectedWorkspace=a)}return!0}catch(a){return console.error("[Workspaces] pin toggle failed:",a),!1}}async function ib(e,t,n,s){if(!e.client||!e.connected)return!1;try{if(await e.client.request(s?"workspaces.unpinSession":"workspaces.pinSession",{workspaceId:t,sessionKey:n}),e.selectedWorkspace?.id===t){const a=await va(e,t);a&&(e.selectedWorkspace=a)}return!0}catch(a){return console.error("[Workspaces] session pin toggle failed:",a),!1}}async function ob(e,t){if(!e.client||!e.connected)return null;const n=String(t.name??"").trim();if(!n)return e.workspacesError="Workspace name is required",null;const s=t.type??"project",a=String(t.path??"").trim();try{const i=await e.client.request("workspaces.create",{name:n,type:s,...a?{path:a}:{}});if(!i.workspace)return e.workspacesError="Workspace creation returned no workspace",null;const o=Io(i.workspace),l=e.workspaces??[],c=new Map(l.map(u=>[u.id,u]));return c.set(o.id,o),e.workspaces=Array.from(c.values()).toSorted((u,p)=>p.lastUpdated.getTime()-u.lastUpdated.getTime()),e.workspacesError=null,o}catch(i){return console.error("[Workspaces] create failed:",i),e.workspacesError=i instanceof Error?i.message:"Failed to create workspace",null}}async function rb(e,t){if(!e.client||!e.connected)return!1;try{return await e.client.request("workspaces.delete",{id:t}),e.workspaces=(e.workspaces??[]).filter(n=>n.id!==t),e.selectedWorkspace?.id===t&&(e.selectedWorkspace=null),e.workspacesError=null,!0}catch(n){return console.error("[Workspaces] delete failed:",n),e.workspacesError=n instanceof Error?n.message:"Failed to delete workspace",!1}}function lb(e,t){e.workspacesSearchQuery=t}function cb(e){e.selectedWorkspace=null}async function db(e){if(!(!e.client||!e.connected))try{const t=await e.client.request("workspaces.teamSetupPrompt",{});t?.prompt&&(e.chatMessage=t.prompt,e.setTab?.("chat"))}catch{e.chatMessage="I want to set up a Team Workspace so my team can collaborate. Please walk me through it step by step, keeping it simple.",e.setTab?.("chat")}}function ub(e,t){const n=new Set(e);return n.has(t)?n.delete(t):n.add(t),n}async function pb(e,t){if(!e.client||!e.connected)return[];try{return((await e.client.request("tasks.byProject",{project:t})).tasks??[]).map(zt)}catch(n){return console.error("[Workspaces] loadWorkspaceTasks failed:",n),[]}}async function hb(e){if(!e.client||!e.connected)return[];try{return((await e.client.request("tasks.list",{})).tasks??[]).map(zt)}catch(t){return console.error("[Workspaces] loadAllTasks failed:",t),[]}}const fb={coding:"Builder",research:"Researcher",analysis:"Analyst",creative:"Creative",review:"Reviewer",ops:"Ops",task:"Agent",url:"Reader",idea:"Explorer"};async function gb(e){if(!e.client||!e.connected)return[];try{const[t,n]=await Promise.all([e.client.request("tasks.list",{}),e.client.request("queue.list",{limit:100}).catch(()=>({items:[]}))]),s=new Map;for(const a of n.items)a.sourceTaskId&&(a.status==="processing"||a.status==="review"||a.status==="failed")&&s.set(a.sourceTaskId,{status:a.status,type:a.type,roleName:fb[a.type]??a.type,queueItemId:a.id});return(t.tasks??[]).map(a=>({...zt(a),queueStatus:s.get(a.id)??null}))}catch(t){return console.error("[Workspaces] loadAllTasksWithQueueStatus failed:",t),[]}}async function mb(e,t,n){if(!e.client||!e.connected)return null;const s=n==="complete"?"pending":"complete";try{const a=await e.client.request("tasks.update",{id:t,status:s});return zt(a)}catch(a){return console.error("[Workspaces] toggleTaskComplete failed:",a),null}}async function vb(e,t,n){if(!e.client||!e.connected)return null;try{const s=await e.client.request("tasks.update",{id:t,...n});return zt(s)}catch(s){return console.error("[Workspaces] updateTask failed:",s),null}}async function yb(e,t){if(!e.client||!e.connected)return null;try{return await e.client.request("tasks.openSession",{taskId:t})}catch(n){return console.error("[Workspaces] startTask failed:",n),null}}async function bb(e,t,n){if(!e.client||!e.connected)return null;try{const s=await e.client.request("tasks.create",{title:t,project:n,source:"chat"});return zt(s)}catch(s){return console.error("[Workspaces] createTask failed:",s),null}}async function wb(e,t,n){if(!e.client||!e.connected)return null;try{return await e.client.request("workspaces.browseFolder",{workspaceId:t,folderPath:n})}catch(s){return console.error("[Workspaces] browseFolder failed:",s),null}}async function $b(e,t,n,s=50){if(!e.client||!e.connected)return[];try{return(await e.client.request("workspaces.search",{workspaceId:t,query:n,limit:s})).results??[]}catch(a){return console.error("[Workspaces] search failed:",a),[]}}async function kb(e,t,n){if(!e.client||!e.connected)return!1;try{return await e.client.request("workspaces.createFolder",{workspaceId:t,folderPath:n}),!0}catch(s){return console.error("[Workspaces] createFolder failed:",s),!1}}async function Sb(e,t,n,s){if(!e.client||!e.connected)return!1;try{return await e.client.request("workspaces.moveFile",{workspaceId:t,sourcePath:n,destPath:s}),!0}catch(a){return console.error("[Workspaces] moveFile failed:",a),!1}}async function Ab(e,t,n,s){if(!e.client||!e.connected)return!1;try{return await e.client.request("workspaces.renameFile",{workspaceId:t,filePath:n,newName:s}),!0}catch(a){return console.error("[Workspaces] renameFile failed:",a),!1}}const ge=Object.freeze(Object.defineProperty({__proto__:null,browseWorkspaceFolder:wb,clearWorkspaceSelection:cb,createTask:bb,createWorkspace:ob,createWorkspaceFolder:kb,deleteWorkspace:rb,getWorkspace:va,loadAllTasks:hb,loadAllTasksWithQueueStatus:gb,loadWorkspaceTasks:pb,loadWorkspaces:ma,moveWorkspaceFile:Sb,readWorkspaceFile:tb,renameWorkspaceFile:Ab,scanWorkspaces:nb,searchWorkspaceFiles:$b,selectWorkspace:sb,setWorkspacesSearchQuery:lb,startTask:yb,startTeamSetup:db,toggleTaskComplete:mb,toggleWorkspaceFolder:ub,toggleWorkspacePin:ab,toggleWorkspaceSessionPin:ib,updateTask:vb},Symbol.toStringTag,{value:"Module"})),_u="godmode.ui.settings.v1";function Tb(){const e=new URLSearchParams(location.search),t=(()=>{const a=e.get("gatewayUrl");return a||(typeof window.__GODMODE_DEV__<"u"?"/ws":`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}`)})(),n=e.get("token")||"",s={gatewayUrl:t,token:n,sessionKey:"main",lastActiveSessionKey:"main",theme:"system",chatFocusMode:!1,chatShowThinking:!0,splitRatio:.6,navCollapsed:!1,navGroupsCollapsed:{System:!0},openTabs:["agent:main:main"],tabLastViewed:{},userName:"",userAvatar:"",chatParallelView:!1,parallelLanes:[null,null,null,null]};try{const a=localStorage.getItem(_u);if(!a)return s;const i=JSON.parse(a);return{gatewayUrl:e.get("gatewayUrl")||(typeof window.__GODMODE_DEV__<"u"?"/ws":typeof i.gatewayUrl=="string"&&i.gatewayUrl.trim()?i.gatewayUrl.trim():s.gatewayUrl),token:n||(typeof i.token=="string"?i.token:""),sessionKey:typeof i.sessionKey=="string"&&i.sessionKey.trim()?i.sessionKey.trim():s.sessionKey,lastActiveSessionKey:typeof i.lastActiveSessionKey=="string"&&i.lastActiveSessionKey.trim()?i.lastActiveSessionKey.trim():typeof i.sessionKey=="string"&&i.sessionKey.trim()||s.lastActiveSessionKey,theme:i.theme==="light"||i.theme==="dark"||i.theme==="system"||i.theme==="lifetrack"?i.theme:s.theme,chatFocusMode:typeof i.chatFocusMode=="boolean"?i.chatFocusMode:s.chatFocusMode,chatShowThinking:typeof i.chatShowThinking=="boolean"?i.chatShowThinking:s.chatShowThinking,splitRatio:typeof i.splitRatio=="number"&&i.splitRatio>=.4&&i.splitRatio<=.7?i.splitRatio:s.splitRatio,navCollapsed:typeof i.navCollapsed=="boolean"?i.navCollapsed:s.navCollapsed,navGroupsCollapsed:typeof i.navGroupsCollapsed=="object"&&i.navGroupsCollapsed!==null?i.navGroupsCollapsed:s.navGroupsCollapsed,openTabs:Array.isArray(i.openTabs)&&i.openTabs.every(o=>typeof o=="string")?i.openTabs:s.openTabs,tabLastViewed:typeof i.tabLastViewed=="object"&&i.tabLastViewed!==null?i.tabLastViewed:s.tabLastViewed,userName:typeof i.userName=="string"?i.userName.trim().slice(0,50):s.userName,userAvatar:typeof i.userAvatar=="string"?i.userAvatar.trim():s.userAvatar,chatParallelView:typeof i.chatParallelView=="boolean"?i.chatParallelView:s.chatParallelView,parallelLanes:Array.isArray(i.parallelLanes)&&i.parallelLanes.length===4?i.parallelLanes.map(o=>typeof o=="string"?o:null):s.parallelLanes}}catch{return s}}function _b(e){localStorage.setItem(_u,JSON.stringify(e))}const xb=56,Cb="quantum-particles",Eb="quantum-particle";let dt=null,Un=null;function Te(e,t){return Math.random()*(t-e)+e}function xu(){if(Cu(),typeof document>"u")return;const e=document.querySelector(".shell");if(!e){Un=requestAnimationFrame(()=>{Un=null,xu()});return}dt=document.createElement("div"),dt.className=Cb,Object.assign(dt.style,{position:"fixed",inset:"0",pointerEvents:"none",zIndex:"1",overflow:"hidden"});for(let t=0;t<xb;t++){const n=document.createElement("div");n.className=Eb;const s=Te(2,5),a=Te(.3,.65),i=Te(15,35),o=Te(0,12),l=Te(5,95),c=Te(5,95),u=Te(-150,150),p=Te(-200,200),f=Te(-250,250),m=Te(-350,350),g=Te(.8,1.5);Object.assign(n.style,{position:"absolute",left:`${l}%`,top:`${c}%`,width:`${s}px`,height:`${s}px`,borderRadius:"50%",background:`rgba(235, 158, 15, ${Te(.7,1)})`,boxShadow:`0 0 ${s*3}px rgba(235, 158, 15, ${a*.7})`,opacity:"0",willChange:"transform, opacity",animation:`quantum-float ${i}s ${o}s ease-in-out infinite`}),n.style.setProperty("--particle-opacity",String(a)),n.style.setProperty("--drift-x",`${u}px`),n.style.setProperty("--drift-y",`${p}px`),n.style.setProperty("--drift-end-x",`${f}px`),n.style.setProperty("--drift-end-y",`${m}px`),n.style.setProperty("--particle-scale-mid",String(g)),dt.appendChild(n)}e.prepend(dt)}function Cu(){Un!==null&&(cancelAnimationFrame(Un),Un=null),dt&&(dt.remove(),dt=null)}function Lb(){return typeof window>"u"||typeof window.matchMedia!="function"||window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"lifetrack"}function Do(e){return e==="system"?Lb():e==="light"?"lifetrack":e}const Ts=e=>Number.isNaN(e)?.5:e<=0?0:e>=1?1:e,Rb=()=>typeof window>"u"||typeof window.matchMedia!="function"?!1:window.matchMedia("(prefers-reduced-motion: reduce)").matches??!1,Rn=e=>{e.classList.remove("theme-transition"),e.style.removeProperty("--theme-switch-x"),e.style.removeProperty("--theme-switch-y")},Pb=({nextTheme:e,applyTheme:t,context:n,currentTheme:s})=>{if(s===e)return;const a=globalThis.document??null;if(!a){t();return}const i=a.documentElement,o=a,l=Rb();if(!!o.startViewTransition&&!l){let u=.5,p=.5;if(n?.pointerClientX!==void 0&&n?.pointerClientY!==void 0&&typeof window<"u")u=Ts(n.pointerClientX/window.innerWidth),p=Ts(n.pointerClientY/window.innerHeight);else if(n?.element){const m=n.element.getBoundingClientRect();m.width>0&&m.height>0&&typeof window<"u"&&(u=Ts((m.left+m.width/2)/window.innerWidth),p=Ts((m.top+m.height/2)/window.innerHeight))}i.style.setProperty("--theme-switch-x",`${u*100}%`),i.style.setProperty("--theme-switch-y",`${p*100}%`),i.classList.add("theme-transition");const f=setTimeout(()=>{Rn(i)},1e3);try{const m=o.startViewTransition?.(()=>{t()});m?.finished?m.finished.finally(()=>{clearTimeout(f),Rn(i)}):(clearTimeout(f),Rn(i))}catch{clearTimeout(f),Rn(i),t()}return}t(),Rn(i)};function Ib(e,t){const n=Array.isArray(e)?[...new Set(e.map(s=>s.trim()).filter(s=>s.length>0))]:[];return n.length===0&&n.push(t),n}function Db(e){const t={};if(!e||typeof e!="object")return t;for(const[n,s]of Object.entries(e)){const a=n.trim();!a||typeof s!="number"||!Number.isFinite(s)||(t[a]=Math.max(t[a]??0,s))}return t}function Xe(e,t){const n=t.sessionKey.trim()||"main",s=Ib(t.openTabs,n),a=Db(t.tabLastViewed),i={...t,sessionKey:n,openTabs:s,tabLastViewed:a,lastActiveSessionKey:t.lastActiveSessionKey?.trim()||n};e.settings=i,_b(i),i.theme!==e.theme&&(e.theme=i.theme,rs(e,Do(i.theme))),e.applySessionKey=e.settings.lastActiveSessionKey}function Mo(e,t){const n=t.trim();n&&e.settings.lastActiveSessionKey!==n&&Xe(e,{...e.settings,lastActiveSessionKey:n})}function Eu(e){if(!window.location.search)return;const t=new URLSearchParams(window.location.search),n=t.get("token"),s=t.get("password"),a=t.get("session"),i=t.get("gatewayUrl");let o=!1;if(n!=null){const c=n.trim();c&&c!==e.settings.token&&Xe(e,{...e.settings,token:c}),t.delete("token"),o=!0}if(s!=null){const c=s.trim();c&&(e.password=c),t.delete("password"),o=!0}if(a!=null){const c=a.trim();if(c){e.sessionKey=c;const u=e.settings.openTabs.includes(c)?e.settings.openTabs:[...e.settings.openTabs,c];Xe(e,{...e.settings,sessionKey:c,lastActiveSessionKey:c,openTabs:u})}}if(i!=null){const c=i.trim();c&&c!==e.settings.gatewayUrl&&(e.pendingGatewayUrl=c),t.delete("gatewayUrl"),o=!0}if(!o)return;const l=new URL(window.location.href);l.search=t.toString(),window.history.replaceState({},"",l.toString())}function Oo(e,t){const n=e.tab;if(n!==t&&(e.tab=t),n==="dashboards"&&t!=="dashboards"){const s=e;if(s.dashboardPreviousSessionKey&&s.activeDashboardId){const a=s.dashboardPreviousSessionKey;s.dashboardPreviousSessionKey=null,s.activeDashboardId=null,s.activeDashboardManifest=null,s.activeDashboardHtml=null,s.dashboardChatOpen=!1,e.sessionKey=a}}t==="chat"&&(e.chatHasAutoScrolled=!1),t==="nodes"?To(e):_o(e),t==="logs"?xo(e):Co(e),t==="debug"?Eo(e):Lo(e),t==="mission-control"?fu(e):gu(e),os(e),Fo(e,t,!1)}function Lu(e,t,n){Pb({nextTheme:t,applyTheme:()=>{e.theme=t,Xe(e,{...e.settings,theme:t}),rs(e,Do(t))},context:n,currentTheme:e.theme})}async function os(e){if(e.tab==="overview"&&await Bo(e),(e.tab==="today"||e.tab==="my-day")&&(await Ys(e),L(()=>Promise.resolve().then(()=>On),void 0,import.meta.url).then(async({loadTodayQueueResults:t})=>{e.todayQueueResults=await t(e)}).catch(()=>{})),e.tab==="work"&&await Au(e),e.tab==="people"&&await Su(e),e.tab==="workspaces"&&(await ma(e),L(()=>Promise.resolve().then(()=>ge),void 0,import.meta.url).then(async({loadAllTasksWithQueueStatus:t})=>{e.allTasks=await t(e)})),e.tab==="wheel-of-life"&&await Js(e),e.tab==="vision-board"&&await Mi(e),e.tab==="lifetracks"&&await Qs(e),e.tab==="life"&&await Promise.all([Js(e),Mi(e),Qs(e)]),e.tab==="data"&&e.handleDataRefresh(),e.tab==="channels"&&await Nu(e),e.tab==="instances"&&await Po(e),e.tab==="sessions"&&(await te(e),await Ot(e)),e.tab==="cron"&&await ya(e),e.tab==="skills"&&await is(e),e.tab==="nodes"&&(await ua(e),await ft(e),await Je(e),await Ro(e)),e.tab==="chat"&&(await Ko(e),ln(e,!e.chatHasAutoScrolled)),e.tab==="options"){const t=e;typeof t.handleOptionsLoad=="function"&&await t.handleOptionsLoad()}if(e.tab==="trust"){const t=e,n=[];typeof t.handleTrustLoad=="function"&&n.push(t.handleTrustLoad()),n.push(Ut(t)),n.push(te(t)),await Promise.all(n)}if(e.tab==="guardrails"){const t=e;typeof t.handleGuardrailsLoad=="function"&&await t.handleGuardrailsLoad()}if(e.tab==="mission-control"){const t=e;typeof t.handleMissionControlRefresh=="function"&&await t.handleMissionControlRefresh()}if(e.tab==="setup"){const t=e;typeof t.handleLoadSetupChecklist=="function"&&t.handleLoadSetupChecklist()}if(e.tab==="dashboards"){const t=e;typeof t.handleDashboardsRefresh=="function"&&await t.handleDashboardsRefresh()}if(e.tab==="second-brain"){const t=e,n=t.secondBrainSubtab;n==="intel"?typeof t.handleIntelLoad=="function"&&await t.handleIntelLoad():n==="files"?typeof t.handleSecondBrainFileTreeRefresh=="function"&&await t.handleSecondBrainFileTreeRefresh():typeof t.handleSecondBrainRefresh=="function"&&await t.handleSecondBrainRefresh()}e.tab==="config"&&(await Bc(e),await Je(e)),e.tab==="debug"&&(await pa(e),e.eventLog=zv()),e.tab==="logs"&&(e.logsAtBottom=!0,await Ao(e,{reset:!0}),Wc(e,!0))}function Ru(){if(typeof window>"u")return"";const e=window.__OPENCLAW_CONTROL_UI_BASE_PATH__;return typeof e=="string"&&e.trim()?la(e):_m(window.location.pathname)}function Pu(e){e.theme=e.settings.theme??"system",rs(e,Do(e.theme))}function rs(e,t){if(e.themeResolved=t,typeof document>"u")return;const n=document.documentElement;n.dataset.theme=t,n.style.colorScheme=t==="dark"?"dark":"light",t==="lifetrack"?xu():Cu()}function Iu(e){if(typeof window>"u"||typeof window.matchMedia!="function")return;if(e.themeMedia=window.matchMedia("(prefers-color-scheme: dark)"),e.themeMediaHandler=n=>{e.theme==="system"&&rs(e,n.matches?"dark":"lifetrack")},typeof e.themeMedia.addEventListener=="function"){e.themeMedia.addEventListener("change",e.themeMediaHandler);return}e.themeMedia.addListener(e.themeMediaHandler)}function Du(e){if(!e.themeMedia||!e.themeMediaHandler)return;if(typeof e.themeMedia.removeEventListener=="function"){e.themeMedia.removeEventListener("change",e.themeMediaHandler);return}e.themeMedia.removeListener(e.themeMediaHandler),e.themeMedia=null,e.themeMediaHandler=null}function Mu(e,t){if(typeof window>"u")return;const n=Ad(window.location.pathname,e.basePath)??"chat";No(e,n),Fo(e,n,t)}function Ou(e){if(typeof window>"u")return;const t=Ad(window.location.pathname,e.basePath);if(!t)return;const s=new URL(window.location.href).searchParams.get("session")?.trim();if(s){e.sessionKey=s;const a=e.settings.openTabs.includes(s)?e.settings.openTabs:[...e.settings.openTabs,s];Xe(e,{...e.settings,sessionKey:s,lastActiveSessionKey:s,openTabs:a})}No(e,t)}function No(e,t){e.tab!==t&&(e.tab=t),t==="chat"&&(e.chatHasAutoScrolled=!1),t==="nodes"?To(e):_o(e),t==="logs"?xo(e):Co(e),t==="debug"?Eo(e):Lo(e),t==="mission-control"?fu(e):gu(e),e.connected&&os(e)}function Fo(e,t,n){if(typeof window>"u")return;const s=Xn(ho(t,e.basePath)),a=Xn(window.location.pathname),i=new URL(window.location.href);t==="chat"&&e.sessionKey?i.searchParams.set("session",e.sessionKey):i.searchParams.delete("session"),a!==s&&(i.pathname=s),n?window.history.replaceState({},"",i.toString()):window.history.pushState({},"",i.toString())}function we(e,t,n){if(typeof window>"u")return;const s=new URL(window.location.href);s.searchParams.set("session",t),n?window.history.replaceState({},"",s.toString()):window.history.pushState({},"",s.toString())}async function Bo(e){await Promise.all([Oe(e,!1),Po(e),te(e),as(e),pa(e)])}async function Nu(e){await Promise.all([Oe(e,!0),Bc(e),Je(e)])}async function ya(e){await Promise.all([Oe(e,!1),as(e),ha(e)])}const Mb=Object.freeze(Object.defineProperty({__proto__:null,applyResolvedTheme:rs,applySettings:Xe,applySettingsFromUrl:Eu,attachThemeListener:Iu,detachThemeListener:Du,inferBasePath:Ru,loadChannelsTab:Nu,loadCron:ya,loadOverview:Bo,onPopState:Ou,refreshActiveTab:os,setLastActiveSessionKey:Mo,setTab:Oo,setTabFromRoute:No,setTheme:Lu,syncTabWithLocation:Mu,syncThemeWithSettings:Pu,syncUrlWithSessionKey:we,syncUrlWithTab:Fo},Symbol.toStringTag,{value:"Module"}));function Xs(e){return e.chatSending||!!e.chatRunId}function Se(e,t){const n=t??e.sessionKey,s=e.chatMessage.trim();if(s)e.chatDrafts={...e.chatDrafts,[n]:s};else{const{[n]:a,...i}=e.chatDrafts;e.chatDrafts=i}}function Pe(e,t){const n=t??e.sessionKey;e.chatMessage=e.chatDrafts[n]??""}function Fu(e,t){const n=t??e.sessionKey,{[n]:s,...a}=e.chatDrafts;e.chatDrafts=a,n===e.sessionKey&&(e.chatMessage="")}function Bu(e){const t=e.trim();if(!t)return!1;const n=t.toLowerCase();return n==="/stop"?!0:n==="stop"||n==="esc"||n==="abort"||n==="wait"||n==="exit"}function Ob(e){const t=e.trim();if(!t)return!1;const n=t.toLowerCase();return n==="/new"||n==="/reset"?!0:n.startsWith("/new ")||n.startsWith("/reset ")}async function Uo(e){e.connected&&(e.chatMessage="",await mo(e))}function Nb(e,t,n){const s=t.trim(),a=!!(n&&n.length>0);if(!s&&!a)return;const i=Date.now();e.chatQueue=[...e.chatQueue,{id:ca(),text:s,createdAt:i,attachments:a?n?.map(l=>({...l})):void 0}];const o=[];if(s&&o.push({type:"text",text:s}),a&&n)for(const l of n)o.push({type:"image",source:{type:"base64",media_type:l.mimeType,data:l.dataUrl}});e.chatMessages=[...e.chatMessages,{role:"user",content:o,timestamp:i}],ln(e,!0)}async function Oi(e,t,n){Yi(e);const s=e;s.sessionKey&&s.workingSessions&&(s.workingSessions=new Set([...s.workingSessions,s.sessionKey])),n?.skipOptimisticUpdate||queueMicrotask(()=>{ln(e,!0)});const a=await go(e,t,n?.attachments,{skipOptimisticUpdate:n?.skipOptimisticUpdate});return!a&&n?.previousDraft!=null&&(e.chatMessage=n.previousDraft),!a&&n?.previousAttachments&&(e.chatAttachments=n.previousAttachments),a&&(Mo(e,e.sessionKey),e.chatAttachments=[]),a&&n?.restoreDraft&&n.previousDraft?.trim()&&(e.chatMessage=n.previousDraft),a&&n?.restoreAttachments&&n.previousAttachments?.length&&(e.chatAttachments=n.previousAttachments),ln(e,!0),a&&!e.chatRunId&&zo(e),a&&n?.refreshSessions&&(e.refreshSessionsAfterChat=!0),a}async function zo(e){if(!e.connected||Xs(e))return;const[t,...n]=e.chatQueue;if(!t)return;e.chatQueue=n,await Oi(e,t.text,{attachments:t.attachments,skipOptimisticUpdate:!0})||(e.chatQueue=[t,...e.chatQueue])}function Uu(e,t){e.chatQueue=e.chatQueue.filter(n=>n.id!==t)}async function zu(e,t,n){if(!e.connected)return;const s=e.chatMessage,a=(t??e.chatMessage).trim(),i=e.chatAttachments??[],o=t==null?i:[],l=o.length>0;if(!a&&!l)return;if(Bu(a)){await Uo(e);return}const c=Ob(a);if(t==null&&(e.chatMessage="",Fu(e)),n?.queue){Nb(e,a,o),Xs(e)||await zo(e);return}if(Xs(e)){await mo(e),await new Promise(u=>setTimeout(u,50)),await Oi(e,a,{attachments:l?o:void 0,refreshSessions:c});return}await Oi(e,a,{previousDraft:t==null?s:void 0,restoreDraft:!!(t&&n?.restoreDraft),attachments:l?o:void 0,previousAttachments:t==null?i:void 0,restoreAttachments:!!(t&&n?.restoreDraft),refreshSessions:c})}async function Ko(e){await Promise.all([ae(e),te(e,{activeMinutes:0}),Zs(e)]),ln(e,!0)}const Ku=zo;function Fb(e){const t=Kc(e.sessionKey);return t?.agentId?t.agentId:e.hello?.snapshot?.sessionDefaults?.defaultAgentId?.trim()||"main"}function Bb(e,t){const n=la(e),s=encodeURIComponent(t);return n?`${n}/avatar/${s}?meta=1`:`/avatar/${s}?meta=1`}async function Zs(e){if(!e.connected){e.chatAvatarUrl=null;return}const t=Fb(e);if(!t){e.chatAvatarUrl=null;return}e.chatAvatarUrl=null;const n=Bb(e.basePath,t);try{const s=await fetch(n,{method:"GET"});if(!s.ok){e.chatAvatarUrl=null;return}const a=await s.json(),i=typeof a.avatarUrl=="string"?a.avatarUrl.trim():"";e.chatAvatarUrl=i||null}catch{e.chatAvatarUrl=null}}const ti=Object.freeze(Object.defineProperty({__proto__:null,clearDraft:Fu,flushChatQueueForEvent:Ku,handleAbortChat:Uo,handleSendChat:zu,isChatBusy:Xs,isChatStopCommand:Bu,refreshChat:Ko,refreshChatAvatar:Zs,removeQueuedMessage:Uu,restoreDraft:Pe,saveDraft:Se},Symbol.toStringTag,{value:"Module"})),Ub={trace:!0,debug:!0,info:!0,warn:!0,error:!0,fatal:!0},zb={name:"",description:"",agentId:"",enabled:!0,scheduleKind:"every",scheduleAt:"",everyAmount:"30",everyUnit:"minutes",cronExpr:"0 7 * * *",cronTz:"",sessionTarget:"main",wakeMode:"next-heartbeat",payloadKind:"systemEvent",payloadText:"",deliver:!1,channel:"last",to:"",timeoutSeconds:"",postToMainPrefix:""};const{I:Kb}=ih,Sl=e=>e,Wb=e=>e.strings===void 0,Al=()=>document.createComment(""),Pn=(e,t,n)=>{const s=e._$AA.parentNode,a=t===void 0?e._$AB:t._$AA;if(n===void 0){const i=s.insertBefore(Al(),a),o=s.insertBefore(Al(),a);n=new Kb(i,o,e,e.options)}else{const i=n._$AB.nextSibling,o=n._$AM,l=o!==e;if(l){let c;n._$AQ?.(e),n._$AM=e,n._$AP!==void 0&&(c=e._$AU)!==o._$AU&&n._$AP(c)}if(i!==a||l){let c=n._$AA;for(;c!==i;){const u=Sl(c).nextSibling;Sl(s).insertBefore(c,a),c=u}}}return n},bt=(e,t,n=e)=>(e._$AI(t,n),e),qb={},Vb=(e,t=qb)=>e._$AH=t,Hb=e=>e._$AH,ni=e=>{e._$AR(),e._$AA.remove()};const Tl=(e,t,n)=>{const s=new Map;for(let a=t;a<=n;a++)s.set(e[a],a);return s},ba=Xi(class extends Zi{constructor(e){if(super(e),e.type!==Ji.CHILD)throw Error("repeat() can only be used in text expressions")}dt(e,t,n){let s;n===void 0?n=t:t!==void 0&&(s=t);const a=[],i=[];let o=0;for(const l of e)a[o]=s?s(l,o):o,i[o]=n(l,o),o++;return{values:i,keys:a}}render(e,t,n){return this.dt(e,t,n).values}update(e,[t,n,s]){const a=Hb(e),{values:i,keys:o}=this.dt(t,n,s);if(!Array.isArray(a))return this.ut=o,i;const l=this.ut??=[],c=[];let u,p,f=0,m=a.length-1,g=0,w=i.length-1;for(;f<=m&&g<=w;)if(a[f]===null)f++;else if(a[m]===null)m--;else if(l[f]===o[g])c[g]=bt(a[f],i[g]),f++,g++;else if(l[m]===o[w])c[w]=bt(a[m],i[w]),m--,w--;else if(l[f]===o[w])c[w]=bt(a[f],i[w]),Pn(e,c[w+1],a[f]),f++,w--;else if(l[m]===o[g])c[g]=bt(a[m],i[g]),Pn(e,a[f],a[m]),m--,g++;else if(u===void 0&&(u=Tl(o,g,w),p=Tl(l,f,m)),u.has(l[f]))if(u.has(l[m])){const A=p.get(o[g]),T=A!==void 0?a[A]:null;if(T===null){const C=Pn(e,a[f]);bt(C,i[g]),c[g]=C}else c[g]=bt(T,i[g]),Pn(e,a[f],T),a[A]=null;g++}else ni(a[m]),m--;else ni(a[f]),f++;for(;g<=w;){const A=Pn(e,c[w+1]);bt(A,i[g]),c[g++]=A}for(;f<=m;){const A=a[f++];A!==null&&ni(A)}return this.ut=o,Vb(e,c),pt}});function wa(e){Se(e);const n=`agent:main:webchat-${ca().slice(0,8)}`;e.applySettings({...e.settings,openTabs:[...e.settings.openTabs,n],sessionKey:n,lastActiveSessionKey:n,tabLastViewed:{...e.settings.tabLastViewed,[n]:Date.now()}}),e.sessionKey=n,e.chatMessage="",e.chatMessages=[],e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.loadAssistantIdentity(),we(e,n,!0),ae(e)}function Wu(e,t){const n=ho(t,e.basePath);return r`
  ...
  `}function jb(e){const t=new Date,n=new Date(t.getFullYear(),t.getMonth(),t.getDate()),s=new Date(n.getTime()-864e5);return{today:e.filter(a=>a.updatedAt&&new Date(a.updatedAt)>=n),yesterday:e.filter(a=>a.updatedAt&&new Date(a.updatedAt)>=s&&new Date(a.updatedAt)<n),older:e.filter(a=>!a.updatedAt||new Date(a.updatedAt)<s)}}let si=null;function Gb(e){if(!e.client||!e.connected)return r`
      <div
  ...
  `}function Qb(e){const t=(e.sessionPickerSearch??"").toLowerCase().trim();if(!e.client||!e.connected)return r`
      <div
...
Deletes the session entry and archives its transcript.`)&&(e.sessionsResult?.sessions&&(e.sessionsResult={...e.sessionsResult,sessions:e.sessionsResult.sessions.filter(A=>A.key!==g)}),e.client&&e.connected))try{await e.client.request("sessions.delete",{key:g,deleteTranscript:!0}),te(e)}catch(A){console.error("Failed to delete session:",A),te(e)}},u=m=>r`
  ...
  `}const Yb=["system","light","dark","lifetrack"];function Vu(e){const t=Math.max(0,Yb.indexOf(e.theme)),n=s=>a=>{const o={element:a.currentTarget};(a.clientX||a.clientY)&&(o.pointerClientX=a.clientX,o.pointerClientY=a.clientY),e.setTheme(s,o)};return r`
  ...
  `}function fw(e,t){if(e.isNotification)return r`
  ...
  `}function bw(e){const t=e.connected?`Message ${e.allyName}...`:"Reconnecting...",n=e.draft.trim()||e.attachments.length>0;return r`
    ...
    <div class="ally-panel__input">
      <textarea
        ...
        placeholder=${t}
        rows="1"
        @input=${s=>{const a=s.target;dw(a),e.onDraftChange(a.value)}}
        @paste=${s=>vw(s,e)}
  ...
  `};function Ze(e){const{schema:t,value:n,path:s,hints:a,unsupported:i,disabled:o,onPatch:l}=e,c=e.showLabel??!0,u=qe(t),p=Ce(s,a),f=p?.label??t.title??tt(String(s.at(-1))),m=p?.help??t.description,g=ka(s);if(i.has(g))return r`<div class="cfg-field cfg-field--error">
      ...
      `}return Ml({...e,options:w,value:n??t.default})}if(u==="object")return Cw(e);if(u==="array")return Ew(e);if(u==="boolean"){const w=typeof n=="boolean"?n:typeof t.default=="boolean"?t.default:!1;return r`
    ...
    `}return u==="number"||u==="integer"?xw(e):u==="string"?Dl({...e,inputType:"text"}):r`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${f}</div>
      <div class="cfg-field__error">Unsupported type: ${u}. Use Raw mode.</div>
    </div>
  `}function Dl(e){const{schema:t,value:n,path:s,hints:a,disabled:i,onPatch:o,inputType:l}=e,c=e.showLabel??!0,u=Ce(s,a),p=u?.label??t.title??tt(String(s.at(-1))),f=u?.help??t.description,m=u?.sensitive??Sw(s),g=u?.placeholder??(m?"••••":t.default!==void 0?`Default: ${an(t.default)}`:""),w=n??"";return r`
    <div class="cfg-field">
  ...
  `}function xw(e){const{schema:t,value:n,path:s,hints:a,disabled:i,onPatch:o}=e,l=e.showLabel??!0,c=Ce(s,a),u=c?.label??t.title??tt(String(s.at(-1))),p=c?.help??t.description,f=n??t.default??"",m=typeof f=="number"?f:0;return r`
    <div class="cfg-field">
  ...
  `}function Ml(e){const{schema:t,value:n,path:s,hints:a,disabled:i,options:o,onPatch:l}=e,c=e.showLabel??!0,u=Ce(s,a),p=u?.label??t.title??tt(String(s.at(-1))),f=u?.help??t.description,m=n??t.default,g=o.findIndex(A=>A===m||String(A)===String(m)),w="__unset__";return r`
    <div class="cfg-field">
      ${c?r`<label class="cfg-field__label">${p}</label>`:h}
      ${f?r`<div class="cfg-field__help">${f}</div>`:h}
      <select
  ...
  `}function Ew(e){const{schema:t,value:n,path:s,hints:a,unsupported:i,disabled:o,onPatch:l}=e,c=e.showLabel??!0,u=Ce(s,a),p=u?.label??t.title??tt(String(s.at(-1))),f=u?.help??t.description,m=Array.isArray(t.items)?t.items[0]:t.items;if(!m)return r`
      <div class="cfg-field cfg-field--error">
  ...
  `}function Ol(e){const{schema:t,value:n,path:s,hints:a,unsupported:i,disabled:o,reservedKeys:l,onPatch:c}=e,u=Tw(t),p=Object.entries(n??{}).filter(([f])=>!l.has(f));return r`
    <div class="cfg-map">
      <div class="cfg-map__header">
        ...
        <button
          ...
          class="cfg-map__add"
          ?disabled=${o}
          @click=${()=>{const f={...n};let m=1,g=`custom-${m}`;for(;g in f;)m+=1,g=`custom-${m}`;f[g]=u?{}:Yu(t),c(s,f)}}
        >
      ...
      ${p.length===0?r`
        ...
        <div class="cfg-map__items">
          ${p.map(([f,m])=>{const g=[...s,f],w=_w(m);return r`
  ...
  `},Wo={env:{label:"Environment Variables",description:"Environment variables passed to the gateway process"},update:{label:"Updates",description:"Auto-update settings and release channel"},agents:{label:"Agents",description:"Agent configurations, models, and identities"},auth:{label:"Authentication",description:"API keys and authentication profiles"},channels:{label:"Channels",description:"Messaging channels (Telegram, Discord, Slack, etc.)"},messages:{label:"Messages",description:"Message handling and routing settings"},commands:{label:"Commands",description:"Custom slash commands"},hooks:{label:"Hooks",description:"Webhooks and event hooks"},skills:{label:"Skills",description:"Skill packs and capabilities"},tools:{label:"Tools",description:"Tool configurations (browser, search, etc.)"},gateway:{label:"Gateway",description:"Gateway server settings (port, auth, binding)"},wizard:{label:"Setup Wizard",description:"Setup wizard state and history"},meta:{label:"Metadata",description:"Gateway metadata and version information"},logging:{label:"Logging",description:"Log levels and output configuration"},browser:{label:"Browser",description:"Browser automation settings"},ui:{label:"UI",description:"User interface preferences"},models:{label:"Models",description:"AI model configurations and providers"},bindings:{label:"Bindings",description:"Key bindings and shortcuts"},broadcast:{label:"Broadcast",description:"Broadcast and notification settings"},audio:{label:"Audio",description:"Audio input/output settings"},session:{label:"Session",description:"Session management and persistence"},cron:{label:"Cron",description:"Scheduled tasks and automation"},web:{label:"Web",description:"Web server and API settings"},discovery:{label:"Discovery",description:"Service discovery and networking"},canvasHost:{label:"Canvas Host",description:"Canvas rendering and display"},talk:{label:"Talk",description:"Voice and speech settings"},plugins:{label:"Plugins",description:"Plugin management and extensions"},user:{label:"User Profile",description:"Your display name and avatar"}};function Fl(e){return Nl[e]??Nl.default}function Lw(e,t,n){if(!n)return!0;const s=n.toLowerCase(),a=Wo[e];return e.toLowerCase().includes(s)||a&&(a.label.toLowerCase().includes(s)||a.description.toLowerCase().includes(s))?!0:Nn(t,s)}function Nn(e,t){if(e.title?.toLowerCase().includes(t)||e.description?.toLowerCase().includes(t)||e.enum?.some(s=>String(s).toLowerCase().includes(t)))return!0;if(e.properties){for(const[s,a]of Object.entries(e.properties))if(s.toLowerCase().includes(t)||Nn(a,t))return!0}if(e.items){const s=Array.isArray(e.items)?e.items:[e.items];for(const a of s)if(a&&Nn(a,t))return!0}if(e.additionalProperties&&typeof e.additionalProperties=="object"&&Nn(e.additionalProperties,t))return!0;const n=e.anyOf??e.oneOf??e.allOf;if(n){for(const s of n)if(s&&Nn(s,t))return!0}return!1}function Rw(e){if(!e.schema)return r`
    ...
    <div class="config-form config-form--modern">
      ${p?(()=>{const{sectionKey:f,subsectionKey:m,schema:g}=p,w=Ce([f,m],e.uiHints),A=w?.label??g.title??tt(m),T=w?.help??g.description??"",C=n[f],d=C&&typeof C=="object"?C[m]:void 0,$=`config-section-${f}-${m}`;return r`
            ...
            `})():u.map(([f,m])=>{const g=Wo[f]??{label:f.charAt(0).toUpperCase()+f.slice(1),description:m.description??""};return r`
  ...
  `}const Pw=new Set(["title","description","default","nullable"]);function Iw(e){return Object.keys(e??{}).filter(n=>!Pw.has(n)).length===0}function Ju(e){const t=e.filter(a=>a!=null),n=t.length!==e.length,s=[];for(const a of t)s.some(i=>Object.is(i,a))||s.push(a);return{enumValues:s,nullable:n}}function Xu(e){return!e||typeof e!="object"?{schema:null,unsupportedPaths:["<root>"]}:zn(e,[])}function zn(e,t){const n=new Set,s={...e},a=ka(t)||"<root>";if(e.anyOf||e.oneOf||e.allOf){const l=Dw(e,t);return l||{schema:e,unsupportedPaths:[a]}}const i=Array.isArray(e.type)&&e.type.includes("null"),o=qe(e)??(e.properties||e.additionalProperties?"object":void 0);if(s.type=o??e.type,s.nullable=i||e.nullable,s.enum){const{enumValues:l,nullable:c}=Ju(s.enum);s.enum=l,c&&(s.nullable=!0),l.length===0&&n.add(a)}if(o==="object"){const l=e.properties??{},c={};for(const[u,p]of Object.entries(l)){const f=zn(p,[...t,u]);f.schema&&(c[u]=f.schema);for(const m of f.unsupportedPaths)n.add(m)}if(s.properties=c,e.additionalProperties===!0)n.add(a);else if(e.additionalProperties===!1)s.additionalProperties=!1;else if(e.additionalProperties&&typeof e.additionalProperties=="object"&&!Iw(e.additionalProperties)){const u=zn(e.additionalProperties,[...t,"*"]);s.additionalProperties=u.schema??e.additionalProperties,u.unsupportedPaths.length>0&&n.add(a)}}else if(o==="array"){const l=Array.isArray(e.items)?e.items[0]:e.items;if(!l)n.add(a);else{const c=zn(l,[...t,"*"]);s.items=c.schema??l,c.unsupportedPaths.length>0&&n.add(a)}}else o!=="string"&&o!=="number"&&o!=="integer"&&o!=="boolean"&&!s.enum&&n.add(a);return{schema:s,unsupportedPaths:Array.from(n)}}function Dw(e,t){if(e.allOf)return null;const n=e.anyOf??e.oneOf;if(!n)return null;const s=[],a=[];let i=!1;for(const l of n){if(!l||typeof l!="object")return null;if(Array.isArray(l.enum)){const{enumValues:c,nullable:u}=Ju(l.enum);s.push(...c),u&&(i=!0);continue}if("const"in l){if(l.const==null){i=!0;continue}s.push(l.const);continue}if(qe(l)==="null"){i=!0;continue}a.push(l)}if(s.length>0&&a.length===0){const l=[];for(const c of s)l.some(u=>Object.is(u,c))||l.push(c);return{schema:{...e,enum:l,nullable:i,anyOf:void 0,oneOf:void 0,allOf:void 0},unsupportedPaths:[]}}if(a.length===1){const l=zn(a[0],t);return l.schema&&(l.schema.nullable=i||l.schema.nullable),l}const o=new Set(["string","number","integer","boolean"]);return a.length>0&&s.length===0&&a.every(l=>l.type&&o.has(String(l.type)))?{schema:{...e,nullable:i},unsupportedPaths:[]}:null}function Mw(e,t){let n=e;for(const s of t){if(!n)return null;const a=qe(n);if(a==="object"){const i=n.properties??{};if(typeof s=="string"&&i[s]){n=i[s];continue}const o=n.additionalProperties;if(typeof s=="string"&&o&&typeof o=="object"){n=o;continue}return null}if(a==="array"){if(typeof s!="number")return null;n=(Array.isArray(n.items)?n.items[0]:n.items)??null;continue}return null}return n}function Ow(e,t){const s=(e.channels??{})[t],a=e[t];return(s&&typeof s=="object"?s:null)??(a&&typeof a=="object"?a:null)??{}}function Nw(e){const t=Xu(e.schema),n=t.schema;if(!n)return r`
  ...
  `}function Bl(e){return e?e.length<=20?e:`${e.slice(0,8)}...${e.slice(-8)}`:"n/a"}function zw(e){const{props:t,nostr:n,nostrAccounts:s,accountCountLabel:a,profileFormState:i,profileFormCallbacks:o,onEditProfile:l}=e,c=s[0],u=n?.configured??c?.configured??!1,p=n?.running??c?.running??!1,f=n?.publicKey??c?.publicKey,m=n?.lastStartAt??c?.lastStartAt??null,g=n?.lastError??c?.lastError??null,w=s.length>1,A=i!=null,T=d=>{const $=d.publicKey,S=d.profile,_=S?.displayName??S?.name??d.name??d.accountId;return r`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            ...
            <span>${d.lastInboundAt?z(d.lastInboundAt):"n/a"}</span>
          </div>
          ${d.lastError?r`
    ...
    `},C=()=>{if(A&&o)return bh({state:i,callbacks:o,accountId:s[0]?.accountId??"default"});const d=c?.profile??n?.profile,{name:$,displayName:S,about:_,picture:x,nip05:P}=d??{},K=$||S||_||x||P;return r`
  ...
  `}function jw(e){const{props:t,telegram:n,telegramAccounts:s,accountCountLabel:a}=e,i=s.length>1,o=l=>{const u=l.probe?.bot?.username,p=l.name||l.accountId;return r`
      <div class="account-card">
        <div class="account-card-header">
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"No snapshot yet."}
  ...
  `}function Yw(e){return e?.channelMeta?.length?e.channelMeta.map(t=>t.id):e?.channelOrder?.length?e.channelOrder:["whatsapp","telegram","discord","googlechat","slack","signal","imessage","nostr"]}function Jw(e,t,n){const s=Zu(e,n.channelAccounts);switch(e){case"whatsapp":return Gw({props:t,whatsapp:n.whatsapp,accountCountLabel:s});case"telegram":return jw({props:t,telegram:n.telegram,telegramAccounts:n.channelAccounts?.telegram??[],accountCountLabel:s});case"discord":return Fw({props:t,discord:n.discord,accountCountLabel:s});case"googlechat":return Bw({props:t,accountCountLabel:s});case"slack":return Hw({props:t,slack:n.slack,accountCountLabel:s});case"signal":return Vw({props:t,signal:n.signal,accountCountLabel:s});case"imessage":return Uw({props:t,imessage:n.imessage,accountCountLabel:s});case"nostr":{const a=n.channelAccounts?.nostr??[],i=a[0],o=i?.accountId??"default",l=i?.profile??null,c=t.nostrProfileAccountId===o?t.nostrProfileFormState:null,u=c?{onFieldChange:t.onNostrProfileFieldChange,onSave:t.onNostrProfileSave,onImport:t.onNostrProfileImport,onCancel:t.onNostrProfileCancel,onToggleAdvanced:t.onNostrProfileToggleAdvanced}:null;return zw({props:t,nostr:n.nostr,nostrAccounts:a,accountCountLabel:s,profileFormState:c,profileFormCallbacks:u,onEditProfile:()=>t.onNostrProfileEdit(o,l)})}default:return Xw(e,t,n.channelAccounts??{})}}function Xw(e,t,n){const s=e$(t.snapshot,e),a=t.snapshot?.channels?.[e],i=typeof a?.configured=="boolean"?a.configured:void 0,o=typeof a?.running=="boolean"?a.running:void 0,l=typeof a?.connected=="boolean"?a.connected:void 0,c=typeof a?.lastError=="string"?a.lastError:void 0,u=n[e]??[],p=Zu(e,n);return r`
    <div class="card">
  ...
  `}function Zw(e){return e?.channelMeta?.length?Object.fromEntries(e.channelMeta.map(t=>[t.id,t])):{}}function e$(e,t){return Zw(e)[t]?.label??e?.channelLabels?.[t]??t}const t$=600*1e3;function ep(e){return e.lastInboundAt?Date.now()-e.lastInboundAt<t$:!1}function n$(e){return e.running?"Yes":ep(e)?"Active":"No"}function s$(e){return e.connected===!0?"Yes":e.connected===!1?"No":ep(e)?"Active":"n/a"}function a$(e){const t=n$(e),n=s$(e);return r`
    <div class="account-card">
      <div class="account-card-header">
  ...
  `}const Kn=(e,t)=>{const n=e._$AN;if(n===void 0)return!1;for(const s of n)s._$AO?.(t,!1),Kn(s,t);return!0},ea=e=>{let t,n;do{if((t=e._$AM)===void 0)break;n=t._$AN,n.delete(e),e=t}while(n?.size===0)},tp=e=>{for(let t;t=e._$AM;e=t){let n=t._$AN;if(n===void 0)t._$AN=n=new Set;else if(n.has(e))break;n.add(e),r$(t)}};function i$(e){this._$AN!==void 0?(ea(this),this._$AM=e,tp(this)):this._$AM=e}function o$(e,t=!1,n=0){const s=this._$AH,a=this._$AN;if(a!==void 0&&a.size!==0)if(t)if(Array.isArray(s))for(let i=n;i<s.length;i++)Kn(s[i],!1),ea(s[i]);else s!=null&&(Kn(s,!1),ea(s));else Kn(this,e)}const r$=e=>{e.type==Ji.CHILD&&(e._$AP??=o$,e._$AQ??=i$)};class l$ extends Zi{constructor(){super(...arguments),this._$AN=void 0}_$AT(t,n,s){super._$AT(t,n,s),tp(this),this.isConnected=t._$AU}_$AO(t,n=!0){t!==this.isConnected&&(this.isConnected=t,t?this.reconnected?.():this.disconnected?.()),n&&(Kn(this,t),ea(this))}setValue(t){if(Wb(this._$Ct))this._$Ct._$AI(t,this);else{const n=[...this._$Ct._$AH];n[this._$Ci]=t,this._$Ct._$AI(n,this,0)}}disconnected(){}reconnected(){}}const ii=new WeakMap,c$=Xi(class extends l${render(e){return h}update(e,[t]){const n=t!==this.G;return n&&this.G!==void 0&&this.rt(void 0),(n||this.lt!==this.ct)&&(this.G=t,this.ht=e.options?.host,this.rt(this.ct=e.element)),h}rt(e){if(this.isConnected||(e=void 0),typeof this.G=="function"){const t=this.ht??globalThis;let n=ii.get(t);n===void 0&&(n=new WeakMap,ii.set(t,n)),n.get(this.G)!==void 0&&this.G.call(this.ht,void 0),n.set(this.G,e),e!==void 0&&this.G.call(this.ht,e)}else this.G.value=e}get lt(){return typeof this.G=="function"?ii.get(this.ht??globalThis)?.get(this.G):this.G?.value}disconnected(){this.lt===this.ct&&this.rt(void 0)}reconnected(){this.rt(this.ct)}}),Ul=25*1024*1024,zl=50*1024*1024,Kl=20;function oi(e){return e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:e<1024*1024*1024?`${(e/(1024*1024)).toFixed(1)} MB`:`${(e/(1024*1024*1024)).toFixed(1)} GB`}function qo(e,t=0){const n=[],s=[];let a=t;const i=Array.from(e);for(const o of i){if(n.length>=Kl){s.push(`Maximum ${Kl} files allowed per upload`);break}if(o.size>Ul){s.push(`"${o.name}" is too large (${oi(o.size)}). Max ${oi(Ul)}. For larger files, mention the file path instead.`);continue}if(a+o.size>zl){s.push(`Total upload size exceeds ${oi(zl)} limit`);break}a+=o.size,n.push(o)}return{validFiles:n,errors:s}}const d$=new Set(["md","markdown","mdx"]),u$=new Set(["htm","html"]),p$=new Set(["avif","bmp","gif","heic","heif","jpeg","jpg","png","svg","svgz","webp"]);function h$(e){const t=e.replaceAll("\\","/").trim();if(!t)return e;const n=t.split("/");return n[n.length-1]||t}function f$(e){if(!e)return null;const n=e.trim().toLowerCase().split(".").pop()??"";return n?d$.has(n)?"text/markdown":u$.has(n)?"text/html":p$.has(n)?n==="svg"||n==="svgz"?"image/svg+xml":n==="jpg"?"image/jpeg":`image/${n}`:n==="pdf"?"application/pdf":n==="json"||n==="json5"?"application/json":n==="txt"||n==="text"||n==="log"?"text/plain":null:null}function np(e){const t=e.mimeType?.split(";")[0]?.trim().toLowerCase()??"";if(t)return t;const n=e.content?.trim()??"";if(n.startsWith("data:image/")){const s=/^data:(image\/[^;]+);/i.exec(n);return s?.[1]?s[1].toLowerCase():"image/*"}return f$(e.filePath??null)??"text/markdown"}function g$(e){if(!e.startsWith("file://"))return null;let t=e.slice(7);return t.startsWith("/~/")&&(t="~"+t.slice(2)),decodeURIComponent(t)}function m$(e,t){if(!t)return;const s=e.target.closest("a");if(!s)return;const a=s.getAttribute("href");if(!a)return;const i=g$(a);i&&(e.preventDefault(),e.stopPropagation(),t(i))}function v$(e){if(e.error)return r`
    ...
    `;const t=np(e),n=e.content,s=n.trim();if(t.startsWith("image/"))return s.startsWith("data:image/")?r`
        <div class="sidebar-image">
          <img src=${s} alt=${h$(e.filePath??"Image preview")} />
        </div>
      `:r`
      <div class="callout">
    ...
    ></iframe>`}if(t==="text/markdown"||t==="text/x-markdown"){const a=ug(n);return r`<div
      ...
      @click=${i=>m$(i,e.onOpenFile)}
    >${Ee($e(a))}</div>`}return r`<pre class="sidebar-plain">${n}</pre>`}function y$(e){const t=np(e);return t==="text/html"||t==="application/xhtml+xml"}function b$(e){const t=new Blob([e],{type:"text/html"}),n=URL.createObjectURL(t);window.open(n,"_blank","noopener,noreferrer")}function Ni(e){const t=e.title?.trim()||"Tool Output",n=y$(e)&&e.content;return r`
  ...
  `;ls([ns({type:Number})],Ft.prototype,"splitRatio",2);ls([ns({type:Number})],Ft.prototype,"minRatio",2);ls([ns({type:Number})],Ft.prototype,"maxRatio",2);ls([ns({type:String})],Ft.prototype,"direction",2);Ft=ls([Oc("resizable-divider")],Ft);const k$=5e3;function S$(e){const t=(e??"").trim();if(!t||t==="/")return"/consciousness-icon.webp";const n=t.startsWith("/")?t:`/${t}`;return`${n.endsWith("/")?n.slice(0,-1):n}/consciousness-icon.webp`}function Wl(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function A$(e){const t=e.sessions?.sessions?.find(a=>a.key===e.sessionKey);if(!t)return null;const n=t.totalTokens??0,s=t.contextTokens??e.sessions?.defaults?.contextTokens??2e5;return s<=0?null:n/s}function T$(e){const t=A$(e);if(t===null)return h;const n=Math.round(t*100),s=n>=90?"danger":n>=70?"warn":"ok",a=e.sessions?.sessions?.find(l=>l.key===e.sessionKey),i=a?.totalTokens??0,o=a?.contextTokens??e.sessions?.defaults?.contextTokens??2e5;return r`
    <button
      type="button"
  ...
  `}function _$(e){return e?e.active?r`
    ...
    `:e.completedAt&&Date.now()-e.completedAt<k$?r`
  ...
  `}function D$(e){return e.button===0&&!e.metaKey&&!e.ctrlKey&&!e.shiftKey&&!e.altKey}function M$(e){const t=e.href;if(t){if(e.target==="_blank"){window.open(t,"_blank","noopener,noreferrer");return}window.location.assign(t)}}function O$(e){const t=e.trim();return!t||t.includes(`
`)?null:t.startsWith("/")||t.startsWith("~/")||t.startsWith("./")||t.startsWith("../")||/^[a-z]:[\\/]/i.test(t)||/^[^:\s]+\/[^\s]+$/.test(t)&&/\.[a-z0-9]{1,12}$/i.test(t)||/^[^\s/\\:*?"<>|]+\.[a-z0-9]{1,12}$/i.test(t)&&t.length<=100?t:null}async function N$(e,t){const n=e.target,s=n instanceof Element?n:n instanceof Node?n.parentElement:null;if(!s||!t.onMessageLinkClick||!D$(e))return;const a=s.closest("a");if(a instanceof HTMLAnchorElement){if(a.hasAttribute("download"))return;const c=a.getAttribute("href");if(!c)return;try{const p=new URL(c,window.location.href);if(/^https?:$/.test(p.protocol)&&p.origin!==window.location.origin){e.preventDefault(),window.open(p.href,"_blank","noopener,noreferrer");return}}catch{}e.preventDefault(),await t.onMessageLinkClick(c)||M$(a);return}const i=s.closest("code");if(!(i instanceof HTMLElement))return;const o=(i.textContent??"").trim();if(/^https?:\/\/\S+$/i.test(o)){e.preventDefault(),window.open(o,"_blank","noopener,noreferrer");return}const l=O$(o);l&&(e.preventDefault(),await t.onMessageLinkClick(l))}function F$(e){const t=e.connected,n=e.sending||e.stream!==null;e.canAbort&&e.onAbort;const a=e.sessions?.sessions?.find(g=>g.key===e.sessionKey)?.reasoningLevel??"off",i=e.showThinking&&a!=="off",o={name:e.assistantName,avatar:e.assistantAvatar??e.assistantAvatarUrl??null},l=(e.attachments?.length??0)>0,c=e.connected?l?"Add a message or paste more images...":"Message (↩ to send, ⌘↩ to queue, Shift+↩ for line breaks)":"Connect to the gateway to start chatting…",u=e.splitRatio??.6,p=!!(e.sidebarOpen&&e.onCloseSidebar),f=S$(e.basePath),m=r`
    <div
      class="chat-thread"
      role="log"
      aria-live="polite"
  ...
  `;return r`
    <section 
      class="card chat"
      @dragover=${x$}
      @dragenter=${g=>C$(g,g.currentTarget)}
      @dragleave=${g=>E$(g,g.currentTarget)}
      @drop=${g=>L$(g,e)}
    >
  ...
  `}const ql=200;function B$(e){const t=[];let n=null;for(const s of e){if(s.kind!=="message"){n&&(t.push(n),n=null),t.push(s);continue}const a=gd(s.message),i=uo(a.role),o=a.timestamp||Date.now();!n||n.role!==i?(n&&t.push(n),n={kind:"group",key:`group:${i}:${s.key}`,role:i,messages:[{message:s.message,key:s.key}],timestamp:o,isStreaming:!1}):n.messages.push({message:s.message,key:s.key})}return n&&t.push(n),t}function U$(e){const n=e.content;if(!Array.isArray(n))return!1;for(const s of n){if(typeof s!="object"||s===null)continue;const a=s;if(a.type==="image")return!0;if(Array.isArray(a.content)){for(const i of a.content)if(!(typeof i!="object"||i===null)&&i.type==="image")return!0}}return!1}function z$(e){const t=[],n=Array.isArray(e.messages)?e.messages:[],s=Array.isArray(e.toolMessages)?e.toolMessages:[],a=Math.max(0,n.length-ql);a>0&&t.push({kind:"message",key:"chat:history:notice",message:{role:"system",content:`Showing last ${ql} messages (${a} hidden).`,timestamp:Date.now()}});for(let i=a;i<n.length;i++){const o=n[i];if(o._chatIdx=i,Am(o)){t.push({kind:"compaction-summary",key:`compaction:${i}`,message:o});continue}const l=gd(o);!e.showThinking&&l.role.toLowerCase()==="toolresult"&&!U$(o)||t.push({kind:"message",key:Vl(o,i),message:o})}if(e.showThinking)for(let i=0;i<s.length;i++)t.push({kind:"message",key:Vl(s[i],i+n.length),message:s[i]});if(e.stream!==null){const i=`stream:${e.sessionKey}:${e.streamStartedAt??"live"}`;e.stream.trim().length>0?t.push({kind:"stream",key:i,text:e.stream,startedAt:e.streamStartedAt??Date.now()}):t.push({kind:"reading-indicator",key:i})}else if(e.isWorking){const i=`working:${e.sessionKey}`;t.push({kind:"reading-indicator",key:i})}else if(e.sending||e.canAbort){const i=`sending:${e.sessionKey}`;t.push({kind:"reading-indicator",key:i})}return B$(t)}function Vl(e,t){const n=e,s=typeof n.toolCallId=="string"?n.toolCallId:"";if(s)return`tool:${s}`;const a=typeof n.id=="string"?n.id:"";if(a)return`msg:${a}`;const i=typeof n.messageId=="string"?n.messageId:"";if(i)return`msg:${i}`;const o=typeof n.timestamp=="number"?n.timestamp:null,l=typeof n.role=="string"?n.role:"unknown";return o!=null?`msg:${l}:${o}:${t}`:`msg:${l}:${t}`}function K$(e,t=128){return new Promise((n,s)=>{const a=new Image;a.addEventListener("load",()=>{const i=document.createElement("canvas");i.width=t,i.height=t;const o=i.getContext("2d");if(!o){s(new Error("Could not get canvas context"));return}const l=Math.min(a.width,a.height),c=(a.width-l)/2,u=(a.height-l)/2;o.drawImage(a,c,u,l,l,0,0,t,t),n(i.toDataURL("image/png"))}),a.addEventListener("error",()=>s(new Error("Failed to load image"))),a.src=URL.createObjectURL(e)})}let Zt="",Fn=null,_t=null,Hl=!1,ot=!1;function W$(e){Hl||(Zt=e.userName||"",Fn=e.userAvatar||null,_t=e.userAvatar||null,Hl=!0,ot=!1)}function q$(e){W$(e);const t=c=>{Zt=c.target.value,ot=!0},n=async c=>{const p=c.target.files?.[0];if(p){if(!p.type.startsWith("image/")){alert("Please select an image file");return}if(p.size>5*1024*1024){alert("Image must be less than 5MB");return}try{const f=await K$(p,128);Fn=f,_t=f,ot=!0,document.dispatchEvent(new CustomEvent("user-settings-updated"))}catch(f){console.error("Failed to process image:",f),alert("Failed to process image")}}},s=()=>{Fn=null,_t=null,ot=!0;const c=document.getElementById("user-avatar-input");c&&(c.value=""),document.dispatchEvent(new CustomEvent("user-settings-updated"))},a=()=>{e.onUpdate(Zt,Fn||""),ot=!1;const c=document.querySelector(".user-settings__save");c&&(c.textContent="Saved!",setTimeout(()=>{c.textContent="Save"},1500))},i=()=>{Zt=e.userName||"",Fn=e.userAvatar||null,_t=e.userAvatar||null,ot=!1,document.dispatchEvent(new CustomEvent("user-settings-updated"))},o=Zt||"You",l=_t?r`<img src="${_t}" alt="${o}" class="user-settings__avatar-img" />`:r`<span class="user-settings__avatar-initial">${o.charAt(0).toUpperCase()}</span>`;return r`
  ...
  `},ri=[{key:"model",label:"AI Model"},{key:"env",label:"Environment"},{key:"update",label:"Updates"},{key:"agents",label:"Agents"},{key:"auth",label:"Authentication"},{key:"channels",label:"Channels"},{key:"messages",label:"Messages"},{key:"commands",label:"Commands"},{key:"hooks",label:"Hooks"},{key:"skills",label:"Skills"},{key:"tools",label:"Tools"},{key:"gateway",label:"Gateway"},{key:"wizard",label:"Setup Wizard"},{key:"user",label:"User"}],jl=new Set(["user","model"]),Gl="__all__";function Ql(e){return Fi[e]??Fi.default}function V$(e,t){const n=Wo[e];return n||{label:t?.title??tt(e),description:t?.description??""}}function H$(e){const{key:t,schema:n,uiHints:s}=e;if(!n||qe(n)!=="object"||!n.properties)return[];const a=Object.entries(n.properties).map(([i,o])=>{const l=Ce([t,i],s),c=l?.label??o.title??tt(i),u=l?.help??o.description??"",p=l?.order??50;return{key:i,label:c,description:u,order:p}});return a.sort((i,o)=>i.order!==o.order?i.order-o.order:i.key.localeCompare(o.key)),a}function j$(e,t){if(!e||!t)return[];const n=[];function s(a,i,o){if(a===i)return;if(typeof a!=typeof i){n.push({path:o,from:a,to:i});return}if(typeof a!="object"||a===null||i===null){a!==i&&n.push({path:o,from:a,to:i});return}if(Array.isArray(a)&&Array.isArray(i)){JSON.stringify(a)!==JSON.stringify(i)&&n.push({path:o,from:a,to:i});return}const l=a,c=i,u=new Set([...Object.keys(l),...Object.keys(c)]);for(const p of u)s(l[p],c[p],o?`${o}.${p}`:p)}return s(e,t,""),n}function Yl(e,t=40){let n;try{n=JSON.stringify(e)??String(e)}catch{n=String(e)}return n.length<=t?n:n.slice(0,t-3)+"..."}const Jl={anthropic:"#d97706",openai:"#10b981","openai-codex":"#10b981",xai:"#6366f1"};function G$(e){const t=[],n=e.models,s=e.agents,a=n?.providers;if(a&&typeof a=="object")for(const[o,l]of Object.entries(a)){const c=l;for(const u of c.models??[])t.push({id:`${o}/${u.id}`,name:u.name??u.id,provider:o,providerLabel:o.charAt(0).toUpperCase()+o.slice(1),reasoning:u.reasoning??!1,contextWindow:u.contextWindow??0})}const i=s?.defaults?.models;if(i&&typeof i=="object")for(const o of Object.keys(i)){if(t.some(c=>c.id===o))continue;const l=o.split("/");t.push({id:o,name:l.slice(1).join("/"),provider:l[0]??"unknown",providerLabel:(l[0]??"unknown").replace(/-/g," ").replace(/\b\w/g,c=>c.toUpperCase()),reasoning:!1,contextWindow:0})}return t}function Q$(e){return e.startsWith("anthropic/")?["openai-codex/gpt-5.3-codex"]:["anthropic/claude-sonnet-4-6"]}function Y$(e){const t=e.formValue;if(!t)return r`<div class="config-loading"><span>Loading config...</span></div>`;const n=t.agents,s=n?.defaults?.model?.primary??"",a=n?.defaults?.model?.fallbacks??[],i=G$(t),o=new Map;for(const c of i){const u=o.get(c.provider)??[];u.push(c),o.set(c.provider,u)}const l=e.saving||e.applying;return r`
      ...
      ${Array.from(o.entries()).map(([c,u])=>r`
          <div class="model-picker__group">
            ...
            <div class="model-picker__cards">
              ${u.map(p=>{const f=p.id===s,m=Jl[p.provider]??"var(--accent)";return r`
                  <button
                    class="model-card ${f?"model-card--active":""}"
                    style="--model-accent: ${m}"
                    ?disabled=${l}
                    @click=${()=>{f||!e.onModelSwitch||e.onModelSwitch(p.id,Q$(p.id))}}
                  >
  ...
  `}function J$(e){const t=e.valid==null?"unknown":e.valid?"valid":"invalid",n=Xu(e.schema),s=n.schema?n.unsupportedPaths.length>0:!1,a=n.schema?.properties??{},i=ri.filter(P=>P.key in a&&!jl.has(P.key)),o=new Set(ri.map(P=>P.key)),l=Object.keys(a).filter(P=>!o.has(P)).map(P=>({key:P,label:P.charAt(0).toUpperCase()+P.slice(1)})),c=ri.filter(P=>jl.has(P.key)),u=[...i,...l,...c],p=e.activeSection&&n.schema&&qe(n.schema)==="object"?n.schema.properties?.[e.activeSection]:void 0,f=e.activeSection?V$(e.activeSection,p):null,m=e.activeSection?H$({key:e.activeSection,schema:p,uiHints:e.uiHints}):[],g=e.formMode==="form"&&!!e.activeSection&&m.length>0,w=e.activeSubsection===Gl,A=e.searchQuery||w?null:e.activeSubsection??m[0]?.key??null,T=e.formMode==="form"?j$(e.originalValue,e.formValue):[],C=e.formMode==="raw"&&e.raw!==e.originalRaw,d=e.formMode==="form"?T.length>0:C,$=!!e.formValue&&!e.loading&&!!n.schema,S=e.connected&&!e.saving&&d&&(e.formMode==="raw"?!0:$),_=e.connected&&!e.applying&&!e.updating&&d&&(e.formMode==="raw"?!0:$),x=e.connected&&!e.applying&&!e.updating;return r`
    <div class="config-layout">
      ...
      <main class="config-main">
        ...
        <div class="config-content">
          ${e.activeSection==="model"?Y$(e):e.activeSection==="user"?q$({userName:e.userName,userAvatar:e.userAvatar,onUpdate:e.onUserProfileUpdate}):e.formMode==="form"?r`
                  ${e.schemaLoading?r`
  ...
  `}function X$(e){const t=e.host??"unknown",n=e.ip?`(${e.ip})`:"",s=e.mode??"",a=e.version??"";return`${t} ${n} ${s} ${a}`.trim()}function Z$(e){const t=e.ts??null;return t?z(t):"n/a"}function sp(e){return e?`${Gn(e)} (${z(e)})`:"n/a"}function ek(e){if(e.totalTokens==null)return"n/a";const t=e.totalTokens??0,n=e.contextTokens??0;return n?`${t} / ${n}`:String(t)}function tk(e){if(e==null)return"";try{return JSON.stringify(e,null,2)}catch{return String(e)}}function nk(e){const t=e.state??{},n=t.nextRunAtMs?Gn(t.nextRunAtMs):"n/a",s=t.lastRunAtMs?Gn(t.lastRunAtMs):"n/a";return`${t.lastStatus??"n/a"} · next ${n} · last ${s}`}function sk(e){const t=e.schedule;return t.kind==="at"?`At ${Gn(t.atMs)}`:t.kind==="every"?`Every ${Qi(t.everyMs)}`:`Cron ${t.expr}${t.tz?` (${t.tz})`:""}`}function ak(e){const t=e.payload;return t.kind==="systemEvent"?`System: ${t.text}`:`Agent: ${t.message}`}function ik(e){const t=["last",...e.channels.filter(Boolean)],n=e.form.channel?.trim();n&&!t.includes(n)&&t.push(n);const s=new Set;return t.filter(a=>s.has(a)?!1:(s.add(a),!0))}function ok(e,t){if(t==="last")return"last";const n=e.channelMeta?.find(s=>s.id===t);return n?.label?n.label:e.channelLabels?.[t]??t}function rk(e){const t=ik(e);return r`
    <section class="grid grid-cols-2">
      <div class="card">
  ...
  `}function lk(e){const t=e.form;return t.scheduleKind==="at"?r`
      <label class="field" style="margin-top: 12px;">
        <span>Run at</span>
        <input
          type="datetime-local"
  ...
  `}function ck(e,t){const s=`list-item list-item-clickable${t.runsJobId===e.id?" list-item-selected":""}`;return r`
    <div class=${s} @click=${()=>t.onLoadRuns(e.id)}>
      <div class="list-main">
        <div class="list-title">${e.name}</div>
        <div class="list-sub">${sk(e)}</div>
        <div class="muted">${ak(e)}</div>
  ...
  `}function uk(e){const n=(e.status&&typeof e.status=="object"?e.status.securityAudit:null)?.summary??null,s=n?.critical??0,a=n?.warn??0,i=n?.info??0,o=s>0?"danger":a>0?"warn":"success",l=s>0?`${s} critical`:a>0?`${a} warnings`:"No critical issues";return r`
    <section class="grid grid-cols-2">
      <div class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
  ...
  `}function pk(e){const t=Math.max(0,e),n=Math.floor(t/1e3);if(n<60)return`${n}s`;const s=Math.floor(n/60);return s<60?`${s}m`:`${Math.floor(s/60)}h`}function wt(e,t){return t?r`<div class="exec-approval-meta-row"><span>${e}</span><span>${t}</span></div>`:h}function hk(e){const t=e.execApprovalQueue[0];if(!t)return h;const n=t.request,s=t.expiresAtMs-Date.now(),a=s>0?`expires in ${pk(s)}`:"expired",i=e.execApprovalQueue.length;return r`
  ...
  `}function fk(e){const{pendingGatewayUrl:t}=e;return t?r`
    ...
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${e.entries.length===0?r`
              ...
              `:e.entries.map(t=>mk(t))}
      </div>
    </section>
  `}function mk(e){const t=e.lastInputSeconds!=null?`${e.lastInputSeconds}s ago`:"n/a",n=e.mode??"unknown",s=Array.isArray(e.roles)?e.roles.filter(Boolean):[],a=Array.isArray(e.scopes)?e.scopes.filter(Boolean):[],i=a.length>0?a.length>3?`${a.length} scopes`:`scopes: ${a.join(", ")}`:null;return r`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${e.host??"unknown host"}</div>
        <div class="list-sub">${X$(e)}</div>
        <div class="chip-row">
  ...
  `}function vk(e){return new Date(e).toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"})}function yk(e){return new Date(e).toLocaleDateString("en-US",{month:"short",day:"numeric"})}function ap(e){return e===se()}function bk(e){const t=new Date,n=new Date(e),s=Math.floor((t.getTime()-n.getTime())/(1e3*60*60*24));return s===0?"Today":s===1?"Yesterday":s<7?`${s} days ago`:yk(e)}function wk(e){if(!e||e.length===0)return!1;const t=se();return e.some(n=>n.date===t)}function $k(e){return r`
  ...
  `}function Sk(e,t,n,s){return e?r`
  ...
  `:r`
    ...
    `}function Ak(e,t,n){return!e||e.length===0?r`
    ...
    <div class="lifetracks-list">
      ${e.map(s=>{const a=t?.date===s.date,i=ap(s.date);return r`
          <button
            ...
            @click=${()=>n?.(s)}
          >
            <span class="list-item-date">${bk(s.date)}</span>
            ${i?r`
  ...
  `}function Tk(e){if(!e?.stats)return h;const{totalGenerated:t,estimatedCost:n}=e.stats;return t===0?h:r`
  ...
  `}function _k(e){if(!e.config?.enabled&&!e.loading)return $k(e.onEnable);if(e.loading)return r`
      <div class="lifetracks-container">
    ...
    `;if(!e.data||e.data.length===0)return r`
      <div class="lifetracks-container">
        ...
        ${kk(e.generating??!1,e.generationError??null,e.onGenerate)}
      </div>
    `;const t=wk(e.data);return r`
    <div class="lifetracks-container">
      <!-- Header -->
      <div class="lifetracks-header">
        <div class="lifetracks-header-left">
  ...
  `}const Xl=["trace","debug","info","warn","error","fatal"];function xk(e){if(!e)return"";const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleTimeString()}function Ck(e,t){return t?[e.message,e.subsystem,e.raw].filter(Boolean).join(" ").toLowerCase().includes(t):!0}function Ek(e){const t=e.filterText.trim().toLowerCase(),n=Xl.some(i=>!e.levelFilters[i]),s=e.entries.filter(i=>i.level&&!e.levelFilters[i.level]?!1:Ck(i,t)),a=t||n?"filtered":"visible";return r`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${e.onScroll}>
        ${s.length===0?r`
              ...
              `:s.map(i=>r`
                <div class="log-row">
                  <div class="log-time mono">${xk(i.time)}</div>
  ...
  `}const Lk=/(^~\/|^\/|^\.\.?\/|[\\/])/;function Zl(e){const t=e.trim();if(!t)return null;const n=t.replace(/^["']|["']$/g,"").trim();return!n||/^[a-z][a-z0-9+.-]*:\/\//i.test(n)||/[*?<>|]/.test(n)||n.includes("\0")||n.includes(`
`)||n.includes("\r")||!Lk.test(n)&&!/\.[a-z0-9]{1,12}$/i.test(n)?null:n}function ip(e){const t=e instanceof Element?e:e instanceof Node?e.parentElement:null;if(!t)return null;const n=t.closest("a");if(n){const a=n.getAttribute("href")??"";let i=a;if(a.includes("%"))try{i=decodeURIComponent(a)}catch{i=a}return Zl(i)}const s=t.closest("code");return!s||s.closest("pre")?null:Zl(s.textContent??"")}function Rk(e){const n=new DOMParser().parseFromString(`<div>${e}</div>`,"text/html").body.firstElementChild;if(!n)return"";const a=Ke(n,{listDepth:0,orderedIndex:[]});return Ik(a)}function Bi(e,t){if(e.nodeType===Node.TEXT_NODE)return e.textContent??"";if(e.nodeType!==Node.ELEMENT_NODE)return"";const n=e;switch(n.tagName.toLowerCase()){case"h1":return`# ${Ye(n,t)}
...
`+Bi(s,t):n+=Bi(s,t)}return n.trim()}function Pk(e,t){const n=[],s=e.querySelector("thead tr"),a=e.querySelectorAll("tbody tr");if(s){const u=Array.from(s.querySelectorAll("th, td")).map(p=>Ye(p,t));n.push(u)}for(const u of Array.from(a)){const p=Array.from(u.querySelectorAll("td, th")).map(f=>Ye(f,t));n.push(p)}if(n.length===0){const u=e.querySelectorAll("tr");for(const p of Array.from(u)){const f=Array.from(p.querySelectorAll("td, th")).map(m=>Ye(m,t));n.push(f)}}if(n.length===0)return"";const i=Math.max(...n.map(u=>u.length)),o=[];for(let u=0;u<i;u++)o[u]=Math.max(3,...n.map(p=>(p[u]??"").length));let l="";const c=u=>`| ${o.map((f,m)=>(u[m]??"").padEnd(f)).join(" | ")} |`;l+=c(n[0])+`
`,l+=`| ${o.map(u=>"-".repeat(u)).join(" | ")} |
`;for(let u=1;u<n.length;u++)l+=c(n[u])+`
`;return l+`
`}function Ik(e){let t=e;return t=t.replace(/\u00a0/g," "),t=t.replace(/\n{3,}/g,`

`),t=t.trim(),t&&!t.endsWith(`
`)&&(t+=`
`),t}function Dk(e){return e.includes(`
`)&&e.indexOf(`
`)<e.length-1?e:e.replace(/\\n/g,`
`)}function Mk(e){const t=new Date(e),s=new Date().getTime()-t.getTime(),a=Math.floor(s/(1e3*60));if(a<1)return"Just now";if(a<60)return`${a}m ago`;const i=Math.floor(a/60);return i<24?`${i}h ago`:t.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}function Ok(e){const t="VAULT",n=encodeURIComponent(`01-Daily/${e}`);return`obsidian://open?vault=${t}&file=${n}`}let Wn=null,pn=null;function tc(e,t,n=1500){Wn&&clearTimeout(Wn),Wn=setTimeout(()=>{e!==pn&&(pn=e,t(e))},n)}function Nk(e,t){Wn&&clearTimeout(Wn),e!==pn&&(pn=e,t(e))}function li(e){for(const t of e.querySelectorAll('input[type="checkbox"]')){const n=t;n.checked?n.setAttribute("checked",""):n.removeAttribute("checked")}return Rk(e.innerHTML)}function Fk(e){const{data:t,loading:n,error:s,onRefresh:a,onOpenInObsidian:i,onSaveBrief:o,onToggleCheckbox:l,onOpenFile:c}=e;if(n)return r`
  ...
  `}function Bk(e,t){return e.trim()?t.toLowerCase().includes(e.trim().toLowerCase()):!0}function ac(e,t){if(!e.trim())return t;const n=e.trim().toLowerCase();return t.filter(s=>s.name.toLowerCase().includes(n)||s.path.toLowerCase().includes(n)||s.type.toLowerCase().includes(n)||(s.searchText??"").toLowerCase().includes(n))}function ic(e,t){if(!e.trim())return t;const n=e.trim().toLowerCase();return t.filter(s=>s.title.toLowerCase().includes(n)||s.key.toLowerCase().includes(n))}function pp(e,t){if(!e.trim())return t;const n=e.trim().toLowerCase();return t.reduce((s,a)=>{if(a.type==="file")(a.name.toLowerCase().includes(n)||a.path.toLowerCase().includes(n))&&s.push(a);else{const i=pp(e,a.children??[]);i.length>0&&s.push({...a,children:i})}return s},[])}function hp(e){let t=0;for(const n of e)n.type==="file"?t++:n.children&&(t+=hp(n.children));return t}const Uk=10;function zk(e){if(!e.searchText)return null;const t=e.searchText.trim();if(!t)return null;const n=t.match(/#+ (.+?)(?:\s#|$)/);return n?n[1].trim().slice(0,120):(t.startsWith("---")?t.replace(/^---.*?---\s*/s,""):t).slice(0,120)||null}function Kk(e,t=Uk){return[...e].sort((n,s)=>s.modified.getTime()-n.modified.getTime()).slice(0,t)}function fp(e,t,n){if(e.type==="file"){const o=n.pinnedPaths.has(e.path);return r`
  ...
  `}function qk(e){const{workspaceId:t,entry:n,pinned:s,onOpen:a,onPinToggle:i}=e,o=zk(n);return r`
    <div class="ws-list-row">
      <button class="ws-list-main" @click=${()=>a?.(n)}>
        <span class="ws-list-icon">${Ho(n.type)}</span>
        <span class="ws-list-title">${n.name}</span>
  ...
  `}function Hk(e){const{browseEntries:t,breadcrumbs:n,browseSearchQuery:s,browseSearchResults:a,onBrowseFolder:i,onBrowseSearch:o,onBrowseBack:l,onCreateFolder:c,onItemClick:u}=e,p=a??t??[];return r`
      ...
      <div class="workspace-content">
                ...
                <section class="ws-section">
                  ...
                  <div class="ws-list">
                    ${_.map(R=>r`
                      ...
                      `)}
                    ${S.map(R=>ci({workspaceId:t.id,entry:R,pinned:!0,onOpen:l,onPinToggle:u}))}
                  </div>
                </section>
              `:h}

        ${Gk({tasks:t.tasks??[],workspaceName:t.name,showCompleted:a,onToggleTaskComplete:m,onCreateTask:g,onToggleCompletedTasks:w,onStartTask:A,editingTaskId:T,onEditTask:C,onUpdateTask:d})}
        ...
        ${B?r`
              <section class="ws-section">
                ...
                <div class="ws-list">
                  ${N.map(R=>qk({workspaceId:t.id,entry:R,pinned:H.has(R.path),onOpen:l,onPinToggle:u}))}
                </div>
              </section>
            `:h}
        ...
        <section class="ws-section">
          ...
          <div class="ws-list ws-list--scroll">
            ${P?K.length===0?r`<div class="ws-empty">No artifacts</div>`:K.map(R=>fp(R,0,Q)):x.length===0?r`<div class="ws-empty">No artifacts</div>`:x.map(R=>ci({workspaceId:t.id,entry:R,pinned:!1,onOpen:l,onPinToggle:u}))}
          </div>
        </section>
        ...
        ${(t.memory?.length??0)>0?r`
              <section class="ws-section">
                ...
                <div class="ws-list ws-list--scroll">
                  ${t.memory.map(R=>ci({workspaceId:t.id,entry:R,pinned:H.has(R.path),onOpen:l,onPinToggle:u}))}
                </div>
              </section>
            `:h}
      </div>
    </div>
  `}function Gk(e){const{tasks:t,workspaceName:n,showCompleted:s,onToggleTaskComplete:a,onCreateTask:i,onToggleCompletedTasks:o,onStartTask:l,editingTaskId:c,onEditTask:u,onUpdateTask:p}=e,f=ta(t.filter(g=>g.status==="pending")),m=ta(t.filter(g=>g.status==="complete"));return r`
    <section class="ws-section">
      <div class="ws-section__header">
        <h3>Tasks</h3>
  ...
  `}function Qk(e){const{connected:t,workspaces:n,selectedWorkspace:s,searchQuery:a,itemSearchQuery:i,expandedFolders:o,loading:l,createLoading:c,error:u,allTasks:p=[],taskFilter:f="outstanding",taskSort:m="due",showCompletedTasks:g=!1,editingTaskId:w,workspaceNames:A=[],onSearch:T,onItemSearch:C,onSelectWorkspace:d,onBack:$,onItemClick:S,onSessionClick:_,onPinToggle:x,onPinSessionToggle:P,onCreateWorkspace:K,onDeleteWorkspace:q,onToggleFolder:X,onTeamSetup:H,onToggleTaskComplete:j,onCreateTask:de,onSetTaskFilter:M,onSetTaskSort:N,onToggleCompletedTasks:B,onStartTask:Q,onEditTask:R,onUpdateTask:ue}=e,Ae=n.filter(F=>Bk(a,`${F.name} ${F.path} ${F.type}`));return s?jk({workspace:s,itemSearchQuery:i??"",expandedFolders:o,showCompletedTasks:g,onItemSearch:C,onBack:$,onItemClick:S,onSessionClick:_,onPinToggle:x,onPinSessionToggle:P,onToggleFolder:X,onToggleTaskComplete:j,onCreateTask:de,onToggleCompletedTasks:B,onStartTask:Q,editingTaskId:w,onEditTask:R,onUpdateTask:ue,browsePath:e.browsePath,browseEntries:e.browseEntries,breadcrumbs:e.breadcrumbs,browseSearchQuery:e.browseSearchQuery,browseSearchResults:e.browseSearchResults,onBrowseFolder:e.onBrowseFolder,onBrowseSearch:e.onBrowseSearch,onBrowseBack:e.onBrowseBack,onCreateFolder:e.onCreateFolder,onBatchPushToDrive:e.onBatchPushToDrive}):r`
    <div class="workspaces-container">
      ...
      ${l?r`
            ...
            `:r`
              <div class="workspaces-body">
                ...
                ${Yk({tasks:p,taskFilter:f,taskSort:m,onToggleTaskComplete:j,onSetTaskFilter:M,onSetTaskSort:N,onCreateTask:de,workspaceNames:A,onStartTask:Q,editingTaskId:w,onEditTask:R,onUpdateTask:ue})}
              </div>
            `}
    </div>
  `}function Yk(e){const{tasks:t,taskFilter:n,taskSort:s="due",onToggleTaskComplete:a,onSetTaskFilter:i,onSetTaskSort:o,onCreateTask:l,workspaceNames:c=[],onStartTask:u,editingTaskId:p,onEditTask:f,onUpdateTask:m}=e;if(t.length===0&&!l)return r``;let g;if(n==="outstanding")g=t.filter(A=>A.status==="pending");else if(n==="today"){const A=se();g=t.filter(T=>T.status==="pending"&&T.dueDate!=null&&T.dueDate<=A)}else n==="complete"?g=t.filter(A=>A.status==="complete"):g=t;const w=ta(g,s);return r`
    <div class="ws-all-tasks-section">
      <section class="ws-section">
        ...
        ${l?r`
              <form
                class="ws-task-create-form"
                @submit=${A=>{A.preventDefault();const T=A.currentTarget,C=T.querySelector(".ws-task-create-input"),d=T.querySelector(".ws-task-create-project"),$=C.value.trim();if(!$)return;const S=d?.value||c[0]||"";l($,S),C.value=""}}
              >
                <input
                  type="text"
                  class="ws-task-create-input"
            ...
            `:h}
        <div class="ws-list ws-list--scroll">
          ${w.length===0?r`<div class="ws-empty">No tasks</div>`:w.map(A=>Ui(A,a,u,p,f,m))}
        </div>
      </section>
    </div>
  `}function Jk(e){return e===se()}function gp(e){const t=new Date(e+"T12:00:00");return Xk(t)}function Xk(e){const t=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],n=["January","February","March","April","May","June","July","August","September","October","November","December"],s=t[e.getDay()],a=n[e.getMonth()],i=e.getDate();return`${s}, ${a} ${i}`}function Zk(e){if(!e)return"";const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const s=new Date().getTime()-t.getTime(),a=Math.floor(s/(1e3*60));if(a<1)return"Just now";if(a<60)return`${a}m ago`;const i=Math.floor(a/60);return i<24?`${i}h ago`:t.toLocaleString()}function e0(e){if(!e)return"";const t=e.split("/");return t[t.length-1]||e}function t0(e,t,n){const s=a=>{if(!e.onOpenFile)return;const i=ip(a.target);i&&(a.preventDefault(),e.onOpenFile(i))};return r`
    <div class="my-day-card agent-log-section brief-editor">
      <div class="my-day-card-header">
        ...
        <div class="agent-log-header-actions">
          ${t?.updatedAt?r`<span class="brief-updated">${Zk(t.updatedAt)}</span>`:h}
  ...
  `}function s0(e){const t=ta(e.todayTasks??[],"due"),n=t.filter(a=>a.status==="pending"),s=t.filter(a=>a.status==="complete");return r`
    <div class="my-day-card today-tasks-panel">
      ...
      <div class="my-day-card-content">
        ${e.todayTasksLoading?r`<div class="brief-loading"><div class="spinner"></div><span>Loading tasks...</span></div>`:r`
              ${e.onCreateTask?n0(e.onCreateTask):h}
  ...
  `}function a0(e,t){const n=e.status==="review";return r`
  ...
  `}function i0(e){if(!e.items.length)return h;const t=e.items.length>3;return r`
    <div class="decision-cards">
      ...
      <div class="decision-cards-list ${t?"decision-cards-list--scroll":""}">
        ${e.items.map(n=>a0(n,e))}
      </div>
    </div>
  `}function o0(e){const t=se(),n=e.selectedDate??t,s=Jk(n),a=gp(n),i=e.viewMode??"my-day";return r`
  ...
  `}function r0(e){const t=se(),n=e.selectedDate??t,s=e.viewMode??"my-day",a=e.agentLog??null;if(e.loading)return r`
    ...
    <div class="my-day-container">
      ${e.decisionCards&&e.decisionCards.items.length>0?i0(e.decisionCards):h}
      ...
      <div class="my-day-columns">
        ...
        <div class="my-day-brief-col">
          ${s==="my-day"?Fk(i):t0(e,a,gp(n))}
        </div>
      </div>
    </div>
  `}function l0(e){const t=f0(e),n=w0(e);return r`
    ${k0(n)}
    ${$0(t)}
    ${c0(e)}
    <section class="card">
  ...
  `}function c0(e){const t=e.devicesList??{pending:[],paired:[]},n=Array.isArray(t.pending)?t.pending:[],s=Array.isArray(t.paired)?t.paired:[];return r`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function d0(e,t){const n=e.displayName?.trim()||e.deviceId,s=typeof e.ts=="number"?z(e.ts):"n/a",a=e.role?.trim()?`role: ${e.role}`:"role: -",i=e.isRepair?" · repair":"",o=e.remoteIp?` · ${e.remoteIp}`:"";return r`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function u0(e,t){const n=e.displayName?.trim()||e.deviceId,s=e.remoteIp?` · ${e.remoteIp}`:"",a=`roles: ${gi(e.roles)}`,i=`scopes: ${gi(e.scopes)}`,o=Array.isArray(e.tokens)?e.tokens:[];return r`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${n}</div>
        ...
        ${o.length===0?r`
              ...
              <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 6px;">
                ${o.map(l=>p0(e.deviceId,l,t))}
              </div>
            `}
      </div>
    </div>
  `}function p0(e,t,n){const s=t.revokedAtMs?"revoked":"active",a=`scopes: ${gi(t.scopes)}`,i=z(t.rotatedAtMs??t.createdAtMs??t.lastUsedAtMs??null);return r`
  ...
  `}const ut="__defaults__",oc=[{value:"deny",label:"Deny"},{value:"allowlist",label:"Allowlist"},{value:"full",label:"Full"}],h0=[{value:"off",label:"Off"},{value:"on-miss",label:"On miss"},{value:"always",label:"Always"}];function f0(e){const t=e.configForm,n=E0(e.nodes),{defaultBinding:s,agents:a}=R0(t),i=!!t,o=e.configSaving||e.configFormMode==="raw";return{ready:i,disabled:o,configDirty:e.configDirty,configLoading:e.configLoading,configSaving:e.configSaving,defaultBinding:s,agents:a,nodes:n,onBindDefault:e.onBindDefault,onBindAgent:e.onBindAgent,onSave:e.onSaveBindings,onLoadConfig:e.onLoadConfig,formMode:e.configFormMode}}function rc(e){return e==="allowlist"||e==="full"||e==="deny"?e:"deny"}function g0(e){return e==="always"||e==="off"||e==="on-miss"?e:"on-miss"}function m0(e){const t=e?.defaults??{};return{security:rc(t.security),ask:g0(t.ask),askFallback:rc(t.askFallback??"deny"),autoAllowSkills:!!(t.autoAllowSkills??!1)}}function v0(e){const t=e?.agents??{},n=Array.isArray(t.list)?t.list:[],s=[];return n.forEach(a=>{if(!a||typeof a!="object")return;const i=a,o=typeof i.id=="string"?i.id.trim():"";if(!o)return;const l=typeof i.name=="string"?i.name.trim():void 0,c=i.default===!0;s.push({id:o,name:l||void 0,isDefault:c})}),s}function y0(e,t){const n=v0(e),s=Object.keys(t?.agents??{}),a=new Map;n.forEach(o=>a.set(o.id,o)),s.forEach(o=>{a.has(o)||a.set(o,{id:o})});const i=Array.from(a.values());return i.length===0&&i.push({id:"main",isDefault:!0}),i.sort((o,l)=>{if(o.isDefault&&!l.isDefault)return-1;if(!o.isDefault&&l.isDefault)return 1;const c=o.name?.trim()?o.name:o.id,u=l.name?.trim()?l.name:l.id;return c.localeCompare(u)}),i}function b0(e,t){return e===ut?ut:e&&t.some(n=>n.id===e)?e:ut}function w0(e){const t=e.execApprovalsForm??e.execApprovalsSnapshot?.file??null,n=!!t,s=m0(t),a=y0(e.configForm,t),i=L0(e.nodes),o=e.execApprovalsTarget;let l=o==="node"&&e.execApprovalsTargetNodeId?e.execApprovalsTargetNodeId:null;o==="node"&&l&&!i.some(f=>f.id===l)&&(l=null);const c=b0(e.execApprovalsSelectedAgent,a),u=c!==ut?(t?.agents??{})[c]??null:null,p=Array.isArray(u?.allowlist)?u.allowlist??[]:[];return{ready:n,disabled:e.execApprovalsSaving||e.execApprovalsLoading,dirty:e.execApprovalsDirty,loading:e.execApprovalsLoading,saving:e.execApprovalsSaving,form:t,defaults:s,selectedScope:c,selectedAgent:u,agents:a,allowlist:p,target:o,targetNodeId:l,targetNodes:i,onSelectScope:e.onExecApprovalsSelectAgent,onSelectTarget:e.onExecApprovalsTargetChange,onPatch:e.onExecApprovalsPatch,onRemove:e.onExecApprovalsRemove,onLoad:e.onLoadExecApprovals,onSave:e.onSaveExecApprovals}}function $0(e){const t=e.nodes.length>0,n=e.defaultBinding??"";return r`
    <section class="card">
  ...
  `}function k0(e){const t=e.ready,n=e.target!=="node"||!!e.targetNodeId;return r`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
  ...
  `}function S0(e){const t=e.targetNodes.length>0,n=e.targetNodeId??"";return r`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
  ...
  `}function x0(e,t,n){const s=t.lastUsedAt?z(t.lastUsedAt):"never",a=t.lastUsedCommand?Qn(t.lastUsedCommand,120):null,i=t.lastResolvedPath?Qn(t.lastResolvedPath,120):null;return r`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function C0(e,t){const n=e.binding??"__default__",s=e.name?.trim()?`${e.name} (${e.id})`:e.id,a=t.nodes.length>0;return r`
    <div class="list-item">
      <div class="list-main">
  ...
  `}const D0=["","off","minimal","low","medium","high"],M0=["","off","on"],O0=[{value:"",label:"inherit"},{value:"off",label:"off (explicit)"},{value:"on",label:"on"}],N0=["","off","on","stream"];function F0(e){if(!e)return"";const t=e.trim().toLowerCase();return t==="z.ai"||t==="z-ai"?"zai":t}function mp(e){return F0(e)==="zai"}function B0(e){return mp(e)?M0:D0}function U0(e,t){return!t||!e||e==="off"?e:"on"}function z0(e,t){return e?t&&e==="on"?"low":e:null}function K0(e){switch(e){case"idle-7d":return"Idle > 7 days";case"task-complete":return"Task completed";case"manual":return"Manual";default:return e}}function W0(){return r`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <rect x="2" y="3" width="20" height="5" rx="1"/>
    <path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"/>
    <path d="M10 12h4"/>
  </svg>`}function q0(){return r`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <section class="card">
      ...
      <div class="table" style="margin-top: 16px;">
        ...
        ${s.length===0?r`
              ...
              `:s.map(i=>G0(i,e.basePath,e.onPatch,e.onDelete,e.onArchive,e.loading))}
      </div>
    </section>
    ...
    <section class="card archived-section">
      ...
      ${e.archivedSessionsExpanded?r`
              <div class="archived-table" style="margin-top: 12px;">
                ...
                ${e.archivedSessionsLoading?r`<div class="muted" style="padding: 12px;">Loading...</div>`:e.archivedSessions.length===0?r`<div class="muted" style="padding: 12px;">No archived sessions.</div>`:e.archivedSessions.map(n=>Q0(n,e.onUnarchive,e.loading))}
              </div>
            `:h}
    </section>
  `}function G0(e,t,n,s,a,i){const o=e.updatedAt?z(e.updatedAt):"n/a",l=e.thinkingLevel??"",c=mp(e.modelProvider),u=U0(l,c),p=B0(e.modelProvider),f=e.verboseLevel??"",m=e.reasoningLevel??"",g=e.displayName??e.key,w=e.kind!=="global",A=w?`${ho("chat",t)}?session=${encodeURIComponent(e.key)}`:null;return r`
    <div class="table-row">
      ...
      <div>
        <select
          .value=${u}
          ?disabled=${i}
          @change=${T=>{const C=T.target.value;n(e.key,{thinkingLevel:z0(C,c)})}}
        >
      ...
      <div>
        ...
        >
          ${O0.map(T=>r`<option value=${T.value}>${T.label}</option>`)}
        </select>
      </div>
      <div>
        <select
        ...
        >
          ${N0.map(T=>r`<option value=${T}>${T||"inherit"}</option>`)}
        </select>
      </div>
  ...
  `}function Q0(e,t,n){const s=z(Date.parse(e.archivedAt));return r`
  ...
  `}const Y0=[{key:"trending",label:"Trending"},{key:"updated",label:"Newest"},{key:"stars",label:"Top Rated"},{key:"downloads",label:"Popular"}];function J0(e){return r`
  ...
  `}function Z0(e){return e.results&&e.query.trim()?r`
      <div>
    ...
    <div>
      ...
      ${e.exploreItems&&e.exploreItems.length>0?r`<div class="list">
            ${e.exploreItems.map(t=>tS(t,e))}
          </div>`:e.loading?h:r`<div class="muted">No skills found.</div>`}
    </div>
  `}function eS(e,t){const n=e.slug??"unknown",s=e.displayName??n,a=t.importing===n;return r`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${s}</div>
  ...
  `}function tS(e,t){const n=t.importing===e.slug,s=sS(e.updatedAt);return r`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function nS(e){const t=e.detail,n=e.detailSlug??"",s=e.importing===n;if(e.loading&&!t)return r`<div class="muted">Loading skill details...</div>`;if(!t||!t.skill)return r`
      <div>
  ...
  `}function di(e){return e>=1e6?`${(e/1e6).toFixed(1)}M`:e>=1e3?`${(e/1e3).toFixed(1)}k`:String(e)}function jo(e){if(!e)return h;const t=[];return typeof e.stars=="number"&&e.stars>0&&t.push(r`<span class="chip" title="Stars">\u2605 ${di(e.stars)}</span>`),typeof e.downloads=="number"&&e.downloads>0&&t.push(r`<span class="chip" title="Downloads">\u2193 ${di(e.downloads)}</span>`),typeof e.installsCurrent=="number"&&e.installsCurrent>0&&t.push(r`<span class="chip" title="Active installs">\u2713 ${di(e.installsCurrent)}</span>`),t}function sS(e){const t=Date.now()-e,n=Math.floor(t/1e3),s=Math.floor(n/60),a=Math.floor(s/60),i=Math.floor(a/24);return i>30?`${Math.floor(i/30)}mo ago`:i>0?`${i}d ago`:a>0?`${a}h ago`:s>0?`${s}m ago`:"just now"}function aS(e){return r`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
  ...
  `}const lc={launch:"🚀",milestone:"🎉",health:"💪",systems:"⚙️",default:"🎯"},mS={health:"Health",wealth:"Wealth",career:"Career",relationships:"Relationships",fun:"Fun",environment:"Environment",growth:"Growth",contribution:"Contribution"};function vS(e){const t=new Date(e),n=new Date,s=t.getFullYear()-n.getFullYear();return s<=0?"This year":s===1?"Next year":`${s} years`}function yp(e){return`${Math.round(e*100)}%`}function yS(e){return lc[e]||lc.default}function bS(e){const t=e.progress*100;return r`
    <div class="vision-cda-section">
      ...
      <div class="vision-cda-meta">
        <div class="vision-cda-deadline">
          ...
          <span class="meta-label">(${vS(e.deadline)})</span>
        </div>
  ...
  `:null}function $S(e){return!e||e.length===0?r`
        ...
        ${e.map(t=>{const n=t.progress*100;return r`
            <div class="vision-theme-card">
              <div class="theme-card-header">
                <span class="theme-icon">${yS(t.id)}</span>
              ...
              ${t.wheelSpokes&&t.wheelSpokes.length>0?r`
                    <div class="theme-spokes">
                      ${t.wheelSpokes.map(s=>r`
                          <span class="theme-spoke-badge">${mS[s]||s}</span>
                        `)}
                    </div>
                  `:null}
  ...
  `}function kS(e){return!e||e.length===0?null:r`
  ...
  `}function SS(e){return!e||e.length===0?null:r`
  ...
  `}function AS(e){if(e.loading)return r`
    ...
    <div class="vision-container">
      ...
      <div class="vision-bottom-grid">
        ${kS(t.values)} ${SS(t.antiGoals)}
      </div>
    </div>
  `}const rt=["health","wealth","career","relationships","fun","environment","growth","contribution"],Ds={health:"Health",wealth:"Wealth",career:"Career",relationships:"Relationships",fun:"Fun",environment:"Environment",growth:"Growth",contribution:"Contribution"},bp={health:"💪",wealth:"💰",career:"🚀",relationships:"❤️",fun:"🎉",environment:"🏠",growth:"📚",contribution:"🤝"};function TS(e){return new Date(e).toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}function _S(e){switch(e){case"up":return"↑";case"down":return"↓";case"stable":return"→"}}function xS(e){switch(e){case"up":return"trend-up";case"down":return"trend-down";case"stable":return"trend-stable"}}function CS(e){return e<=4?"score-low":e<=6?"score-medium":"score-high"}function ES(e){const a=2*Math.PI/rt.length,i=rt.map((l,c)=>{const u=c*a-Math.PI/2,p=(e[l]?.current??5)/10,f=150+Math.cos(u)*120*p,m=150+Math.sin(u)*120*p;return`${f},${m}`}).join(" "),o=rt.map((l,c)=>{const u=c*a-Math.PI/2,p=(e[l]?.target??8)/10,f=150+Math.cos(u)*120*p,m=150+Math.sin(u)*120*p;return`${f},${m}`}).join(" ");return r`
    <div class="wheel-chart-container">
      <svg viewBox="0 0 300 300" class="wheel-chart">
        ...
        <!-- Spoke lines -->
        ${rt.map((l,c)=>{const u=c*a-Math.PI/2,p=150+Math.cos(u)*120,f=150+Math.sin(u)*120;return ys`
            <line
              x1="${150}"
              y1="${150}"
              x2="${p}"
        ...
        <!-- Score dots -->
        ${rt.map((l,c)=>{const u=c*a-Math.PI/2,p=(e[l]?.current??5)/10,f=150+Math.cos(u)*120*p,m=150+Math.sin(u)*120*p;return ys`
            <circle
              cx="${f}"
              cy="${m}"
              r="5"
        ...
        <!-- Spoke labels -->
        ${rt.map((l,c)=>{const u=c*a-Math.PI/2,p=145,f=150+Math.cos(u)*p,m=150+Math.sin(u)*p,g=e[l]?.current??5;return ys`
            <text
              x="${f}"
              y="${m}"
              text-anchor="middle"
  ...
  `}function RS(e){if(e.loading)return r`
    ...
    <div class="wheel-container">
      ...
      <!-- Date badge -->
      <div class="wheel-date-badge">As of ${TS(t.asOf)}</div>
      ...
      <div class="wheel-content">
        <!-- Radar chart -->
        <div class="wheel-chart-section">${ES(t.scores)}</div>
  ...
  `}const cc=[{key:"focusPulse.enabled",icon:"☀️",name:"Focus Pulse",description:"Morning priority ritual + persistent focus widget in the topbar. Silent 30-min pulse checks compare your activity against your plan.",default:!0}];function PS(e,t){return r`
    <button
  ...
  `}function IS(e,t,n){const a=!!(t?.[e.key]??e.default);return r`
  ...
  `}function DS(e){const{connected:t,loading:n,options:s,onToggle:a,onOpenWizard:i}=e;return t?n&&!s?r`
  ...
  `}function NS(e){if(e>=uc.length)return r`${h}`;const t=uc[e];return r`
    <div class="wizard-step-header">
      <h2 class="wizard-question">${t.question}</h2>
    </div>
  `}function FS(e,t,n,s){return r`
  ...
  `}function wp(){return r`<p class="wizard-skip-hint">Press Enter to use the default and continue</p>`}function BS(e,t){function n(a){const i=a.target.value;t.onAnswerChange("name",i)}function s(a){a.key==="Enter"&&(a.preventDefault(),t.onStepChange(1))}return r`
  ...
  `}function US(e,t){const n=Intl.DateTimeFormat().resolvedOptions().timeZone;return r`
    <div class="wizard-field">
  ...
  `}function zS(e,t){function n(a){t.onAnswerChange("focus",a.target.value)}function s(a){a.key==="Enter"&&(a.preventDefault(),t.onStepChange(3))}return r`
    <div class="wizard-field">
      <input
        type="text"
        class="wizard-input"
  ...
  `}function qS(e,t){const n=e.hardRules.length>0?e.hardRules:[];function s(o){n.includes(o)?t.onAnswerChange("hardRules",n.filter(l=>l!==o)):t.onAnswerChange("hardRules",[...n,o])}function a(){const o=document.querySelector(".wizard-rule-input");if(!o)return;const l=o.value.trim();l&&(t.onAnswerChange("hardRules",[...n,l]),o.value="",o.focus())}function i(o){o.key==="Enter"&&(o.preventDefault(),o.target.value.trim()&&a())}return r`
    <div class="wizard-field">
  ...
  `}function jS(e,t){const{answers:n,preview:s,generating:a}=e;return r`
  ...
  `}function GS(e,t){const n=e.result;return n?r`
  ...
  `:r`${h}`}function $p(){return{name:"",timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,focus:"",projects:[],commStyle:"Direct and concise -- no fluff, just answers",hardRules:[],keyPeople:[],defaultModel:"sonnet"}}function QS(){return{step:0,answers:$p(),preview:null,generating:!1,result:null,error:null}}function kp(e,t){const{step:n,answers:s}=e;if(n===9)return r`
    ...
    `;const a=(()=>{switch(n){case 0:return s.name.trim().length>0;case 1:return!0;case 2:return!0;case 3:return!0;case 4:return s.commStyle.trim().length>0;case 5:return!0;case 6:return!0;case 7:return!0;default:return!1}})(),i=n===7,o=(()=>{switch(n){case 0:return BS(s,t);case 1:return US(s,t);case 2:return zS(s,t);case 3:return KS(s,t);case 4:return WS(s,t);case 5:return qS(s,t);case 6:return VS(s,t);case 7:return HS(s,t);default:return r`${h}`}})();return r`
    <div class="wizard-fullscreen">
      <div class="wizard-container">
  ...
  `}const YS=Object.freeze(Object.defineProperty({__proto__:null,emptyWizardAnswers:$p,emptyWizardState:QS,renderOnboardingWizard:kp},Symbol.toStringTag,{value:"Module"}));function gn(e){return e===null?"none":e>=8?"high":e>=5?"medium":"low"}function cs(e){return{high:"trust-score--high",medium:"trust-score--medium",low:"trust-score--low",none:"trust-score--none"}[e]}function JS(e){return{improving:"Improving",declining:"Declining",stable:"Stable",new:"New"}[e]??"New"}function XS(e){return{improving:"trust-trend--up",declining:"trust-trend--down",stable:"trust-trend--stable",new:"trust-trend--new"}[e]??"trust-trend--new"}function ZS(e){return{improving:"↑",declining:"↓",stable:"→",new:"•"}[e]??"•"}function e1(e){const t=Date.now()-Date.parse(e),n=Math.floor(t/6e4);if(n<1)return"just now";if(n<60)return`${n}m ago`;const s=Math.floor(n/60);if(s<24)return`${s}h ago`;const a=Math.floor(s/24);return a<7?`${a}d ago`:new Date(e).toLocaleDateString()}function t1(e){const t=e.overallScore,n=gn(t);return r`
  ...
  `}function n1(e,t){const s=Math.min(100,Math.max(0,(e??t)/10*100)),a=gn(e??(t>0?t:null));return r`
  ...
  `}function s1(e,t){const s=e.trustScore!==null?e.trustScore.toFixed(1):e.avgRating>0?e.avgRating.toFixed(1):"--",a=gn(e.trustScore??(e.avgRating>0?e.avgRating:null)),i=e.count<10?10-e.count:0;return r`
    <div class="trust-card">
      ...
      <div class="trust-card-score-row">
        ...
        <span class="trust-trend ${XS(e.trend)}">
          ${ZS(e.trend)} ${JS(e.trend)}
        </span>
      </div>

      ${n1(e.trustScore,e.avgRating)}
  ...
  `}function a1(){return[{workflow:"Code Reviews",avgRating:8.2,count:14,trustScore:8.2,needsFeedback:!1,trend:"improving",recentNotes:[],recentFeedback:[]},{workflow:"Email Drafts",avgRating:6.5,count:11,trustScore:6.5,needsFeedback:!0,trend:"stable",recentNotes:[],recentFeedback:["Be more concise","Match my tone"]},{workflow:"Research",avgRating:7.8,count:3,trustScore:null,needsFeedback:!1,trend:"new",recentNotes:[],recentFeedback:[]}]}function i1(){const e=a1();return{workflows:e.map(t=>t.workflow),summaries:e,ratings:[],total:0,overallScore:7.6,totalRatings:28,totalUses:28}}function o1(e){const t=gn(e.rating);return r`
  ...
  `}function d1(e){return e<=4?"trust-daily-button--low":e<=7?"trust-daily-button--med":"trust-daily-button--high"}function hc(e){const t=[];for(let n=0;n<7;n++)t.push(e[n]??null);return r`
  ...
  `}function u1(e){const t=e.data,n=t?.todayRating??null,s=t?.recentDaily??[],a=t?.dailyStreak??0,i=t?.dailyAverage??null;if(!e.onDailyRate)return h;if(n){const o=gn(n.rating),l=n.rating<7&&!n.note;return r`
  ...
  `}function h1(e){return!e||e==="idle"?"Idle":e==="loading"?"Syncing...":e==="ok"?"Synced":"Error"}function f1(e){return!e||e==="idle"?"trust-health-dot--idle":e==="loading"?"trust-health-dot--warn":e==="ok"?"trust-health-dot--ok":"trust-health-dot--error"}function g1(e){const t=e.connected,n=e.consciousnessStatus,s=e.sessionsCount,a=e.gatewayUptimeMs,l=(t?1:0)+(n==="ok"||n==="idle"?1:0)===2&&t;return r`
    <div class="trust-health-card">
      <div class="trust-health-card-header">
      ...
      <div class="trust-health-row">
        <span class="trust-health-dot ${f1(n)}"></span>
        <span class="trust-health-label">Consciousness</span>
        <span class="trust-health-value">${h1(n)}</span>
      </div>
  ...
  `}function v1(e){const{connected:t,loading:n,data:s,onRemoveWorkflow:a,onRefresh:i}=e;if(!t)return r`
    ...
    `;const l=!(s?.summaries??[]).some(f=>f.count>0),c=l?i1():s,u=c.summaries,p=l?[]:s?.ratings??[];return r`
    <section class="tab-body trust-section">
      ${c1(e)}
  ...
  `}function y1(e){const t=Date.now()-Date.parse(e),n=Math.floor(t/6e4);if(n<1)return"just now";if(n<60)return`${n}m ago`;const s=Math.floor(n/60);if(s<24)return`${s}h ago`;const a=Math.floor(s/24);return a<7?`${a}d ago`:new Date(e).toLocaleDateString()}function b1(e){return{fired:"guardrails-badge--fired",blocked:"guardrails-badge--blocked",cleaned:"guardrails-badge--cleaned"}[e]??""}function Sp(e,t){return r`
    <button
      class="options-toggle ${e?"options-toggle--on":""}"
      role="switch"
      aria-checked="${e}"
  ...
  `}function w1(e,t,n,s){const a=e.thresholds?.[t]??0;return r`
  ...
  `}function $1(e,t,n){const s=e.thresholdLabels?Object.keys(e.thresholdLabels):[];return r`
    <div class="guardrails-card card ${e.enabled?"":"guardrails-card--disabled"}">
      ...
      ${s.length>0?r`
            <div class="guardrails-thresholds">
              ${s.map(a=>w1(e,a,e.thresholdLabels[a],n))}
            </div>
          `:h}
    </div>
  `}function k1(e,t,n){const s=e.action==="redirect"?"↪":"🚫",a=e.action==="redirect"?"redirect":"block";return r`
  ...
  `}function S1(e){return r`
    <div class="guardrails-activity-row">
      <span class="guardrails-badge ${b1(e.action)}">${e.action}</span>
      <span class="guardrails-activity-gate">${e.gateId}</span>
      <span class="guardrails-activity-detail">${e.detail}</span>
      <span class="guardrails-activity-time">${y1(e.timestamp)}</span>
    </div>
  `}function A1(e){const{connected:t,loading:n,data:s,onToggle:a,onThresholdChange:i,onCustomToggle:o,onCustomDelete:l,onToggleAddForm:c,onOpenAllyChat:u}=e;if(!t)return r`
  ...
  `}const T1={coding:"Builder",research:"Researcher",analysis:"Analyst",creative:"Creative",review:"Reviewer",ops:"Ops",task:"Agent",url:"Reader",idea:"Explorer"};function _1(e){const t=Date.now()-e,n=Math.floor(t/6e4);if(n<1)return"just now";if(n<60)return`${n}m ago`;const s=Math.floor(n/60);return s<24?`${s}h ago`:`${Math.floor(s/24)}d ago`}function x1(e){switch(e){case"started":return"▶️";case"completed":return"✅";case"failed":return"❌";case"queued":return"⏳";case"stage":return"🔄";default:return"📋"}}function fc(e){switch(e){case"coding":return"mc-type-badge mc-type-badge--coding";case"subagent":return"mc-type-badge mc-type-badge--subagent";case"swarm":return"mc-type-badge mc-type-badge--swarm";case"queue":return"mc-type-badge mc-type-badge--queue";default:return"mc-type-badge"}}function gc(e){return`mc-agent-card mc-agent-card--${e}`}function C1(e){const t=[];return e.activeNow>0&&t.push(`${e.activeNow} agent${e.activeNow>1?"s":""} working`),e.queueReview>0&&t.push(`${e.queueReview} need${e.queueReview>1?"":"s"} your attention`),e.completedToday>0&&t.push(`${e.completedToday} done today`),t.length===0&&(t.push("Nothing running"),e.completedToday>0&&t.push(`${e.completedToday} completed today`)),r`
  ...
  `}function L1(e){return e.swarmStages?r`
  ...
  `}function R1(e,t,n,s,a){const i=e.filter(o=>o.isReview===!0);return i.length===0?h:r`
  ...
  `}function P1(e,t){const n=e.filter(s=>s.status==="pending");return n.length===0?h:r`
    <div style="margin-bottom: 1.5rem">
      ...
      <div class="mc-agents-grid">
        ${n.map(s=>r`
          <div class="mc-agent-card mc-agent-card--queued">
            <div class="mc-agent-card-header">
              <div class="mc-agent-card-info">
                <span class="mc-type-badge mc-type-badge--queue">${T1[s.type]??s.type}</span>
  ...
  `}function I1(e,t,n){const s=e.filter(a=>a.isReview===!0||a.status==="failed");return s.length===0?r`
  ...
  `}function D1(e){const t=e.filter(n=>n.status==="pending");return t.length===0?h:r`<div class="mc-queue-depth-text">${t.length} more queued</div>`}function M1(e){return e?r`
    <div class="mc-idle-cta">
      <p>Prosper is idle.</p>
      <button class="mc-open-session-btn" @click=${e}>Ask Prosper what to work on</button>
    </div>
  `:h}function O1(e,t){const n=(e.type==="failed"||e.type==="completed")&&e.agentRef;return r`
    <div class="mc-feed-item ${n?"mc-feed-item--clickable":""}"
         @click=${n?()=>t?.(e.agentRef):h}>
      <span class="mc-feed-time">${_1(e.timestamp)}</span>
      <span class="mc-feed-icon">${x1(e.type)}</span>
  ...
  `}function N1(e,t){const s=e.filter(i=>i.status==="done"||i.status==="failed").filter(i=>!i.isReview);if(s.length===0)return h;const a=s.slice(0,10);return r`
    <div style="margin-bottom: 1.5rem">
  ...
  `}function F1(e){if(!e.connected)return r`<div class="mc-section"><div class="mc-empty">Not connected to gateway.</div></div>`;if(e.loading&&!e.data)return r`<div class="mc-section"><div class="mc-loading">Loading agent data...</div></div>`;if(e.error&&!e.data)return r`
    ...
    <div class="mc-section">
      ...
      ${e.fullControl?E1(t.stats):C1(t.stats)}
      ...
      ${e.fullControl?r`
        <div class="mc-two-col">
          <div class="mc-col-main">
            ...
            ${R1(t.agents,e.onApproveItem,e.onViewDetail,e.onOpenTaskSession,e.onViewTaskFiles)}

            ${P1(t.queueItems,e.onStartQueueItem)}
      ...
      `:r`
        <div>
          ...
          ${t.stats.activeNow===0&&t.stats.queueDepth===0?M1(e.onAskProsper):h}
  ...
  `}function W1(e,t,n){const{state:s,onAssignLane:a,onSendInLane:i}=n,o=s.sessionsResult?.sessions??[],l=Me(o,t),c=l?.key??t,u=s.workingSessions.has(t)||s.workingSessions.has(c),p=B1(l,t),f=Vs.get(t)??Vs.get(c),m=l?.model??"",g=l?.totalTokens??0,w=s.settings.tabLastViewed[c]??s.settings.tabLastViewed[t]??0,A=l?.updatedAt??0,T=!u&&A>w,d=t===s.sessionKey?s.chatMessages:We.get(t)??We.get(c)??[],$=S=>{S instanceof HTMLElement&&S.dispatchEvent(new CustomEvent("lane-viewed",{detail:{sessionKey:c},bubbles:!0,composed:!0}))};return r`
    <div
    ...
    >
      ...
      >
        ...
        <div class="parallel-col__meta">
          ...
          <span class="parallel-col__tokens">${U1(g)} tokens</span>
        </div>
      </div>

      <!-- Messages -->
      <div class="parallel-col__messages">
        ${d.length>0?d.slice(-120).map(z1):r`<div class="parallel-col__empty">No messages yet</div>`}
      </div>
  ...
  `}const V1=20;function Tp(e){switch(e.split(".").pop()?.toLowerCase()){case"md":return"📝";case"html":return"🌐";case"json":case"yaml":case"yml":case"toml":return"⚙️";case"ts":case"js":case"py":case"sh":case"rs":case"go":return"💻";case"css":return"🎨";default:return"📄"}}function _p(e,t=V1){const n=[];function s(a){for(const i of a){if(n.length>=t)return;const o=i;o.type==="file"?n.push(o):o.type==="directory"&&o.children&&s(o.children)}}return s(e),n}const H1=8;function j1(e){return _p(e,500).filter(n=>n.modifiedAt!=null).sort((n,s)=>(s.modifiedAt??0)-(n.modifiedAt??0)).slice(0,H1)}function G1(e){const n=Date.now()-e,s=Math.floor(n/6e4);if(s<1)return"just now";if(s<60)return`${s}m ago`;const a=Math.floor(s/60);if(a<24)return`${a}h ago`;const i=Math.floor(a/24);return i<30?`${i}d ago`:`${Math.floor(i/30)}mo ago`}function Q1(e,t){if(!e||e.length===0)return h;const n=j1(e);return n.length===0?h:r`
      ...
      <div class="work-file-list">
        ${n.map(s=>r`
          ...
          >
            ...
            <span class="work-file-meta">${s.modifiedAt?G1(s.modifiedAt):""}</span>
          </button>
        `)}
      </div>
    </div>
  `}function Y1(e,t){if(!e||e.length===0)return h;const n=_p(e);return n.length===0?h:r`
    <div class="work-file-list">
      ${n.map(s=>r`
        <button
          class="work-file-item"
      ...
      <div class="work-workspaces-section">
        ...
        <div class="my-day-grid" style="grid-template-columns: 1fr;">
          ${m.length===0&&g.length===0?r`
                ...
                `:h}
          ${m.map(w=>J1(w,a.has(w.id),i[w.id]??[],o.has(w.id),()=>c?.(w.id),u,p,f))}
          ${g.length>0?r`
  ...
  `}const e2={urgent:"badge-danger",high:"badge-warning",medium:"badge-info",low:"badge-muted"},t2={"new-feature":"New Feature","skill-recommendation":"Skill","config-optimization":"Config","workflow-suggestion":"Workflow","trend-alert":"Trend","goal-nudge":"Goal","health-warning":"Health"};function qn(e){const t=Date.now()-e;return t<6e4?"just now":t<36e5?`${Math.floor(t/6e4)}m ago`:t<864e5?`${Math.floor(t/36e5)}h ago`:`${Math.floor(t/864e5)}d ago`}function n2(e){return r`
    <div style="display: flex; flex-direction: column; gap: 16px;">
      ${s2(e)}
  ...
  `}function a2(e){const t=e.insights.filter(n=>!n.dismissed&&!n.actedOn);return t.length===0?r`
      ...
      ${t.map(n=>r`
          <div class="card" style="padding: 12px;">
            <div style="display: flex; align-items: start; gap: 8px;">
              <span class="badge ${e2[n.priority]??"badge-muted"}" style="flex-shrink: 0; font-size: 0.75em;">
                ${t2[n.category]??n.category}
              </span>
  ...
  `}function i2(e){if(!e.patterns)return h;const t=e.patterns,n=[{label:"Completion Rate (7d)",value:`${Math.round(t.taskPatterns.completionRate7d*100)}%`},{label:"Tasks/Day",value:t.taskPatterns.avgTasksPerDay.toFixed(1)},{label:"Stuck Tasks",value:String(t.taskPatterns.stuckTasks.length)},{label:"Active Days (7d)",value:String(t.activityPatterns.activeDaysLast7d)},{label:"Coding Sessions (7d)",value:String(t.codingPatterns.totalSessionsLast7d)},{label:"Stalled Goals",value:String(t.goalStatus.stalledGoals.length)}];return r`
    <div>
  ...
  `}function o2(e){if(e.discoveries.length===0)return h;const t=e.discoveries.slice(0,10);return r`
    <div>
  ...
  `}function r2(e){if(!e.status)return h;const t=e.status;return r`
  ...
  `}function et(e){if(!e)return"";try{return z(new Date(e).getTime())}catch{return""}}function hn(e){return r`<div class="second-brain-md-body">${Ee($e(e))}</div>`}function l2(e){const{identity:t}=e;return!t||t.files.length===0?ds("No identity files found","Start building your Second Brain by creating USER.md in ~/godmode/."):r`
  ...
  `}function c2(e){const{memoryBank:t,selectedEntry:n,searchQuery:s,browsingFolder:a,folderEntries:i,folderName:o}=e;if(n)return r`
      <div class="second-brain-panel">
    ...
    `;if(a&&i)return r`
      <div class="second-brain-panel">
        ...
        <div class="second-brain-section">
          ...
          <div class="second-brain-entry-list">
            ${i.length>0?i.map(u=>Ms(u,e)):r`<div class="second-brain-empty-inline">Empty folder</div>`}
          </div>
        </div>
      </div>
    `;if(!t)return ds("No memory bank files found","Start building your memory bank by telling your ally about the people, companies, and projects in your life.");const l=(s??"").toLowerCase().trim(),c=u=>l?u.filter(p=>p.name.toLowerCase().includes(l)||p.excerpt.toLowerCase().includes(l)):u;return r`
    <div class="second-brain-panel">
  ...
  `}function d2(e){const{aiPacket:t,syncing:n}=e;return r`
    <div class="second-brain-panel">
  ...
  `}const yc={connected:{dot:"●",label:"Connected",cls:"second-brain-source--connected"},available:{dot:"○",label:"Available",cls:"second-brain-source--available"}};function u2(e){const{sourcesData:t}=e;if(!t||t.sources.length===0)return ds("No sources detected","Connect data sources to build your context universe.");const n=t.sources.filter(a=>a.status==="connected"),s=t.sources.filter(a=>a.status==="available");return r`
    <div class="second-brain-panel">
  ...
  `}function p2(e){const{researchData:t,selectedEntry:n,searchQuery:s,browsingFolder:a,folderEntries:i,folderName:o}=e;if(n)return r`
      <div class="second-brain-panel">
        <button class="second-brain-back-btn" @click=${()=>e.onBack()}>
          \u{2190} Back
        </button>
    ...
    `;if(a&&i)return r`
      <div class="second-brain-panel">
        ...
        <div class="second-brain-section">
          ...
          <div class="second-brain-entry-list">
            ${i.length>0?i.map(u=>wc(u,e)):r`<div class="second-brain-empty-inline">No research in this category</div>`}
          </div>
        </div>
      </div>
  ...
  `}function h2(e){return e<1024?`${e}B`:e<1024*1024?`${(e/1024).toFixed(1)}K`:`${(e/(1024*1024)).toFixed(1)}M`}function f2(e){return r`
    <div class="sb-files-container">
      ...
      ${e.fileSearchResults?g2(e):e.fileTreeLoading?r`<div class="sb-files-loading">Loading file tree...</div>`:e.fileTree?xp(e.fileTree,e):r`<div class="sb-files-empty">No files found</div>`}
    </div>
  `}function g2(e){const t=e.fileSearchResults??[];return t.length===0?r`<div class="sb-files-empty">No results found</div>`:r`
    ...
    <div class="sb-file-tree" style="padding-left: ${n*16}px">
      ${e.map(s=>s.type==="folder"?r`
          ...
          >
            ...
            ${s.size!=null?r`<span class="sb-tree-size">${h2(s.size)}</span>`:h}
          </button>
        `)}
    </div>
  ...
  `}function m2(e){if(!e)return h;if(!e.available)return r`
  ...
  `}function v2(e){const{subtab:t,loading:n,vaultHealth:s}=e;return r`
    <section class="second-brain-container">
      ...
      ${t==="intel"?e.intelProps?n2(e.intelProps):r`<div class="second-brain-loading"><div class="second-brain-loading-spinner"></div>Loading...</div>`:n?r`<div class="second-brain-loading"><div class="second-brain-loading-spinner"></div>Loading...</div>`:t==="identity"?l2(e):t==="memory-bank"?c2(e):t==="ai-packet"?d2(e):t==="sources"?u2(e):t==="resources"?y2(e):t==="files"?f2(e):p2(e)}
    </section>
  ...
  `}function $2(e){return e==="global"?r`<span class="dashboard-card-scope">Global</span>`:r`<span class="dashboard-card-scope">${e}</span>`}function k2(e,t,n){return r`
  ...
  `}function S2(e){const{activeDashboardHtml:t,activeDashboardManifest:n,isWorking:s}=e;return!t||!n?h:r`
  ...
  `}function A2(e){const{loading:t,dashboards:n}=e;return r`
    <section class="dashboards-container">
      ...
      ${t?r`<div class="dashboards-loading">Loading...</div>`:!n||n.length===0?r`
            ...
            `:r`
              <div class="dashboards-grid">
                ${n.map(s=>k2(s,e.onSelectDashboard,e.onDeleteDashboard))}
              </div>
            `}
    </section>
  `}function T2(e){return e.error?r`
	...
	`}function E2(e,t){return r`
		<div class="onboarding-tools-overlay">
			...
			<div class="onboarding-tools-grid">
				${x2.map(n=>{const a=e.find(i=>i.id===n.id)?.status??"pending";return r`
	...
	`:r`${h}`}function R2(e){const t=e>=70?"#38a169":e>=40?"#d69e2e":"#e53e3e",n=e>=70?"Good":e>=40?"Needs Work":"Getting Started";return r`
	...
	`}function D2(e){let t="",n="",s="";const a=["rocket","lightning","fire","star","brain","crown","diamond","target","compass","mountain"],i={rocket:"🚀",lightning:"⚡",fire:"🔥",star:"⭐",brain:"🧠",crown:"👑",diamond:"💎",target:"🎯",compass:"🧭",mountain:"⛰️"};function o(l){l.preventDefault();const c=l.target,u=new FormData(c);t=u.get("name")?.trim()??"",n=u.get("mission")?.trim()??"",s=u.get("emoji")?.trim()||"🚀",t&&e({name:t,mission:n,emoji:s})}return r`
		<div class="onboarding-fullscreen">
  ...
  `}function B2(e){const t=e.steps.filter(a=>a.completed).length,n=e.steps.length,s=t===n;return r`
  ...
  `}function U2(e){const{checklist:t,checklistLoading:n,onHideSetup:s,onOpenWizard:a}=e;if(n&&!t)return r`<div class="setup-loading">Loading setup progress...</div>`;if(!t)return r`<div class="setup-loading">Couldn't load setup progress. Is the gateway running?</div>`;const{milestones:i,percentComplete:o}=t;return r`
  ...
  `:r`
    ...
    `}function $c(e){const{connected:t,integrations:n,coreProgress:s,expandedCard:a,activeGuide:i,loadingGuide:o,testingId:l,testResult:c,configValues:u}=e;if(!t)return r`
  ...
  `}function kc(e,t){const{expandedCard:n,activeGuide:s,loadingGuide:a,testingId:i,testResult:o,configValues:l}=t,c=n===e.id,u=e.status.working||e.status.configured,p=i===e.id,f=o?.id===e.id,m=e.id==="messaging-channel";return r`
...
`),n=[];for(const s of t)s.startsWith("```")||(s.match(/^\d+\./)?n.push(r`<p class="guide-step">${Sc(s)}</p>`):s.trim()&&n.push(r`<p>${Sc(s)}</p>`));return r`${n}`}function Sc(e){const t=/`([^`]+)`/g,n=/\[([^\]]+)\]\(([^)]+)\)/g;let s=e;return s=s.replace(n,'<a href="$2" target="_blank" rel="noopener">$1</a>'),s=s.replace(t,"<code>$1</code>"),s=s.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),r`<span .innerHTML=${s}></span>`}const W2=/^data:/i,q2=/^https?:\/\//i;function V2(e){const t=e.agentsList?.agents??[],s=Kc(e.sessionKey)?.agentId??e.agentsList?.defaultId??"main",i=t.find(l=>l.id===s)?.identity,o=i?.avatarUrl??i?.avatar;if(o)return W2.test(o)||q2.test(o)?o:i?.avatarUrl}function In(e,t){const n=e.dynamicSlots[t];return n?r`
  ...
  `:h}function Ac(e){return e.trim().toLowerCase().replace(/[^a-z0-9+\s]/g," ").replace(/\s+/g," ")}function zi(e){const t=e.trim();if(!t)return t;const n=t.split(/\s+/);if(n.length>=2&&n.length%2===0){const l=n.length/2,c=n.slice(0,l).join(" "),u=n.slice(l).join(" ");if(c.toLowerCase()===u.toLowerCase())return c}const s=t.replace(/\s+/g," ").toLowerCase(),a=Math.floor(s.length/2),i=s.slice(0,a).trim(),o=s.slice(a).trim();return i&&i===o?t.slice(0,Math.ceil(t.length/2)).trim():t}function Ki(e,t){const n=t?.sessionId?.trim();if(n)return`session:${n}`;const s=t?.label??t?.displayName??xe.get(t?.key??e)??xe.get(e)??"",a=Ac(zi(s));if(a){const i=String(t?.surface??"").trim().toLowerCase(),o=Ac(String(t?.subject??"")).slice(0,20),l=a.split(" ").filter(Boolean).slice(0,3).join(" ");return`name:${i}|${o}|${l||a.slice(0,24)}`}return`key:${e.trim().toLowerCase()}`}function H2(e){const t=e.sessionsResult?.sessions,n=[...new Set(e.settings.openTabs.map(l=>l.trim()).filter(Boolean))].filter(l=>l!==ce),s=Me(t,e.sessionKey),a=Ki(e.sessionKey,s),i=new Map;for(const l of n){const c=Me(t,l),u=Ki(l,c);if(!i.has(u)){i.set(u,l);continue}l===e.sessionKey&&i.set(u,l)}const o=[...i.values()];return o.length===0&&o.push(e.sessionKey.trim()||"main"),{tabKeys:o,activeIdentity:a}}function j2(e){const t=e.onboardingActive&&e.onboarding,n=e.onboardingPhase??0,s=e.onboardingData;if(t&&n===0)return I2(()=>{e.handleOnboardingStart?.()},s?.assessment);if(t&&n===1)return D2(d=>{e.handleOnboardingIdentitySubmit?.(d)});if(t&&n===6)return O2(s?.summary??null,s?.identity??null,()=>{e.handleOnboardingComplete?.()});if(e.wizardActive&&e.wizardState)return kp(e.wizardState,{onStepChange:d=>{e.handleWizardStepChange?.(d)},onAnswerChange:(d,$)=>{e.handleWizardAnswerChange?.(d,$)},onPreview:()=>{e.handleWizardPreview?.()},onGenerate:()=>{e.handleWizardGenerate?.()},onClose:()=>{e.handleWizardClose?.()}});const a=e.presenceEntries.length,i=e.sessionsResult?.count??null,o=e.cronStatus?.nextWakeAtMs??null,l=e.connected?null:"Disconnected from gateway.",c=e.tab==="chat",u=c&&(e.settings.chatFocusMode||e.onboarding||t&&n>=2),p=e.onboarding?!1:e.settings.chatShowThinking,f=V2(e),m=e.chatAvatarUrl??f??null,g=!!(e.godmodeOptions?.["focusPulse.enabled"]??!0),w=!!e.focusPulseData?.active,A=g&&w&&!!e.focusPulseData?.currentFocus,{tabKeys:T,activeIdentity:C}=H2(e);return r`
    <div class="shell ${c?"shell--chat":""} ${u?"shell--chat-focus":""} ${e.settings.navCollapsed?"shell--nav-collapsed":""} ${e.onboarding?"shell--onboarding":""}">
      ...
      <aside class="nav ${e.settings.navCollapsed?"nav--collapsed":""}">
        ${Tm.map(d=>{const $=e.settings.navGroupsCollapsed[d.label]??!1,S=d.tabs.some(x=>x===e.tab),_=!d.label||d.tabs.length===1&&Zn(d.tabs[0])===d.label;return r`
            <div class="nav-group ${$&&!S?"nav-group--collapsed":""} ${_?"nav-group--no-header":""}">
              ${_?h:r`
                <button
                  class="nav-label"
                  @click=${()=>{const x={...e.settings.navGroupsCollapsed};x[d.label]=!$,e.applySettings({...e.settings,navGroupsCollapsed:x})}}
                  aria-expanded=${!$}
                >
      ...
      <main class="content ${c?"content--chat":""}">
          ...
          <div>
            ...
            `:e.tab==="chat"?r`
              <div class="session-tabs">
                ...
                ${ba(T,d=>d,(d,$)=>{const S=Me(e.sessionsResult?.sessions,d),_=Ki(d,S)===C,P=(()=>{if(S?.label||S?.displayName)return zi(S.label??S.displayName);const M=xe.get(d);if(M)return zi(M);if(d==="agent:main:support")return"Support";if(d.includes("webchat")){const B=d.match(/webchat[:-](\d+)/);return B?`Chat ${B[1]}`:"Chat"}if(d.includes("main"))return"MAIN";const N=d.split(/[:-]/);return N[N.length-1]||d})(),K=T.length>1,q=e.workingSessions.has(d),X=e.settings.tabLastViewed[d]??0,H=S?.updatedAt??0,j=!_&&!q&&H>X,de=e.editingTabKey===d;return r`
                      <div
                        class="session-tab ${_?"session-tab--active":""} ${q?"session-tab--working":""} ${j?"session-tab--ready":""} ${de?"session-tab--editing":""}"
                        draggable="true"
                        @dragstart=${M=>{if(e.editingTabKey===d){M.preventDefault();return}M.dataTransfer.effectAllowed="move",M.dataTransfer.setData("text/session-key",d),M.dataTransfer.setData("text/plain",$.toString()),M.target.classList.add("dragging")}}
                        @click=${()=>{if(!de){if(_){e.applySettings({...e.settings,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}});return}Se(e),e.sessionKey=d,Pe(e,d),e.chatLoading=!0,e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:d,lastActiveSessionKey:d,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}}),e.loadAssistantIdentity(),we(e,d,!0),ae(e),te(e)}}}
                      ...
                      >
                        ${de?r`
                          <input
                            ...
                            @blur=${async M=>{const N=M.target;if(N._committedByEnter)return;const B=N.value.trim();e.editingTabKey=null;const Q=S?.label??S?.displayName??"";if(B!==Q){B?xe.set(d,B):xe.delete(d),e.sessionsResult?.sessions&&(e.sessionsResult={...e.sessionsResult,sessions:e.sessionsResult.sessions.map(F=>F.key===d?{...F,label:B||void 0,displayName:B||void 0}:F)});const R=await Ps(e,d,{label:B||null,displayName:B||null});te(e);const ue=R.ok&&R.canonicalKey!==d?R.canonicalKey:d,Ae=d===e.sessionKey;e.applySettings({...e.settings,...R.ok&&R.canonicalKey!==d&&e.settings.openTabs.includes(d)?{openTabs:e.settings.openTabs.map(F=>F===d?R.canonicalKey:F)}:{},tabLastViewed:{...e.settings.tabLastViewed,[ue]:Date.now()},...Ae&&R.ok&&R.canonicalKey!==d?{sessionKey:R.canonicalKey,lastActiveSessionKey:R.canonicalKey}:{}}),Ae&&R.ok&&R.canonicalKey!==d&&(e.sessionKey=R.canonicalKey,we(e,R.canonicalKey,!0))}else e.applySettings({...e.settings,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}})}}
                            @keydown=${async M=>{if(M.key==="Enter"){M.preventDefault();const N=M.target;N._committedByEnter=!0;const B=N.value.trim();e.editingTabKey=null;const Q=S?.label??S?.displayName??"";if(B!==Q){B?xe.set(d,B):xe.delete(d),e.sessionsResult?.sessions&&(e.sessionsResult={...e.sessionsResult,sessions:e.sessionsResult.sessions.map(F=>F.key===d?{...F,label:B||void 0,displayName:B||void 0}:F)});const R=await Ps(e,d,{label:B||null,displayName:B||null});te(e);const ue=R.ok&&R.canonicalKey!==d?R.canonicalKey:d,Ae=d===e.sessionKey;e.applySettings({...e.settings,...R.ok&&R.canonicalKey!==d&&e.settings.openTabs.includes(d)?{openTabs:e.settings.openTabs.map(F=>F===d?R.canonicalKey:F)}:{},tabLastViewed:{...e.settings.tabLastViewed,[ue]:Date.now()},...Ae&&R.ok&&R.canonicalKey!==d?{sessionKey:R.canonicalKey,lastActiveSessionKey:R.canonicalKey}:{}}),Ae&&R.ok&&R.canonicalKey!==d&&(e.sessionKey=R.canonicalKey,we(e,R.canonicalKey,!0))}else e.applySettings({...e.settings,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}})}else M.key==="Escape"&&(M.preventDefault(),e.editingTabKey=null)}}
                          />
                        `:(()=>{let M=null;return r`
                          <span
                            class="session-tab__name"
                            draggable="false"
                            @click=${N=>{N.stopPropagation(),M&&clearTimeout(M),M=setTimeout(()=>{M=null,e.editingTabKey!==d&&(d===e.sessionKey?e.applySettings({...e.settings,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}}):(Se(e),e.sessionKey=d,e.chatPrivateMode=!!e.privateSessions?.has(d),Pe(e,d),e.chatLoading=!0,e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:d,lastActiveSessionKey:d,tabLastViewed:{...e.settings.tabLastViewed,[d]:Date.now()}}),e.loadAssistantIdentity(),we(e,d,!0),ae(e),te(e)))},250)}}
                            @dblclick=${N=>{N.preventDefault(),N.stopPropagation(),M&&(clearTimeout(M),M=null),e.editingTabKey=d;const B=N.target.closest(".session-tab"),Q=R=>{const ue=R.target;B&&!B.contains(ue)&&(e.editingTabKey=null,document.removeEventListener("mousedown",Q,!0))};document.addEventListener("mousedown",Q,!0),requestAnimationFrame(()=>{requestAnimationFrame(()=>{const R=B?.querySelector(".session-tab__name-input");R&&(R.focus(),R.select())})})}}
                          >${P}</span>
                        `})()}
                        ${e.privateSessions?.has(d)?(()=>{const M=e.privateSessions.get(d),N=Math.max(0,M-Date.now()),B=Math.floor(N/36e5),Q=Math.floor(N%36e5/6e4),R=B>0?`${B}h ${Q}m`:`${Q}m`;return r`
                        ...
                        ${K?r`
                          <button
                            ...
                            @click=${M=>{if(M.stopPropagation(),e.privateSessions?.has(d)){e._destroyPrivateSession(d);return}const N=e.settings.openTabs.filter(Q=>Q!==d),B=d===e.sessionKey;e.applySettings({...e.settings,openTabs:N,...B?{sessionKey:N[0],lastActiveSessionKey:N[0]}:{}}),B&&(e.sessionKey=N[0],we(e,N[0],!0),ae(e))}}
        ...
        ${e.tab==="overview"?I0({connected:e.connected,hello:e.hello,settings:e.settings,password:e.password,lastError:e.lastError,presenceCount:a,sessionsCount:i,cronEnabled:e.cronStatus?.enabled??null,cronNext:o,lastChannelsRefresh:e.channelsLastSuccess,updateStatus:e.updateStatus,updateLoading:e.updateLoading,updateError:e.updateError,updateLastChecked:e.updateLastChecked,updateRunning:e.updateRunning,onSettingsChange:d=>e.applySettings(d),onPasswordChange:d=>e.password=d,onSessionKeyChange:d=>{Se(e),e.sessionKey=d,Pe(e,d),e.resetToolStream(),e.applySettings({...e.settings,sessionKey:d,lastActiveSessionKey:d}),e.loadAssistantIdentity()},onConnect:()=>e.connect(),onRefresh:()=>e.loadOverview(),onCheckUpdates:()=>wv(e),onUpdateNow:()=>{Tr(e)}}):h}

        ${e.tab==="workspaces"?Qk({connected:e.connected,workspaces:e.workspaces??[],selectedWorkspace:e.selectedWorkspace??null,searchQuery:e.workspacesSearchQuery??"",itemSearchQuery:e.workspaceItemSearchQuery??"",expandedFolders:e.workspaceExpandedFolders??new Set,loading:e.workspacesLoading??!1,createLoading:e.workspacesCreateLoading??!1,error:e.workspacesError??null,onSearch:d=>e.workspacesSearchQuery=d,onItemSearch:d=>e.workspaceItemSearchQuery=d,onCreateWorkspace:async d=>{e.workspacesCreateLoading=!0;try{const{createWorkspace:$,selectWorkspace:S}=await L(async()=>{const{createWorkspace:x,selectWorkspace:P}=await Promise.resolve().then(()=>ge);return{createWorkspace:x,selectWorkspace:P}},void 0,import.meta.url),_=await $(e,d);return _?(e.workspaceItemSearchQuery="",await S(e,_),e.showToast(`Created workspace: ${_.name}`,"success"),!0):(e.showToast("Failed to create workspace","error"),!1)}finally{e.workspacesCreateLoading=!1}},onDeleteWorkspace:async d=>{const{deleteWorkspace:$,loadAllTasksWithQueueStatus:S}=await L(async()=>{const{deleteWorkspace:x,loadAllTasksWithQueueStatus:P}=await Promise.resolve().then(()=>ge);return{deleteWorkspace:x,loadAllTasksWithQueueStatus:P}},void 0,import.meta.url);if(!await $(e,d.id)){e.showToast(`Failed to delete ${d.name}`,"error");return}e.showToast(`Deleted workspace: ${d.name}`,"success"),e.allTasks=await S(e)},onSelectWorkspace:async d=>{e.workspaceItemSearchQuery="";const{selectWorkspace:$}=await L(async()=>{const{selectWorkspace:S}=await Promise.resolve().then(()=>ge);return{selectWorkspace:S}},void 0,import.meta.url);await $(e,d)},onBack:()=>{e.selectedWorkspace=null,e.workspaceItemSearchQuery=""},onItemClick:async d=>{const{readWorkspaceFile:$}=await L(async()=>{const{readWorkspaceFile:x}=await Promise.resolve().then(()=>ge);return{readWorkspaceFile:x}},void 0,import.meta.url),S=e.selectedWorkspace?.id,_=await $(e,d.path,S);if(!_){e.showToast(`Failed to open ${d.name}`,"error");return}e.handleOpenSidebar(_.content,{mimeType:_.mime,filePath:d.path,title:d.name})},onSessionClick:async d=>{if(!d.key)return;const $=d.key;Se(e),e.sessionKey=$,Pe(e,$),e.chatLoading=!0,e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll();const S=e.settings.openTabs.includes($)?e.settings.openTabs:[...e.settings.openTabs,$];e.applySettings({...e.settings,openTabs:S,sessionKey:$,lastActiveSessionKey:$,tabLastViewed:{...e.settings.tabLastViewed,[$]:Date.now()}}),e.setTab("chat"),e.loadAssistantIdentity(),we(e,$,!0),ae(e)},onPinToggle:async(d,$,S)=>{const{toggleWorkspacePin:_}=await L(async()=>{const{toggleWorkspacePin:P}=await Promise.resolve().then(()=>ge);return{toggleWorkspacePin:P}},void 0,import.meta.url);await _(e,d,$,S)||e.showToast("Failed to update pin","error")},onPinSessionToggle:async(d,$,S)=>{const{toggleWorkspaceSessionPin:_}=await L(async()=>{const{toggleWorkspaceSessionPin:P}=await Promise.resolve().then(()=>ge);return{toggleWorkspaceSessionPin:P}},void 0,import.meta.url);await _(e,d,$,S)||e.showToast("Failed to update session pin","error")},onToggleFolder:d=>{L(async()=>{const{toggleWorkspaceFolder:$}=await Promise.resolve().then(()=>ge);return{toggleWorkspaceFolder:$}},void 0,import.meta.url).then(({toggleWorkspaceFolder:$})=>{e.workspaceExpandedFolders=$(e.workspaceExpandedFolders??new Set,d),e.requestUpdate()})},onTeamSetup:async()=>{let d="I want to set up a Team Workspace so my team can collaborate. Please walk me through it step by step, keeping it simple.";try{const $=await e.client?.request("workspaces.teamSetupPrompt",{});$?.prompt&&(d=$.prompt)}catch{}e.handleStartChatWithPrompt(d)},allTasks:e.allTasks??[],taskFilter:e.taskFilter??"outstanding",taskSort:e.taskSort??"due",showCompletedTasks:e.showCompletedTasks??!1,onToggleTaskComplete:async(d,$)=>{const{toggleTaskComplete:S,loadAllTasksWithQueueStatus:_,getWorkspace:x}=await L(async()=>{const{toggleTaskComplete:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}=await Promise.resolve().then(()=>ge);return{toggleTaskComplete:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}},void 0,import.meta.url);if(!await S(e,d,$)){e.showToast("Failed to update task","error");return}if(e.allTasks=await _(e),e.selectedWorkspace){const K=await x(e,e.selectedWorkspace.id);K&&(e.selectedWorkspace=K)}},onCreateTask:async(d,$)=>{const{createTask:S,loadAllTasksWithQueueStatus:_,getWorkspace:x}=await L(async()=>{const{createTask:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}=await Promise.resolve().then(()=>ge);return{createTask:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}},void 0,import.meta.url),P=await S(e,d,$);if(!P){e.showToast("Failed to create task","error");return}if(e.showToast(`Task created: ${P.title}`,"success"),e.allTasks=await _(e),e.selectedWorkspace){const K=await x(e,e.selectedWorkspace.id);K&&(e.selectedWorkspace=K)}},onSetTaskFilter:d=>{e.taskFilter=d},onSetTaskSort:d=>{e.taskSort=d},onToggleCompletedTasks:()=>{e.showCompletedTasks=!(e.showCompletedTasks??!1)},editingTaskId:e.editingTaskId??null,workspaceNames:(e.workspaces??[]).map(d=>d.name),onStartTask:async d=>{const{startTask:$,loadAllTasksWithQueueStatus:S}=await L(async()=>{const{startTask:K,loadAllTasksWithQueueStatus:q}=await Promise.resolve().then(()=>ge);return{startTask:K,loadAllTasksWithQueueStatus:q}},void 0,import.meta.url),_=await $(e,d);if(!_?.sessionId){e.showToast("Failed to open session for task","error");return}Se(e);const x=_.sessionId;_.task?.title&&xe.set(x,_.task.title);const P=e.settings.openTabs.includes(x)?e.settings.openTabs:[...e.settings.openTabs,x];if(e.applySettings({...e.settings,openTabs:P,sessionKey:x,lastActiveSessionKey:x,tabLastViewed:{...e.settings.tabLastViewed,[x]:Date.now()}}),e.sessionKey=x,e.setTab("chat"),_.created&&!_.queueOutput){const K=e.allTasks??[],q=e.selectedWorkspace?.tasks??[],X=[...K,...q].find(j=>j.id===d),H=X?.project?` (project: ${X.project})`:"";e.chatMessage=`Let's work on: ${X?.title??"this task"}${H}`}else e.chatMessage="";e.chatMessages=[],e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.loadAssistantIdentity(),we(e,x,!0),await ae(e),_.queueOutput&&e.chatMessages.length===0&&e.seedSessionWithAgentOutput(_.task?.title??"this task",_.queueOutput,_.agentPrompt??void 0),e.allTasks=await S(e),e.requestUpdate()},onEditTask:d=>{e.editingTaskId=d},onUpdateTask:async(d,$)=>{const{updateTask:S,loadAllTasksWithQueueStatus:_,getWorkspace:x}=await L(async()=>{const{updateTask:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}=await Promise.resolve().then(()=>ge);return{updateTask:K,loadAllTasksWithQueueStatus:q,getWorkspace:X}},void 0,import.meta.url);if(!await S(e,d,$)){e.showToast("Failed to update task","error");return}if(e.editingTaskId=null,e.allTasks=await _(e),e.selectedWorkspace){const K=await x(e,e.selectedWorkspace.id);K&&(e.selectedWorkspace=K)}},browsePath:e.workspaceBrowsePath??null,browseEntries:e.workspaceBrowseEntries??null,breadcrumbs:e.workspaceBreadcrumbs??null,browseSearchQuery:e.workspaceBrowseSearchQuery??"",browseSearchResults:e.workspaceBrowseSearchResults??null,onBrowseFolder:d=>e.handleWorkspaceBrowse(d),onBrowseSearch:d=>e.handleWorkspaceBrowseSearch(d),onBrowseBack:()=>e.handleWorkspaceBrowseBack(),onCreateFolder:d=>e.handleWorkspaceCreateFolder(d),onBatchPushToDrive:d=>e.handleBatchPushToDrive(d)}):h}

        ${e.tab==="today"||e.tab==="my-day"?e.dynamicSlots.today?In(e,"today"):r0({connected:e.connected,loading:e.myDayLoading??!1,error:e.myDayError??null,onRefresh:()=>e.handleMyDayRefresh(),dailyBrief:e.dailyBrief??null,dailyBriefLoading:e.dailyBriefLoading??!1,dailyBriefError:e.dailyBriefError??null,onBriefRefresh:()=>e.handleDailyBriefRefresh(),onBriefOpenInObsidian:()=>e.handleDailyBriefOpenInObsidian(),onBriefSave:d=>e.handleBriefSave(d),onBriefToggleCheckbox:(d,$)=>e.handleBriefToggleCheckbox(d,$),onOpenFile:d=>{e.handleOpenFile(d)},selectedDate:e.todaySelectedDate,onDatePrev:()=>e.handleDatePrev(),onDateNext:()=>e.handleDateNext(),onDateToday:()=>e.handleDateToday(),viewMode:e.todayViewMode??"my-day",onViewModeChange:d=>e.handleTodayViewModeChange(d),agentLog:e.agentLog??null,agentLogLoading:e.agentLogLoading??!1,agentLogError:e.agentLogError??null,onAgentLogRefresh:()=>e.handleMyDayRefresh(),focusPulseActive:w,onStartMorningSet:g?()=>e.handleFocusPulseStartMorning():void 0,todayTasks:e.todayTasks??[],todayTasksLoading:e.todayTasksLoading??!1,onToggleTaskComplete:(d,$)=>e.handleMyDayTaskStatusChange(d,$==="complete"?"pending":"complete"),onStartTask:d=>e.handleTodayStartTask(d),onCreateTask:d=>e.handleTodayCreateTask(d),onEditTask:d=>e.handleTodayEditTask(d),onUpdateTask:(d,$)=>e.handleTodayUpdateTask(d,$),editingTaskId:e.todayEditingTaskId,showCompletedTasks:e.todayShowCompleted,onToggleCompletedTasks:()=>e.handleTodayToggleCompleted(),decisionCards:(e.todayQueueResults??[]).length>0?{items:e.todayQueueResults,onApprove:d=>e.handleDecisionApprove(d),onReject:d=>e.handleDecisionReject(d),onViewOutput:(d,$)=>e.handleDecisionViewOutput(d,$),onOpenChat:d=>e.handleDecisionOpenChat(d)}:void 0}):h}

        ${e.tab==="work"?e.dynamicSlots.work?In(e,"work"):Z1({connected:e.connected,projects:e.workProjects??[],loading:e.workLoading??!1,error:e.workError??null,expandedProjects:e.workExpandedProjects,projectFiles:e.workProjectFiles??{},detailLoading:e.workDetailLoading??new Set,onRefresh:()=>e.handleWorkRefresh(),onToggleProject:d=>e.handleWorkToggleProject(d),onPersonClick:d=>e.handleWorkPersonClick(d),onFileClick:d=>e.handleWorkFileClick(d),onSkillClick:(d,$)=>e.handleWorkSkillClick(d,$)}):h}

        ${e.tab==="wheel-of-life"?e.dynamicSlots["wheel-of-life"]?In(e,"wheel-of-life"):RS({connected:e.connected,data:e.wheelOfLifeData??null,loading:e.wheelOfLifeLoading??!1,error:e.wheelOfLifeError??null,editMode:e.wheelOfLifeEditMode??!1,onRefresh:()=>e.handleWheelOfLifeRefresh(),onEdit:()=>e.handleWheelOfLifeEdit(),onSave:d=>e.handleWheelOfLifeSave(d),onCancel:()=>e.handleWheelOfLifeCancel()}):h}

        ${e.tab==="vision-board"?e.dynamicSlots["vision-board"]?In(e,"vision-board"):AS({connected:e.connected,data:e.visionBoardData??null,identityToday:e.visionBoardIdentityToday??null,loading:e.visionBoardLoading??!1,error:e.visionBoardError??null,onRefresh:()=>e.handleVisionBoardRefresh()}):h}

        ${e.tab==="lifetracks"?e.dynamicSlots.lifetracks?In(e,"lifetracks"):_k({connected:e.connected,data:e.lifetracksData??null,currentTrack:e.lifetracksCurrentTrack??null,loading:e.lifetracksLoading??!1,error:e.lifetracksError??null,config:e.lifetracksConfig??null,generating:e.lifetracksGenerating??!1,generationError:e.lifetracksGenerationError??null,onRefresh:()=>e.handleLifetracksRefresh(),onSelectTrack:d=>e.handleLifetracksSelectTrack(d),onEnable:()=>e.handleLifetracksEnable(),onGenerate:()=>e.handleLifetracksGenerate()}):h}

        ${e.tab==="channels"?Qw({connected:e.connected,loading:e.channelsLoading,snapshot:e.channelsSnapshot,lastError:e.channelsError,lastSuccessAt:e.channelsLastSuccess,whatsappMessage:e.whatsappLoginMessage,whatsappQrDataUrl:e.whatsappLoginQrDataUrl,whatsappConnected:e.whatsappLoginConnected,whatsappBusy:e.whatsappBusy,configSchema:e.configSchema,configSchemaLoading:e.configSchemaLoading,configForm:e.configForm,configUiHints:e.configUiHints,configSaving:e.configSaving,configFormDirty:e.configFormDirty,nostrProfileFormState:e.nostrProfileFormState,nostrProfileAccountId:e.nostrProfileAccountId,onRefresh:d=>Oe(e,d),onWhatsAppStart:d=>e.handleWhatsAppStart(d),onWhatsAppWait:()=>e.handleWhatsAppWait(),onWhatsAppLogout:()=>e.handleWhatsAppLogout(),onConfigPatch:(d,$)=>en(e,d,$),onConfigSave:()=>e.handleChannelConfigSave(),onConfigReload:()=>e.handleChannelConfigReload(),onNostrProfileEdit:(d,$)=>e.handleNostrProfileEdit(d,$),onNostrProfileCancel:()=>e.handleNostrProfileCancel(),onNostrProfileFieldChange:(d,$)=>e.handleNostrProfileFieldChange(d,$),onNostrProfileSave:()=>e.handleNostrProfileSave(),onNostrProfileImport:()=>e.handleNostrProfileImport(),onNostrProfileToggleAdvanced:()=>e.handleNostrProfileToggleAdvanced()}):h}

        ${e.tab==="instances"?gk({loading:e.presenceLoading,entries:e.presenceEntries,lastError:e.presenceError,statusMessage:e.presenceStatus,onRefresh:()=>Po(e)}):h}

        ${e.tab==="sessions"?H0({loading:e.sessionsLoading,result:e.sessionsResult,error:e.sessionsError,activeMinutes:e.sessionsFilterActive,limit:e.sessionsFilterLimit,includeGlobal:e.sessionsIncludeGlobal,includeUnknown:e.sessionsIncludeUnknown,basePath:e.basePath,archivedSessions:e.archivedSessions,archivedSessionsLoading:e.archivedSessionsLoading,archivedSessionsExpanded:e.archivedSessionsExpanded,onFiltersChange:d=>{e.sessionsFilterActive=d.activeMinutes,e.sessionsFilterLimit=d.limit,e.sessionsIncludeGlobal=d.includeGlobal,e.sessionsIncludeUnknown=d.includeUnknown},onRefresh:()=>{te(e),Ot(e)},onPatch:async(d,$)=>{const S=await Ps(e,d,$);if(S.ok&&S.canonicalKey!==d&&e.settings.openTabs.includes(d)){const _=e.settings.openTabs.map(P=>P===d?S.canonicalKey:P),x=d===e.sessionKey;e.applySettings({...e.settings,openTabs:_,tabLastViewed:{...e.settings.tabLastViewed,[S.canonicalKey]:e.settings.tabLastViewed[d]??Date.now()},...x?{sessionKey:S.canonicalKey,lastActiveSessionKey:S.canonicalKey}:{}}),x&&(e.sessionKey=S.canonicalKey,we(e,S.canonicalKey,!0))}},onDelete:d=>Jd(e,d),onArchive:d=>Xd(e,d),onUnarchive:d=>Zd(e,d),onToggleArchived:()=>{e.archivedSessionsExpanded=!e.archivedSessionsExpanded,e.archivedSessionsExpanded&&e.archivedSessions.length===0&&Ot(e)},onAutoArchive:()=>eu(e)}):h}

        ${e.tab==="cron"?rk({loading:e.cronLoading,status:e.cronStatus,jobs:e.cronJobs,error:e.cronError,busy:e.cronBusy,form:e.cronForm,channels:e.channelsSnapshot?.channelMeta?.length?e.channelsSnapshot.channelMeta.map(d=>d.id):e.channelsSnapshot?.channelOrder??[],channelLabels:e.channelsSnapshot?.channelLabels??{},channelMeta:e.channelsSnapshot?.channelMeta??[],runsJobId:e.cronRunsJobId,runs:e.cronRuns,onFormChange:d=>e.cronForm={...e.cronForm,...d},onRefresh:()=>e.loadCron(),onAdd:()=>$y(e),onToggle:(d,$)=>ky(e,d,$),onRun:d=>Sy(e,d),onRemove:d=>Ay(e,d),onLoadRuns:d=>mu(e,d)}):h}

        ${e.tab==="skills"?aS({loading:e.skillsLoading,report:e.skillsReport,error:e.skillsError,filter:e.skillsFilter,edits:e.skillEdits,messages:e.skillMessages,busyKey:e.skillsBusyKey,subTab:e.skillsSubTab,onFilterChange:d=>e.skillsFilter=d,onRefresh:()=>is(e,{clearMessages:!0}),onToggle:(d,$)=>Gy(e,d,$),onEdit:(d,$)=>jy(e,d,$),onSaveKey:d=>Qy(e,d),onInstall:(d,$,S)=>Yy(e,d,$,S),onSubTabChange:d=>{e.skillsSubTab=d,d==="clawhub"&&!e.clawhubExploreItems&&xl(e)},clawhub:{loading:e.clawhubLoading,error:e.clawhubError,query:e.clawhubQuery,results:e.clawhubResults,exploreItems:e.clawhubExploreItems,exploreSort:e.clawhubExploreSort,detailSlug:e.clawhubDetailSlug,detail:e.clawhubDetail,importing:e.clawhubImporting,message:e.clawhubMessage,onSearch:d=>{e.clawhubQuery=d,ow(e,d)},onExplore:d=>xl(e,d),onDetail:d=>rw(e,d),onCloseDetail:()=>cw(e),onImport:d=>Cl(e,d),onImportAndPersonalize:async d=>{if(!await Cl(e,d))return;const S=await lw(e,d);S&&(Oo(e,"chat"),wa(e),e.chatMessage=S)}}}):h}

        ${e.tab==="nodes"?l0({loading:e.nodesLoading,nodes:e.nodes,devicesLoading:e.devicesLoading,devicesError:e.devicesError,devicesList:e.devicesList,configForm:e.configForm??e.configSnapshot?.config,configLoading:e.configLoading,configSaving:e.configSaving,configDirty:e.configFormDirty,configFormMode:e.configFormMode,execApprovalsLoading:e.execApprovalsLoading,execApprovalsSaving:e.execApprovalsSaving,execApprovalsDirty:e.execApprovalsDirty,execApprovalsSnapshot:e.execApprovalsSnapshot,execApprovalsForm:e.execApprovalsForm,execApprovalsSelectedAgent:e.execApprovalsSelectedAgent,execApprovalsTarget:e.execApprovalsTarget,execApprovalsTargetNodeId:e.execApprovalsTargetNodeId,onRefresh:()=>ua(e),onDevicesRefresh:()=>ft(e),onDeviceApprove:d=>pv(e,d),onDeviceReject:d=>hv(e,d),onDeviceRotate:(d,$,S)=>fv(e,{deviceId:d,role:$,scopes:S}),onDeviceRevoke:(d,$)=>gv(e,{deviceId:d,role:$}),onLoadConfig:()=>Je(e),onLoadExecApprovals:()=>{const d=e.execApprovalsTarget==="node"&&e.execApprovalsTargetNodeId?{kind:"node",nodeId:e.execApprovalsTargetNodeId}:{kind:"gateway"};return Ro(e,d)},onBindDefault:d=>{d?en(e,["tools","exec","node"],d):_r(e,["tools","exec","node"])},onBindAgent:(d,$)=>{const S=["agents","list",d,"tools","exec","node"];$?en(e,S,$):_r(e,S)},onSaveBindings:()=>Bs(e),onExecApprovalsTargetChange:(d,$)=>{e.execApprovalsTarget=d,e.execApprovalsTargetNodeId=$,e.execApprovalsSnapshot=null,e.execApprovalsForm=null,e.execApprovalsDirty=!1,e.execApprovalsSelectedAgent=null},onExecApprovalsSelectAgent:d=>{e.execApprovalsSelectedAgent=d},onExecApprovalsPatch:(d,$)=>Dy(e,d,$),onExecApprovalsRemove:d=>My(e,d),onSaveExecApprovals:()=>{const d=e.execApprovalsTarget==="node"&&e.execApprovalsTargetNodeId?{kind:"node",nodeId:e.execApprovalsTargetNodeId}:{kind:"gateway"};return Iy(e,d)}}):h}
        ...
        ${e.tab==="chat"&&e.settings.chatParallelView?q1({state:e,onAssignLane:(d,$)=>{const S=$?Me(e.sessionsResult?.sessions,$)?.key??$:null,_=[...e.settings.parallelLanes];_[d]=S,e.applySettings({...e.settings,parallelLanes:_}),S&&e.client&&fo(e.client,S).then(()=>{e.applySettings({...e.settings})})},onReorderLanes:(d,$)=>{if(d===$||d<0||$<0||d>=e.settings.parallelLanes.length||$>=e.settings.parallelLanes.length)return;const S=[...e.settings.parallelLanes],[_]=S.splice(d,1);S.splice($,0,_),e.applySettings({...e.settings,parallelLanes:S})},onLaneViewed:d=>{const $=Me(e.sessionsResult?.sessions,d)?.key??d,S=Date.now(),x=Me(e.sessionsResult?.sessions,$)?.updatedAt??0,P=Math.max(e.settings.tabLastViewed[d]??0,e.settings.tabLastViewed[$]??0);x>0&&P>=x||e.applySettings({...e.settings,tabLastViewed:{...e.settings.tabLastViewed,[d]:S,[$]:S}})},onSendInLane:(d,$)=>{d!==e.sessionKey?(Se(e),e.sessionKey=d,Pe(e,d),e.chatLoading=!0,e.chatMessages=[],e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.applySettings({...e.settings,sessionKey:d,lastActiveSessionKey:d}),e.loadAssistantIdentity(),we(e,d,!0),ae(e).then(()=>{e.chatMessage=$,e.handleSendChat($)})):(e.chatMessage=$,e.handleSendChat($))}}):h}

        ${e.tab==="chat"&&!e.settings.chatParallelView?F$({basePath:e.basePath,sessionKey:e.sessionKey,onSessionKeyChange:d=>{Se(e),e.sessionKey=d,Pe(e,d),e.chatLoading=!0,e.chatMessages=[],e.chatAttachments=[],e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.chatQueue=[],e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:d,lastActiveSessionKey:d}),e.loadAssistantIdentity(),ae(e),Zs(e)},thinkingLevel:e.chatThinkingLevel,showThinking:p,loading:e.chatLoading,sending:e.chatSending,sendingSessionKey:e.chatSendingSessionKey,compactionStatus:e.compactionStatus,assistantAvatarUrl:m,messages:e.chatMessages,toolMessages:e.chatToolMessages,stream:e.chatStream,streamStartedAt:e.chatStreamStartedAt,draft:e.chatMessage,queue:e.chatQueue,connected:e.connected,canSend:e.connected,disabledReason:l,error:e.lastError,sessions:e.sessionsResult,focusMode:u,onRefresh:()=>(e.resetToolStream(),Promise.all([ae(e),Zs(e)])),onToggleFocusMode:()=>{e.onboarding||e.applySettings({...e.settings,chatFocusMode:!e.settings.chatFocusMode})},onChatScroll:d=>e.handleChatScroll(d),onDraftChange:d=>e.chatMessage=d,attachments:e.chatAttachments,onAttachmentsChange:d=>e.chatAttachments=d,showToast:(d,$)=>e.showToast(d,$),onSend:d=>e.handleSendChat(void 0,{queue:d}),canAbort:!!e.chatRunId,onAbort:()=>{e.handleAbortChat()},onCompact:()=>{e.handleCompactChat()},pendingRetry:e.pendingRetry,onRetry:()=>{e.handleRetryMessage()},onClearRetry:()=>e.handleClearRetry(),onQueueRemove:d=>e.removeQueuedMessage(d),onNewSession:()=>e.handleSendChat("/new",{restoreDraft:!0}),onConsciousnessFlush:()=>{e.handleConsciousnessFlush()},consciousnessStatus:e.consciousnessStatus,sidebarOpen:e.sidebarOpen,sidebarContent:e.sidebarContent,sidebarError:e.sidebarError,sidebarMimeType:e.sidebarMimeType,sidebarFilePath:e.sidebarFilePath,sidebarTitle:e.sidebarTitle,splitRatio:e.splitRatio,onOpenSidebar:(d,$)=>e.handleOpenSidebar(d,$),onMessageLinkClick:d=>e.handleOpenMessageFileLink(d),onCloseSidebar:()=>e.handleCloseSidebar(),onOpenFile:d=>e.handleOpenFile(d),onSplitRatioChange:d=>e.handleSplitRatioChange(d),onPushToDrive:(d,$)=>e.handlePushToDrive(d,$),driveAccounts:e.driveAccounts,showDrivePicker:e.showDrivePicker,driveUploading:e.driveUploading,onToggleDrivePicker:()=>e.handleToggleDrivePicker(),onImageClick:(d,$,S)=>e.handleImageClick(d,$,S),resolveImageUrl:(d,$)=>Uv(e.sessionKey,d,$),assistantName:e.assistantName,assistantAvatar:e.assistantAvatar,userName:e.userName,userAvatar:e.userAvatar,currentToolName:e.currentToolName,currentToolInfo:e.currentToolInfo,isWorking:e.workingSessions.has(e.sessionKey),showScrollButton:!e.chatUserNearBottom,showNewMessages:e.chatNewMessagesBelow,onScrollToBottom:()=>{const d=document.querySelector(".chat-thread");d&&(d.scrollTo({top:d.scrollHeight,behavior:"smooth"}),e.chatUserNearBottom=!0,e.chatNewMessagesBelow=!1)},allyPanelOpen:e.allyPanelOpen??!1,allyProps:e.allyPanelOpen?{allyName:e.assistantName,allyAvatar:e.assistantAvatar??null,open:!0,messages:e.allyMessages??[],stream:e.allyStream??null,draft:e.allyDraft??"",sending:e.allySending??!1,isWorking:e.allyWorking??!1,unreadCount:0,connected:e.connected,compact:!0,attachments:e.allyAttachments??[],onToggle:()=>e.handleAllyToggle(),onDraftChange:d=>e.handleAllyDraftChange(d),onSend:()=>e.handleAllySend(),onOpenFullChat:()=>e.handleAllyOpenFull(),onAttachmentsChange:d=>e.handleAllyAttachmentsChange(d)}:null}):h}

        ${e.tab==="options"?DS({connected:e.connected,loading:e.godmodeOptionsLoading,options:e.godmodeOptions,onToggle:(d,$)=>e.handleOptionToggle(d,$),onOpenWizard:e.handleWizardOpen?()=>e.handleWizardOpen?.():void 0}):h}

        ${e.tab==="guardrails"?A1({connected:e.connected,loading:e.guardrailsLoading,data:e.guardrailsData,showAddForm:e.guardrailsShowAddForm,onToggle:(d,$)=>e.handleGuardrailToggle(d,$),onThresholdChange:(d,$,S)=>e.handleGuardrailThresholdChange(d,$,S),onCustomToggle:(d,$)=>e.handleCustomGuardrailToggle(d,$),onCustomDelete:d=>e.handleCustomGuardrailDelete(d),onToggleAddForm:()=>e.handleToggleGuardrailAddForm(),onOpenAllyChat:d=>{e.handleAllyToggle(),d&&e.handleAllyDraftChange(d)}}):h}
