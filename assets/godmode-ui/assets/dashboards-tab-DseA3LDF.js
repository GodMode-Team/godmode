import{d as w,b as r,A as l,o as C,g as k,r as h,t as _}from"./lit-core-CTInmNPB.js";import{a as b,o as S}from"./index-B529-CI3.js";import{B as I}from"./views-settings-C5QwUeqU.js";import{f as y}from"./ctrl-settings-Qu6zIwzj.js";import{loadDashboards as A,loadDashboard as F,deleteDashboard as T,toggleDashboardPin as M}from"./dashboards-CrT3s0NG.js";import"./markdown-i_gIkIP3.js";var P=Object.defineProperty,q=Object.getOwnPropertyDescriptor,c=(s,a,t,i)=>{for(var e=i>1?void 0:i?q(a,t):a,o=s.length-1,u;o>=0;o--)(u=s[o])&&(e=(i?u(a,t,e):u(e))||e);return i&&e&&P(a,t,e),e};const v={all:{icon:"📊",label:"All"},productivity:{icon:"📋",label:"Productivity"},personal:{icon:"🧑",label:"Personal"},business:{icon:"💼",label:"Business"},system:{icon:"⚙️",label:"System"},custom:{icon:"✨",label:"Custom"}},E=[{id:"weekly-impact",name:"Weekly Impact",category:"productivity",description:"Tasks completed, agent outcomes, and trust trends this week",prompt:"Create a weekly impact dashboard. Use these widgets: tasks-summary, queue-status, trust-scores, streak-stats, brief-summary. Show tasks completed vs created, agent queue throughput, trust score trends, and daily streak. Use clean CSS grid with bar charts."},{id:"agent-activity",name:"Agent Activity",category:"system",description:"Queue pipeline, active personas, and trust scores",prompt:"Create an agent activity dashboard. Use these widgets: queue-status, trust-scores, agent-activity. Show queue stats (pending, processing, completed, failed), most active personas, and trust scores by workflow. Use CSS grid layout."},{id:"morning-overview",name:"Morning Overview",category:"productivity",description:"Today's tasks, brief highlights, and queue status",prompt:"Create a morning overview dashboard. Use these widgets: tasks-summary, brief-summary, queue-status, streak-stats. Show today's priorities, daily brief highlights, pending queue items, and your current streak. Use clean CSS grid layout."}];function x(s){return s==="global"?r`<span class="dashboard-card-scope">Global</span>`:r`<span class="dashboard-card-scope">${s}</span>`}function D(s){return Date.now()-new Date(s).getTime()>1440*60*1e3}function $(s){const a=(s.title+" "+(s.description??"")).toLowerCase();return a.includes("health")||a.includes("sleep")||a.includes("oura")||a.includes("energy")||a.includes("goal")?"personal":a.includes("agent")||a.includes("queue")||a.includes("trust")||a.includes("skill")?"system":a.includes("revenue")||a.includes("business")||a.includes("content")||a.includes("metric")?"business":a.includes("task")||a.includes("calendar")||a.includes("morning")||a.includes("impact")||a.includes("weekly")?"productivity":"custom"}function f(s,a){const t=v[s.category]??v.custom;return r`
  ...
  `}function U(s,a,t,i){const e=D(s.updatedAt);return r`
  ...
  `}function O(s){const{activeDashboardHtml:a,activeDashboardManifest:t,isWorking:i}=s;if(!a||!t)return l;const e=D(t.updatedAt);return r`
  ...
  `}function L(s,a,t){const i={all:a.length};for(const e of a){const o=$(e);i[o]=(i[o]??0)+1}return r`
  ...
  `}function H(s){const{loading:a,dashboards:t}=s,i=s.categoryFilter??"all",e=s.templates??E,u=[...i==="all"?t??[]:(t??[]).filter(n=>$(n)===i)].sort((n,p)=>n.pinned&&!p.pinned?-1:!n.pinned&&p.pinned?1:new Date(p.updatedAt).getTime()-new Date(n.updatedAt).getTime()),g=i==="all"?e:e.filter(n=>n.category===i),m=(t??[]).length>0;return r`
    <section class="dashboards-container">
      ...
      ${m&&s.onCategoryFilter?L(i,t??[],s.onCategoryFilter):l}
      ...
      ${a?r`<div class="dashboards-loading"><div class="spinner"></div> Loading dashboards...</div>`:u.length===0&&!m?r`
            ...
            `:r`
              <div class="dashboards-grid">
                ${u.map(n=>U(n,s.onSelectDashboard,s.onDeleteDashboard,s.onTogglePin))}
              </div>
              ${g.length>0?r`
                <div class="dashboards-templates-section">
