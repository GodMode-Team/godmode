...
`)}function gk(e){const t=new Date(e),s=new Date().getTime()-t.getTime(),a=Math.floor(s/(1e3*60));if(a<1)return"Just now";if(a<60)return`${a}m ago`;const i=Math.floor(a/60);return i<24?`${i}h ago`:t.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}function mk(e){const t="VAULT",n=encodeURIComponent(`01-Daily/${e}`);return`obsidian://open?vault=${t}&file=${n}`}let Wn=null,pn=null;function jl(e,t,n=1500){Wn&&clearTimeout(Wn),Wn=setTimeout(()=>{e!==pn&&(pn=e,t(e))},n)}function vk(e,t){Wn&&clearTimeout(Wn),e!==pn&&(pn=e,t(e))}function ei(e){for(const t of e.querySelectorAll('input[type="checkbox"]')){const n=t;n.checked?n.setAttribute("checked",""):n.removeAttribute("checked")}return uk(e.innerHTML)}function yk(e){const{data:t,loading:n,error:s,onRefresh:a,onGenerate:i,onOpenInObsidian:o,onSaveBrief:l,onToggleCheckbox:c,onOpenFile:u}=e;if(n)return r`
  ...
  `}function xk(e){const{workspace:t,itemSearchQuery:n,expandedFolders:s=new Set,showCompletedTasks:a=!1,onItemSearch:i,onBack:o,onItemClick:l,onSessionClick:c,onPinToggle:u,onPinSessionToggle:p,onToggleFolder:f,onToggleTaskComplete:m,onCreateTask:v,onToggleCompletedTasks:$,onStartTask:d,editingTaskId:g,onEditTask:S,onUpdateTask:T,onBatchPushToDrive:_}=e,A=Gl(n,t.pinned).toSorted((B,Oe)=>Oe.modified.getTime()-B.modified.getTime()),x=Ql(n,t.pinnedSessions),L=Gl(n,t.outputs).filter(B=>!t.pinned.some(Oe=>Oe.path===B.path)),R=(t.folderTree?.length??0)>0,Q=R?sp(n,t.folderTree):[],Y=Ql(n,t.sessions),D=new Set(t.pinnedSessions.map(B=>B.key)),O=new Set(t.pinned.map(B=>B.path)),M=n.trim().length>0,W=A.length>0||x.length>0,F=Y.length>0||t.sessions.length===0||M,ue=kk(t.outputs),ke=ue.length>0&&!M,Z={expandedFolders:s,pinnedPaths:O,workspaceId:t.id,onToggleFolder:f,onItemClick:l,onPinToggle:u};return r`
    <div class="workspaces-container">
      ...
      <div class="workspace-content">
        ...
        ${Ck({tasks:t.tasks??[],workspaceName:t.name,showCompleted:a,onToggleTaskComplete:m,onCreateTask:v,onToggleCompletedTasks:$,onStartTask:d,editingTaskId:g,onEditTask:S,onUpdateTask:T})}
  ...
  `}function Ck(e){const{tasks:t,workspaceName:n,showCompleted:s,onToggleTaskComplete:a,onCreateTask:i,onToggleCompletedTasks:o,onStartTask:l,editingTaskId:c,onEditTask:u,onUpdateTask:p}=e,f=Vs(t.filter(v=>v.status==="pending")),m=Vs(t.filter(v=>v.status==="complete"));return r`
    <section class="ws-section">
      <div class="ws-section__header">
        <h3>Tasks</h3>
      ...
      ${l?r`
            ...
            `:r`
              <div class="workspaces-body">
                ...
                ${Lk({tasks:p,taskFilter:f,taskSort:m,onToggleTaskComplete:M,onSetTaskFilter:F,onSetTaskSort:ue,onCreateTask:W,workspaceNames:d,onStartTask:Z,editingTaskId:$,onEditTask:B,onUpdateTask:Oe})}
              </div>
            `}
    </div>
  ...
  `}function Nk(e){const t=Vs(e.todayTasks??[],"due"),n=t.filter(a=>a.status==="pending"),s=t.filter(a=>a.status==="complete");return r`
  ...
  `}function L0(e,t,n,s,a,i){const o=e.updatedAt?z(e.updatedAt):"n/a",l=e.thinkingLevel??"",c=rp(e.modelProvider),u=k0(l,c),p=$0(e.modelProvider),f=e.verboseLevel??"",m=e.reasoningLevel??"",v=e.displayName??e.key,$=e.kind!=="global",d=$?`${Ki("chat",t)}?session=${encodeURIComponent(e.key)}`:null;return r`
    <div class="table-row">
      ...
      <div>
        <select
          .value=${u}
          ?disabled=${i}
          @change=${g=>{const S=g.target.value;n(e.key,{thinkingLevel:S0(S,c)})}}
        >
      ...
      <div>
        ...
        >
          ${y0.map(g=>r`<option value=${g.value}>${g.label}</option>`)}
        </select>
      </div>
      <div>
        <select
        ...
        >
          ${b0.map(g=>r`<option value=${g}>${g||"inherit"}</option>`)}
        </select>
      </div>
  ...
  `}function n1(e,t,n){const{state:s,onAssignLane:a,onSendInLane:i}=n,o=s.sessionsResult?.sessions??[],l=xe(o,t),c=l?.key??t,u=s.workingSessions.has(t)||s.workingSessions.has(c),p=XS(l,t),f=Bs.get(t)??Bs.get(c),m=l?.model??"",v=l?.totalTokens??0,$=s.settings.tabLastViewed[c]??s.settings.tabLastViewed[t]??0,d=l?.updatedAt??0,g=!u&&d>$,T=t===s.sessionKey?s.chatMessages:We.get(t)??We.get(c)??[],_=A=>{A instanceof HTMLElement&&A.dispatchEvent(new CustomEvent("lane-viewed",{detail:{sessionKey:c},bubbles:!0,composed:!0}))};return r`
    <div
    ...
    >
      ...
      >
        ...
        <div class="parallel-col__meta">
          ...
          <span class="parallel-col__tokens">${ZS(v)} tokens</span>
        </div>
      </div>
      ...
      <div class="work-workspaces-section">
        ...
        <div class="my-day-grid" style="grid-template-columns: 1fr;">
          ${m.length===0&&v.length===0?r`
                ...
                `:h}
          ${m.map($=>d1($,a.has($.id),i[$.id]??[],o.has($.id),()=>c?.($.id),u,p,f))}
          ${v.length>0?r`
  ...
  `}const rc={connected:{dot:"●",label:"Connected",cls:"second-brain-source--connected"},available:{dot:"○",label:"Available",cls:"second-brain-source--available"}};function m1(e){const{sourcesData:t}=e;if(!t||t.sources.length===0)return fa("No sources detected","Connect data sources to build your context universe.");const n=t.sources.filter(a=>a.status==="connected"),s=t.sources.filter(a=>a.status==="available");return r`
    <div class="second-brain-panel">
  ...
  `:h}function gc(e){const t=e.trim();if(!t)return t;const n=t.split(/\s+/);if(n.length>=2&&n.length%2===0){const l=n.length/2,c=n.slice(0,l).join(" "),u=n.slice(l).join(" ");if(c.toLowerCase()===u.toLowerCase())return c}const s=t.replace(/\s+/g," ").toLowerCase(),a=Math.floor(s.length/2),i=s.slice(0,a).trim(),o=s.slice(a).trim();return i&&i===o?t.slice(0,Math.ceil(t.length/2)).trim():t}function Di(e,t){const n=t?.sessionId?.trim();return n?`session:${n}`:`key:${e.trim().toLowerCase()}`}function mc(e){if(e===J)return!0;const t=e.toLowerCase();return!!(t==="agent:main:main"||t.endsWith(":main"))}function s2(e){const t=e.sessionsResult?.sessions,n=[...new Set(e.settings.openTabs.map(l=>l.trim()).filter(Boolean))].filter(l=>!mc(l)),s=xe(t,e.sessionKey),a=Di(e.sessionKey,s),i=new Map;for(const l of n){const c=xe(t,l),u=Di(l,c);if(!i.has(u)){i.set(u,l);continue}l===e.sessionKey&&i.set(u,l)}const o=[...i.values()];if(o.length===0){const l=e.sessionKey.trim()||"main";mc(l)||o.push(l)}return{tabKeys:o,activeIdentity:a}}function a2(e){const t=e.onboardingActive&&e.onboarding,n=e.onboardingPhase??0,s=e.onboardingData;if(t&&n===0)return K1(()=>{e.handleOnboardingStart?.()},s?.assessment);if(t&&n===1)return W1(d=>{e.handleOnboardingIdentitySubmit?.(d)});if(t&&n===6)return j1(s?.summary??null,s?.identity??null,()=>{e.handleOnboardingComplete?.()});if(e.wizardActive&&e.wizardState)return up(e.wizardState,{onStepChange:d=>{e.handleWizardStepChange?.(d)},onAnswerChange:(d,g)=>{e.handleWizardAnswerChange?.(d,g)},onPreview:()=>{e.handleWizardPreview?.()},onGenerate:()=>{e.handleWizardGenerate?.()},onClose:()=>{e.handleWizardClose?.()},onFileToggle:(d,g)=>{e.handleWizardFileToggle?.(d,g)},onConfigToggle:(d,g)=>{e.handleWizardConfigToggle?.(d,g)}});const a=e.presenceEntries.length,i=e.sessionsResult?.count??null,o=e.cronStatus?.nextWakeAtMs??null,l=e.connected?null:"Disconnected from gateway.",c=e.tab==="chat",u=c&&(e.settings.chatFocusMode||e.onboarding||t&&n>=2),p=e.onboarding?!1:e.settings.chatShowThinking,f=n2(e),m=e.chatAvatarUrl??f??null,{tabKeys:v,activeIdentity:$}=s2(e);return r`
    <div class="shell ${c?"shell--chat":""} ${u?"shell--chat-focus":""} ${e.settings.navCollapsed?"shell--nav-collapsed":""} ${e.onboarding?"shell--onboarding":""}">
      ...
      <main class="content ${c?"content--chat":""}">
        ...
        ${e.tab==="options"?V0({connected:e.connected,loading:e.godmodeOptionsLoading,options:e.godmodeOptions,onToggle:(d,g)=>e.handleOptionToggle(d,g),onOpenWizard:e.handleWizardOpen?()=>e.handleWizardOpen?.():void 0}):h}

        ${e.tab==="guardrails"?NS({connected:e.connected,loading:e.guardrailsLoading,data:e.guardrailsData,showAddForm:e.guardrailsShowAddForm,onToggle:(d,g)=>e.handleGuardrailToggle(d,g),onThresholdChange:(d,g,S)=>e.handleGuardrailThresholdChange(d,g,S),onCustomToggle:(d,g)=>e.handleCustomGuardrailToggle(d,g),onCustomDelete:d=>e.handleCustomGuardrailDelete(d),onToggleAddForm:()=>e.handleToggleGuardrailAddForm(),onOpenAllyChat:d=>{e.handleAllyToggle(),d&&e.handleAllyDraftChange(d)}}):h}
