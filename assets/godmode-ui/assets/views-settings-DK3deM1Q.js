import{A as d,b as a,o as $a}from"./lit-core-CTInmNPB.js";import{t as wa,f as L,a as Be,b as jn,c as Et,d as Fe}from"./ctrl-settings-C2Q1A5fK.js";import{g as Pt}from"./markdown-i_gIkIP3.js";function xa(e){const{values:t,original:n}=e;return t.name!==n.name||t.displayName!==n.displayName||t.about!==n.about||t.picture!==n.picture||t.banner!==n.banner||t.website!==n.website||t.nip05!==n.nip05||t.lud16!==n.lud16}function ka(e){const{state:t,callbacks:n,accountId:s}=e,i=xa(t),l=(r,u,v={})=>{const{type:p="text",placeholder:g,maxLength:h,help:$}=v,b=t.values[r]??"",y=t.fieldErrors[r],f=`nostr-profile-${r}`;return p==="textarea"?a`
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"No snapshot yet."}
  ...
  `}function wo(e){const n=(e.status&&typeof e.status=="object"?e.status.securityAudit:null)?.summary??null,s=n?.critical??0,i=n?.warn??0,l=n?.info??0,o=s>0?"danger":i>0?"warn":"success",r=s>0?`${s} critical`:i>0?`${i} warnings`:"No critical issues";return a`
    <section class="grid grid-cols-2">
      <div class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
  ...
  `}function ui(e){const t=Math.max(0,e),n=Math.floor(t/1e3);if(n<60)return`${n}s`;const s=Math.floor(n/60);return s<60?`${s}m`:`${Math.floor(s/60)}h`}function pe(e,t){return t?a`<div class="exec-approval-meta-row"><span>${e}</span><span>${t}</span></div>`:d}function xo(e){const t=e.execApprovalQueue[0];if(!t)return d;const n=t.request,s=t.expiresAtMs-Date.now(),i=s>0?`expires in ${ui(s)}`:"expired",l=e.execApprovalQueue.length;return a`
  ...
  `}const Nn=["trace","debug","info","warn","error","fatal"];function vi(e){if(!e)return"";const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleTimeString()}function gi(e,t){return t?[e.message,e.subsystem,e.raw].filter(Boolean).join(" ").toLowerCase().includes(t):!0}function Ao(e){const t=e.filterText.trim().toLowerCase(),n=Nn.some(l=>!e.levelFilters[l]),s=e.entries.filter(l=>l.level&&!e.levelFilters[l.level]?!1:gi(l,t)),i=t||n?"filtered":"visible";return a`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
