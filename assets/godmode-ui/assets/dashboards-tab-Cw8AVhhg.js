...
>>>>>>>> origin/main:assets/godmode-ui/assets/dashboards-tab-Cw8AVhhg.js
  ...
  `}function U(a){const{activeDashboardHtml:s,activeDashboardManifest:e,isWorking:r}=a;if(!s||!e)return l;const t=D(e.updatedAt);return i`
  ...
  `}function H(a,s,e){const r={all:s.length};for(const t of s){const o=w(t);r[o]=(r[o]??0)+1}return i`
  ...
  `}function O(a){const{loading:s,dashboards:e}=a,r=a.categoryFilter??"all",t=a.templates??T,u=[...r==="all"?e??[]:(e??[]).filter(d=>w(d)===r)].sort((d,v)=>d.pinned&&!v.pinned?-1:!d.pinned&&v.pinned?1:new Date(v.updatedAt).getTime()-new Date(d.updatedAt).getTime()),g=r==="all"?t:t.filter(d=>d.category===r),f=(e??[]).length>0;return i`
    <section class="dashboards-container">
      ...
      ${f&&a.onCategoryFilter?H(r,e??[],a.onCategoryFilter):l}

      ${s?i`<div class="dashboards-loading"><div class="spinner"></div> Loading dashboards...</div>`:u.length===0&&!f?i`
            ...
            `:i`
              <div class="dashboards-grid">
                ${u.map(d=>x(d,a.onSelectDashboard,a.onDeleteDashboard,a.onTogglePin))}
              </div>
              ${g.length>0?i`
                <div class="dashboards-templates-section">
