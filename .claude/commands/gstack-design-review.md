---
description: "[gstack] Designer's eye visual audit of live sites with fix loop"
---
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.godmode/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.godmode/skills/gstack/browse/dist/browse"
[ -z "$B" ] && B=~/.godmode/skills/gstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "GodMode browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
First check: If project config already has a `## Testing` section -> skip. Don't duplicate.
...
```bash
REPORT_DIR=".godmode/design-reports"
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. Ask the user: "This site requires authentication. Want to import cookies from your browser?"
...
- Ellipsis character (three-dot unicode) not three dots
...
- Loading states end with three-dot unicode ("Saving..." not "Saving...")
...
**Local:** `.godmode/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`
...
11. **Show screenshots to the user.** After every `$B screenshot`, `$B snapshot -a -o`, or `$B responsive` command, display the output file(s) so the user can see them inline. For `responsive` (3 files), show all three. This is critical -- without it, screenshots are invisible to the user.
...
# Use exec to grep/glob for file patterns matching the affected page
```
...
Write the report to `.godmode/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`
...
11. **Clean working tree required.** If dirty, prompt the user to offer commit/stash/abort before proceeding.
