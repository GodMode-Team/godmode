---
description: "[gstack] CEO/founder-mode plan review with scope expansion, selective expansion, hold, and reduction modes"
---
...
Use `exec` to run the following commands:
```
...
```
Then read project config, TODOS.md, and any existing architecture docs.

**Design doc check:**
Check the `.godmode/projects/` directory for design docs matching the current branch. If a design doc exists (from `/office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
4. **Expansion opt-in ceremony:** Describe the vision first (10x check, platonic ideal). Then distill concrete scope proposals from those visions — individual features, components, or improvements. Present each proposal individually for user approval. Recommend enthusiastically — explain why it's worth doing. But the user decides. Options: **A)** Add to this plan's scope **B)** Defer to TODOS.md **C)** Skip. Accepted items become plan scope for all remaining review sections. Rejected items go to "NOT in scope."
...
Write to `.godmode/projects/{slug}/ceo-plans/{date}-{feature-slug}.md` using this format:
...
For LLM/prompt changes: Check project config for the "Prompt/LLM changes" file patterns. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against.
**STOP.** Ask the user once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
* **One issue = one prompt to the user.** Never combine multiple issues into one question.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only prompt the user when there is a genuine decision with meaningful tradeoffs.
...
### TODOS.md updates
Present each potential TODO individually to the user. Never batch TODOs — one per question. Never silently skip this step.
...
If any question to the user goes unanswered, note it here. Never silently default.

## Next Steps — Review Chaining

After completing the review, recommend the next review(s) based on what this CEO review discovered.
...
**Recommend /plan-design-review if UI scope was detected** — specifically if Section 11 (Design & UX Review) was NOT skipped, or if accepted scope expansions included UI-facing features. In SCOPE REDUCTION mode, skip this recommendation — design review is unlikely relevant for scope cuts.

**If both are needed, recommend eng review first** (required gate), then design review.

Present the next step to the user. Include only applicable options:
...
At the end of the review, if the vision produced a compelling feature direction, offer to promote the CEO plan to the project repo:
...
- **B)** Keep in `.godmode/projects/` only (local, personal reference)
- **C)** Skip
