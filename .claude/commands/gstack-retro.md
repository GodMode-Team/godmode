---
description: "[gstack] Weekly engineering retrospective with commit analysis, work patterns, and trend tracking"
---


# /retro -- Weekly Engineering Retrospective

Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using GodMode/OpenClaw as a force multiplier.
...
Before gathering data, detect the repo's default branch name. Use `exec` to run:
...
First, fetch origin and identify the current user. Use `exec` to run:
```bash
...
Run ALL of these git commands in parallel (they are independent), using `exec` for each:
...
Count consecutive days with at least 1 commit to origin/<default>, going back from today. Track both team streak and personal streak. Use `exec` to run:
...
Before saving the new snapshot, check for prior retro history. Use `exec` to run:

```bash
ls -t .context/retros/*.json 2>/dev/null
```

**If prior retros exist:** Load the most recent one using `exec` to read the file. Calculate deltas for key metrics and include a **Trends vs Last Retro** section:
```
                    Last        Now         Delta
...
After computing all metrics (including streak) and loading any prior history for comparison, save a JSON snapshot. Use `exec` to run:
...
Use `exec` to write the JSON file with this schema:
```json
{
  "date": "2026-03-08",
  "window": "7d",
...
- Do not read project config or other docs -- this skill is self-contained
- On first run (no prior retros), skip comparison sections gracefully

## Completion Status Protocol

When completing the retro workflow, report status using one of:
