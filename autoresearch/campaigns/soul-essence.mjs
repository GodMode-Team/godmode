...
const LLM_API_KEY = XAI_API_KEY || OPENAI_API_KEY;
const LLM_BASE_URL = XAI_API_KEY ? "https://api.x.ai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
const LLM_MODEL = XAI_API_KEY ? "grok-3-mini-fast" : "gpt-4o-mini";
if (!LLM_API_KEY) {
  console.error("XAI_API_KEY or OPENAI_API_KEY not found in environment or ~/.openclaw/.env");
  process.exit(1);
}
console.log(`[soul-essence] Using LLM judge: ${LLM_MODEL} via ${XAI_API_KEY ? "XAI" : "OpenAI"}`);
...
const SCENARIOS = [
  ...
  { user: "I need to follow up with Jake", ideal: "Looks up Jake in memory/vault, creates a task, suggests next action. Doesn't ask 'Who is Jake?'", weight: 1.5 },
  { user: "Draft a competitive analysis for Patient Autopilot", ideal: "Immediately queues this as a research task, doesn't ask clarifying questions if enough context exists.", weight: 1 },
  ...
  { user: "Set up a meeting with Sarah about the redesign", ideal: "Looks up Sarah, checks calendar availability, proposes time. Doesn't ask 'Sarah who?'", weight: 1.5 },
