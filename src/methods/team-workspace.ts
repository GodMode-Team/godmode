...
const resolveConflict: GatewayRequestHandler = async ({ params, respond }) => {
  ...
  try {
    ...
    if (strategy === "ours") {
      ...
      await execFile("git", ["pull", "--no-rebase", "-X", "ours"], opts).catch((err) => console.warn("[team-workspace] git pull (ours) failed:", err.message));
    } else {
      // "theirs" = accept remote changes
      await execFile("git", ["pull", "--no-rebase", "-X", "theirs"], opts).catch((err) => console.warn("[team-workspace] git pull (theirs) failed:", err.message));
    }
